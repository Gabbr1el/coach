...
describe('academic planning from Home conversation', () => {
  ...
  it('updates availability used by replanning', () => { const repository = new AcademicRepository(); new PlanningService(repository, () => now).applyAcademicMessage('Na terça só vou ter 2 horas para estudar', { currentTime: now, timezone }); expect(repository.availability).toContainEqual({ weekday: 2, minutes: 120 }) })

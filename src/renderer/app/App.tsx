...
import { lessonFor, nextStudyStage, type StudyStage } from './study-lesson'
...
function StudyModuleView({ module, roadmap, studyProgress, onSelect, onPosition, onCheckpoint, onComplete, onBack, onAskCoach, onPractice }: { module: RoadmapModule; roadmap: Roadmap; studyProgress: StudyProgressState; onSelect(module: RoadmapModule, topicIndex: number): void; onPosition(position: StudyLessonPosition): void; onCheckpoint(correct: boolean): void; onComplete(): void; onBack(): void; onAskCoach(prompt: string): void; onPractice(): void }) {
  const topicIndex = Math.max(0, module.topics.findIndex((topic) => `${module.id}:${topic}` === studyProgress.topicId))
  const savedPosition = studyProgress.currentPosition
  const latestPosition = useRef<StudyLessonPosition | null>(savedPosition)
  const [stage, setStage] = useState<StudyStage>(savedPosition?.currentStage ?? 'explanation')
  const [selected, setSelected] = useState<number | null>(savedPosition?.selectedAnswer ?? null)
  const [attempts, setAttempts] = useState(savedPosition?.attempt ?? 0)
  const topic = module.topics[topicIndex] ?? module.title
  const lesson = lessonFor(topic, module)
  useEffect(() => { const position = studyProgress.currentPosition; latestPosition.current = position; setStage(position?.currentStage ?? 'explanation'); setSelected(position?.selectedAnswer ?? null); setAttempts(position?.attempt ?? 0) }, [lesson.id, studyProgress.currentPosition])
  const completed = roadmap?.modules.filter((item) => item.status === 'completed').length ?? 0
  const progressPercent = Math.round((completed + topicIndex / Math.max(1, module.topics.length)) / Math.max(1, roadmap.modules.length) * 100)
  const correct = selected === lesson.verification.correct
  const persistPosition = (nextStage: StudyStage, patch: Partial<StudyLessonPosition> = {}) => { const current = latestPosition.current?.lessonId === lesson.id ? latestPosition.current : null; const completed = current?.completedBlockIds ?? []; const currentBlockId = `${lesson.id}:${nextStage}`; const position = { lessonId: lesson.id, currentBlockId, currentStage: nextStage, currentCheckpointId: nextStage === 'verification' || nextStage === 'feedback' ? lesson.verification.id : current?.currentCheckpointId ?? null, currentExerciseId: nextStage === 'exercise' ? `${lesson.id}:exercise` : current?.currentExerciseId ?? null, completedBlockIds: completed, selectedAnswer: selected, attempt: attempts, feedback: current?.feedback ?? null, reinforcementBlocks: current?.reinforcementBlocks ?? [], ...patch }; latestPosition.current = position; onPosition(position) }
  const moveTo = (nextStage: StudyStage, completedStage?: StudyStage, patch: Partial<StudyLessonPosition> = {}) => { setStage(nextStage); const current = latestPosition.current?.completedBlockIds ?? []; const completedBlockId = completedStage ? `${lesson.id}:${completedStage}` : null; persistPosition(nextStage, { ...patch, completedBlockIds: completedBlockId && !current.includes(completedBlockId) ? [...current, completedBlockId] : current }) }
  const resetTopic = (index: number) => { onSelect(module, index) }
  const choose = (index: number) => {
    const answerCorrect = index === lesson.verification.correct
    const nextAttempt = attempts + 1
    const nextStage = nextStudyStage('verification', answerCorrect)
    const reinforcement = answerCorrect ? [] : [lesson.reinforcement]
    setSelected(index); setAttempts(nextAttempt); setStage(nextStage)
    persistPosition(nextStage, { selectedAnswer: index, attempt: nextAttempt, feedback: answerCorrect ? lesson.success : lesson.reinforcement, reinforcementBlocks: reinforcement, currentCheckpointId: lesson.verification.id, completedBlockIds: answerCorrect ? [...new Set([...(savedPosition?.completedBlockIds ?? []), `${lesson.id}:verification`])] : savedPosition?.completedBlockIds ?? [] })
    onCheckpoint(answerCorrect)
    const gap = lesson.verification.gaps[index] ?? 'conceito central'
    onAskCoach(`Em Estudos, apos a explicacao de "${topic}", escolhi "${lesson.verification.options[index]}". ${index === lesson.verification.correct ? 'A resposta esta correta; proponha uma aplicacao curta sem presumir dominio.' : `A lacuna provavel e ${gap}. Reensine somente esse ponto com outra analogia, sem revelar a resposta, e peca nova tentativa.`}`)
  }
  const advance = () => { onComplete(); if (topicIndex < module.topics.length - 1) resetTopic(topicIndex + 1); else onPractice() }
  const stages: Array<[StudyStage, string]> = [['explanation', 'Explicacao'], ['example', 'Exemplo'], ['verification', 'Verificacao'], ['exercise', 'Exercicio'], ['feedback', 'Feedback']]
  const activeStage = stages.findIndex(([value]) => value === stage)
  return <div className="grid h-full min-h-0 grid-cols-[210px_minmax(0,1fr)] bg-[#0b0c10]"><aside className="coach-scroll-pane border-r border-coach-line bg-[#0e0f13] p-4"><button onClick={onBack} className="text-xs font-black text-coach-muted">Voltar ao plano</button><p className="mt-6 text-[10px] font-black uppercase tracking-wider text-coach-muted">Roteiro</p><p className="mt-1 text-xs text-coach-green">{progressPercent}% concluido</p><div className="mt-2 h-1 rounded bg-coach-line"><div className="h-full rounded bg-coach-green" style={{ width: `${progressPercent}%` }} /></div><div className="mt-5 space-y-2">{roadmap.modules.map((item) => <div key={item.id}><button onClick={() => onSelect(item, 0)} className={`w-full rounded-lg px-3 py-2 text-left text-xs font-black ${item.id === module.id ? 'bg-[#1a1c22] text-white' : 'text-coach-muted'}`}>{item.position}. {item.title}</button>{item.id === module.id && <div className="ml-3 mt-1 border-l border-coach-line pl-3">{item.topics.map((entry, index) => { const id = `${item.id}:${entry}`; const status = studyProgress.topicStatuses[id] ?? 'NOT_STARTED'; return <button key={entry} onClick={() => resetTopic(index)} className={`block w-full py-1.5 text-left text-[11px] ${index === topicIndex ? 'font-black text-coach-orange' : 'text-coach-muted'}`}>{index === topicIndex ? '>' : status === 'COMPLETED' ? 'x' : status === 'IN_PROGRESS' ? '-' : 'o'} {entry}</button> })}</div>}</div>)}</div></aside><article className="coach-scroll-pane h-full px-6 py-7 lg:px-12"><div className="mx-auto max-w-4xl"><div className="flex items-center justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-coach-green">Tutor adaptativo - ensino antes da avaliacao</p><h2 className="mt-2 font-display text-3xl font-black">{topic}</h2></div><span className="rounded-full border border-coach-line px-3 py-2 text-[10px] font-black text-coach-muted">ESTUDO - {Math.max(3, Math.round(module.estimatedMinutes / Math.max(1, module.topics.length)))} MIN</span></div><div className="mt-6 flex flex-wrap gap-2">{stages.map(([id, label], index) => <span key={id} className={`rounded-full px-3 py-1.5 text-[10px] font-black ${id === stage ? 'bg-coach-green text-[#0c0d10]' : activeStage > index ? 'bg-coach-green/15 text-coach-green' : 'border border-coach-line text-coach-muted'}`}>{index + 1}. {label}</span>)}</div>{stage === 'explanation' && <section className="coach-enter mt-6 rounded-xl border border-coach-line bg-[#111217] p-6"><p className="text-[10px] font-black uppercase tracking-wider text-coach-orange">Primeiro, vamos construir o conceito</p><h3 className="mt-2 font-display text-xl font-black">Como e por que funciona</h3><p className="mt-4 text-sm leading-7 text-coach-muted">{lesson.concept}</p><p className="mt-3 text-sm leading-7 text-coach-muted">{lesson.whyItWorks}</p><ol className="mt-5 space-y-3">{lesson.steps.map((step, index) => <li key={step} className="rounded-lg bg-[#17191f] p-3 text-sm leading-6 text-[#d5d8df]"><strong className="mr-2 text-coach-green">{index + 1}.</strong>{step}</li>)}</ol><button onClick={() => moveTo(nextStudyStage(stage), stage)} className="mt-6 rounded-lg bg-coach-green px-5 py-3 text-xs font-black text-[#0c0d10]">Ver exemplo passo a passo</button></section>}{stage === 'example' && <section className="coach-enter mt-6 rounded-xl border border-coach-line bg-[#111217] p-6"><p className="text-[10px] font-black uppercase tracking-wider text-coach-orange">Exemplo guiado</p><pre className="mt-4 overflow-x-auto rounded-lg bg-[#090a0d] p-4 text-xs text-[#d5d8df]"><code>{lesson.example}</code></pre><ol className="mt-5 space-y-3">{lesson.walkthrough.map((step, index) => <li key={step} className="flex gap-3 text-sm leading-6 text-coach-muted"><strong className="text-coach-green">{index + 1}</strong><span>{step}</span></li>)}</ol><button onClick={() => moveTo(nextStudyStage(stage), stage, { currentCheckpointId: lesson.verification.id })} className="mt-6 rounded-lg bg-coach-green px-5 py-3 text-xs font-black text-[#0c0d10]">Verificar meu entendimento</button></section>}{stage === 'verification' && <section className="coach-enter mt-6 rounded-xl border border-coach-line bg-[#111217] p-6"><p className="text-[10px] font-black uppercase tracking-wider text-coach-orange">Verificacao apos a explicacao</p><h3 className="mt-2 font-display text-xl font-black">{lesson.verification.question}</h3><div className="mt-4 grid gap-2">{lesson.verification.options.map((option, index) => <button key={option} onClick={() => choose(index)} className="rounded-lg border border-coach-line bg-[#17191f] p-3 text-left text-xs hover:border-coach-green">{String.fromCharCode(65 + index)}. {option}</button>)}</div></section>}{stage === 'exercise' && <section className="coach-enter mt-6 rounded-xl border border-coach-green/40 bg-[#111217] p-6"><p className="text-[10px] font-black uppercase tracking-wider text-coach-green">Exercicio de aplicacao</p><h3 className="mt-2 font-display text-xl font-black">Agora faca, nao apenas reconheca</h3><p className="mt-3 text-sm leading-7 text-coach-muted">{lesson.exercise}</p><div className="mt-5 flex flex-wrap gap-3"><button onClick={onPractice} className="rounded-lg bg-coach-green px-5 py-3 text-xs font-black text-[#0c0d10]">Abrir Pratica</button><button onClick={() => moveTo(nextStudyStage(stage), stage)} className="rounded-lg border border-coach-line px-5 py-3 text-xs font-black text-coach-muted">Conclui o exercicio</button></div><p className="mt-4 text-xs text-coach-muted">Abrir o topico ou a pratica nao significa dominio; o Coach usa verificacao e exercicio como evidencias separadas.</p></section>}{stage === 'feedback' && <section className={`coach-enter mt-6 rounded-xl border p-6 ${correct ? 'border-coach-green/40 bg-coach-green/10' : 'border-coach-orange/40 bg-coach-orange/10'}`}><p className="text-[10px] font-black uppercase tracking-wider text-coach-orange">Feedback da tentativa {attempts}</p><h3 className="mt-2 font-display text-xl font-black">{correct ? 'Compreensao aplicada' : 'Vamos reconstruir um ponto especifico'}</h3><p className="mt-3 text-sm leading-7 text-coach-muted">{correct ? lesson.success : lesson.reinforcement}</p>{!correct && <p className="mt-3 text-xs text-coach-muted">Ainda nao mostrarei a alternativa correta. Releia esta explicacao por outra perspectiva e tente novamente.</p>}<button onClick={() => { if (correct) advance(); else { setSelected(null); moveTo(nextStudyStage(stage, false), undefined, { selectedAnswer: null }) } }} className="mt-5 rounded-lg bg-coach-green px-5 py-3 text-xs font-black text-[#0c0d10]">{correct ? 'Ir para o proximo conteudo' : 'Tentar novamente'}</button></section>}</div></article></div>
}
...
export function App() {
  ...
  function sendWorkspaceMessage(contentOverride?: string) {
    ...
    if (workspacePage === 'studies' && studyProgress && studyModule && /\b(não entendi|nao entendi|ajuda|dica|explique|explica)\b/i.test(content)) {
      void window.coach.studyProgress.record({ workspaceId: selected.id, type: 'HELP_USED', moduleId: studyProgress.moduleId, topicId: studyProgress.topicId, lessonId: studyProgress.lessonId, checkpointId: studyProgress.checkpointId })
    }
    ...
    const activeLesson = activeTopic && studyModule ? lessonFor(activeTopic, studyModule) : null
    workspaceStreamHandle.current = window.coach.conversation.streamWorkspaceMessage({ requestId, workspaceId, content, activePage: workspacePage, activeStudy: studyModule && studyProgress && activeTopic && activeLesson ? { moduleId: studyModule.id, module: studyModule.title, topicId: studyProgress.topicId, topic: activeTopic, lessonId: activeLesson.id, checkpointId: studyProgress.checkpointId, currentExcerpt: studyProgress.checkpointId ? activeLesson.verification.question : activeLesson.concept } : undefined, practiceContext: practiceContext ? { fileName: practiceContext.fileName, language: practiceContext.language, code: practiceContext.code } : undefined, lastExecution: latestExecution ? { stdout: latestExecution.stdout, stderr: latestExecution.stderr, exitCode: latestExecution.exitCode, timedOut: latestExecution.timedOut } : null }, (event) => {
  ...
  if (selected) {
    ...
    const adaptiveMinutes = Math.max(10, Math.min(60, livePriority?.eventPhase === 'today' ? 15 : livePriority?.eventPhase === 'near' ? Math.min(45, activeDuration ?? 30) : activeDuration ?? (observerState?.focusExitCount && observerState.focusExitCount >= 3 ? 15 : 25)))
    ...
    const selectStudyTopic = async (roadmap: Roadmap, module: RoadmapModule, topicIndex: number) => { const topic = module.topics[topicIndex] ?? module.title; const topicId = `${module.id}:${topic}`; const lesson = lessonFor(topic, module); const progress = await window.coach.studyProgress.select({ workspaceId: selected.id, roadmapId: roadmap.id, moduleId: module.id, topicId, lessonId: lesson.id, checkpointId: null }); setStudyRoadmap(roadmap); setStudyModule(module); setStudyProgress(progress) }
    const openStudies = (moduleId?: string) => { setWorkspacePage('studies'); void Promise.all([window.coach.roadmap.get(selected.id), window.coach.studyProgress.get(selected.id)]).then(async ([roadmap, saved]) => { if (!roadmap) { setStudyRoadmap(null); setStudyModule(null); setStudyProgress(null); return } const restoredModule = saved?.roadmapId === roadmap.id ? roadmap.modules.find((item) => item.id === saved.moduleId) : null; const requested = roadmap.modules.find((item) => item.id === moduleId); const module = requested ?? restoredModule ?? roadmap.modules.find((item) => item.status === 'active' || item.status === 'available') ?? roadmap.modules[0]; if (!module) return; if (saved?.roadmapId === roadmap.id && saved.moduleId === module.id && module.topics.some((topic) => `${module.id}:${topic}` === saved.topicId)) { setStudyRoadmap(roadmap); setStudyModule(module); setStudyProgress(saved) } else await selectStudyTopic(roadmap, module, 0) }).catch(() => setWorkspaceError('Não foi possível abrir os estudos.')) }
    const selectPage = (page: WorkspacePage) => { if (page === 'studies') openStudies(); else { setStudyModule(null); if (page === 'materials') openMaterials(); else if (page === 'reports') openReports(); else setWorkspacePage(page) } }
    const openStudyModule = (moduleId: string) => openStudies(moduleId)
    return (
      <WorkspaceShell name={selected.name} objective={selected.objective} page={workspacePage} timerLabel={timerLabel} timerRunning={studyState?.timerStatus === 'running'} finishing={sessionCompleting} shareContext={studyState?.shareContextWithAi ?? false} coachMessages={workspaceMessages} streamedMessage={workspaceStreamedContent} coachInput={workspaceInput} coachBusy={workspaceSending || workspaceLoading} coachError={workspaceError} conversationRef={workspaceConversationPane} messageEndRef={workspaceMessageEnd} onPage={selectPage} onHome={() => { setSelected(null); setHomeSection('home'); void window.coach.planning.listPriorities().then(setPriorities) }} onSettings={() => setProviderDialogOpen(true)} onToggleTimer={() => { if (!studyState || timerUpdating) return; const workspaceId = selected.id; const epoch = workspaceLoadEpoch.current; const action = studyState.timerStatus === 'running' ? 'pause' : 'start'; setTimerUpdating(true); const configure = action === 'start' && studyState.timerStatus === 'idle' && studyState.timerDurationSeconds !== adaptiveMinutes * 60 ? window.coach.studyWorkspace.setTimerDuration({ workspaceId, durationSeconds: adaptiveMinutes * 60 }) : Promise.resolve(studyState); void configure.then(() => window.coach.studyWorkspace.updateTimer({ workspaceId, action })).then(applyStudyState(workspaceId, epoch)).catch(() => setWorkspaceError('Não foi possível atualizar o foco.')).finally(() => setTimerUpdating(false)) }} onFinish={() => { if (sessionCompleting) return; setSessionCompleting(true); void window.coach.studyWorkspace.completeSession(selected.id).then(setStudyState).finally(() => setSessionCompleting(false)) }} onShareContext={(enabled) => { void window.coach.studyWorkspace.updateContextSharing({ workspaceId: selected.id, enabled }).then(setStudyState).catch(() => setWorkspaceError('Não foi possível atualizar o compartilhamento de contexto.')) }} onCoachInput={setWorkspaceInput} onCoachSend={sendWorkspaceMessage} onNotes={() => setNotesOpen(true)}>
        {workspacePage === 'studies' && livePriority?.eventPhase === 'today' && <div className="border-b border-coach-orange/30 bg-coach-orange/10 px-6 py-3 text-xs text-coach-muted"><strong className="text-coach-orange">Revisão para a prova hoje.</strong> Priorizando explicações objetivas, exemplos e aplicações curtas; evite conteúdo pesado novo.</div>}{workspacePage === 'studies' && studyModule && studyRoadmap && studyProgress && <StudyModuleView module={studyModule} roadmap={studyRoadmap} studyProgress={studyProgress} onSelect={(module, topicIndex) => { void selectStudyTopic(studyRoadmap, module, topicIndex) }} onPosition={(position) => { void window.coach.studyProgress.updatePosition({ workspaceId: selected.id, position }).then(setStudyProgress) }} onCheckpoint={(correct) => { void window.coach.studyProgress.record({ workspaceId: selected.id, type: 'CHECKPOINT_ANSWERED', moduleId: studyProgress.moduleId, topicId: studyProgress.topicId, lessonId: studyProgress.lessonId, checkpointId: studyProgress.checkpointId, correct }) }} onComplete={() => { void window.coach.studyProgress.record({ workspaceId: selected.id, type: 'TOPIC_COMPLETED', moduleId: studyProgress.moduleId, topicId: studyProgress.topicId, lessonId: studyProgress.lessonId, checkpointId: studyProgress.checkpointId }).then(() => window.coach.studyProgress.get(selected.id)).then((value) => { if (value) setStudyProgress(value) }) }} onBack={() => setWorkspacePage('plan')} onAskCoach={sendWorkspaceMessage} onPractice={() => setWorkspacePage('practice')} />}
        {workspacePage === 'studies' && !studyModule && <div className="coach-scroll-pane grid h-full place-items-center p-8 text-sm text-coach-muted">Gere ou aceite um roadmap no Plano para começar os estudos.</div>}

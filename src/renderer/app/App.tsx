...
export function App() {
  ...
  if (selected) {
    ...
    const selectStudyTopic = async (roadmap: Roadmap, module: RoadmapModule, topicIndex: number) => { const topic = module.topics[topicIndex] ?? module.title; const topicId = `${module.id}:${topic}`; setStudyLesson(null); const lesson = await window.coach.studyLesson.getOrCreate({ workspaceId: selected.id, roadmapId: roadmap.id, moduleId: module.id, topicId }); const progress = await window.coach.studyProgress.select({ workspaceId: selected.id, roadmapId: roadmap.id, moduleId: module.id, topicId, lessonId: lesson.id, checkpointId: null }); setStudyRoadmap(roadmap); setStudyModule(module); setStudyLesson(lesson); setStudyProgress(progress) }
    const openStudies = (moduleId?: string) => { setWorkspacePage('studies'); void Promise.all([window.coach.roadmap.get(selected.id), window.coach.studyProgress.get(selected.id)]).then(async ([roadmap, saved]) => { if (!roadmap) { setStudyRoadmap(null); setStudyModule(null); setStudyLesson(null); setStudyProgress(null); return } const restoredModule = saved?.roadmapId === roadmap.id ? roadmap.modules.find((item) => item.id === saved.moduleId) : null; const requested = roadmap.modules.find((item) => item.id === moduleId); const module = requested ?? restoredModule ?? roadmap.modules.find((item) => item.status === 'active' || item.status === 'available') ?? roadmap.modules[0]; if (!module) return; if (saved?.roadmapId === roadmap.id && saved.moduleId === module.id && module.topics.some((topic) => `${module.id}:${topic}` === saved.topicId)) { const lesson = await window.coach.studyLesson.getOrCreate({ workspaceId: selected.id, roadmapId: roadmap.id, moduleId: module.id, topicId: saved.topicId }); setStudyRoadmap(roadmap); setStudyModule(module); setStudyLesson(lesson); setStudyProgress(saved) } else await selectStudyTopic(roadmap, module, 0) }).catch(() => setWorkspaceError('Não foi possível abrir os estudos.')) }
    const selectPage = (page: WorkspacePage) => { if (page === 'studies') openStudies(); else { setStudyModule(null); if (page === 'materials') openMaterials(); else if (page === 'reports') openReports(); else if (page === 'plan') { setWorkspacePage(page); void window.coach.studyWorkspace.recalculatePlan({ workspaceId: selected.id }).then(setStudyState).catch(() => setWorkspaceError('Não foi possível recalcular o plano de hoje.')) } else setWorkspacePage(page) } }
    ...
    return (
      <WorkspaceShell name={selected.name} objective={selected.objective} page={workspacePage} timerLabel={timerLabel} timerRunning={studyState?.timerStatus === 'running'} finishing={sessionCompleting} shareContext={studyState?.shareContextWithAi ?? false} coachMessages={workspaceMessages} streamedMessage={workspaceStreamedContent} coachInput={workspaceInput} coachBusy={workspaceSending || workspaceLoading} coachError={workspaceError} conversationRef={workspaceConversationPane} messageEndRef={workspaceMessageEnd} onPage={selectPage} onHome={() => { setSelected(null); setHomeSection('home'); void window.coach.planning.listPriorities().then(setPriorities) }} onSettings={() => setProviderDialogOpen(true)} onToggleTimer={() => { if (!studyState || timerUpdating) return; const workspaceId = selected.id; const epoch = workspaceLoadEpoch.current; const action = studyState.timerStatus === 'running' ? 'pause' : 'start'; setTimerUpdating(true); const configure = action === 'start' && studyState.timerStatus === 'idle' && studyState.timerDurationSeconds !== adaptiveMinutes * 60 ? window.coach.studyWorkspace.setTimerDuration({ workspaceId, durationSeconds: adaptiveMinutes * 60 }) : Promise.resolve(studyState); void configure.then(() => window.coach.studyWorkspace.updateTimer({ workspaceId, action })).then(applyStudyState(workspaceId, epoch)).catch(() => setWorkspaceError('Não foi possível atualizar o foco.')).finally(() => setTimerUpdating(false)) }} onFinish={() => { if (sessionCompleting) return; setSessionCompleting(true); void window.coach.studyWorkspace.completeSession(selected.id).then(setStudyState).finally(() => setSessionCompleting(false)) }} onShareContext={(enabled) => { void window.coach.studyWorkspace.updateContextSharing({ workspaceId: selected.id, enabled }).then(setStudyState).catch(() => setWorkspaceError('Não foi possível atualizar o compartilhamento de contexto.')) }} onCoachInput={setWorkspaceInput} onCoachSend={sendWorkspaceMessage} onNotes={() => setNotesOpen(true)}>
        ...
        {workspacePage === 'plan' && <div className="coach-scroll-pane h-full p-5 lg:p-8"><div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[minmax(0,1fr)_330px]"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-coach-green">Plano de hoje</p><h2 className="mt-2 font-display text-3xl font-black">Seu roteiro de estudo</h2><div className="mt-6 space-y-3">{studyState?.plan.map((item) => <button disabled={planUpdating} key={item.id} onClick={() => { setPlanUpdating(true); void window.coach.studyWorkspace.activatePlanItem({ workspaceId: selected.id, itemId: item.id }).then(setStudyState).finally(() => setPlanUpdating(false)) }} className={`flex w-full items-center gap-4 rounded-lg border p-4 text-left ${item.status === 'active' ? 'border-coach-yellow bg-[#181622]' : 'border-coach-line bg-[#111217]'}`}><span className={`grid size-7 place-items-center rounded-full border ${item.status === 'completed' ? 'border-coach-green bg-coach-green text-white' : 'border-coach-line'}`}>{item.status === 'completed' ? '✓' : item.position}</span><span className="min-w-0"><strong className="block truncate">{item.title}</strong><small className="text-coach-muted">{item.scheduledStartMinutes !== undefined ? `${String(Math.floor(item.scheduledStartMinutes / 60)).padStart(2, '0')}:${String(item.scheduledStartMinutes % 60).padStart(2, '0')} · ` : ''}{item.durationMinutes} min · {item.status}</small></span></button>)}</div></div><RoadmapPanel workspaceId={selected.id} onError={setWorkspaceError} onStudy={openStudyModule} /></div></div>}

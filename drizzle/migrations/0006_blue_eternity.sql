...
CREATE TABLE `__new_provider_configurations` (
	...
	CONSTRAINT "provider_configurations_base_url_check" CHECK(("__new_provider_configurations"."provider_id" = 'openai' and "__new_provider_configurations"."base_url" is null) or ("__new_provider_configurations"."provider_id" = 'openai-compatible' and "__new_provider_configurations"."base_url" is not null and length(trim("__new_provider_configurations"."base_url")) > 0))
);
--> statement-breakpoint
INSERT INTO `__new_provider_configurations`("id", "provider_id", "display_name", "label", "base_url", "model", "secret_reference", "is_active", "created_at", "updated_at") SELECT "id", "provider_id", "display_name", "label", "base_url", "model", "secret_reference", "is_active", "created_at", "updated_at" FROM `provider_configurations`;--> statement-breakpoint

...
import type { PersistedStudyLesson } from '../../shared/contracts/study-lesson-contract'
...
export function App() {
  ...
  useEffect(() => {
    if (!selected || workspacePage !== 'studies' || !learningPathState || learningPathState.status === 'ready') return
    ...
    const poll = async () => {
      ...
      try {
        ...
        const lesson = await window.coach.studyLesson.getOrCreate({ workspaceId, roadmapId: roadmap.id, moduleId: module.id, topicId })
        const progress = saved?.topicId === topicId ? saved : await window.coach.studyProgress.select({ workspaceId, roadmapId: roadmap.id, moduleId: module.id, topicId, lessonId: lesson.id, checkpointId: null })
        if (!cancelled && studySelectionEpoch.current === selectionEpoch) { setStudyRoadmap(roadmap); setStudyModule(module); setStudyLesson(lesson); setStudyProgress(progress); setLearningPathState(state) }
      } catch { if (!cancelled) setWorkspaceError('Não foi possível acompanhar a preparação da Trilha agora.') }
      finally { polling = false }
    }
    const interval = window.setInterval(() => void poll(), 2500)
    return () => { cancelled = true; window.clearInterval(interval) }
  }, [selected?.id, workspacePage, learningPathState?.status])
  ...
  function sendWorkspaceMessage(contentOverride?: string) {
    ...
    const currentExcerpt = currentBlock ? 'content' in currentBlock ? currentBlock.content : currentBlock.type === 'codeExample' ? currentBlock.code : currentBlock.type === 'checkpoint' ? currentBlock.question : currentBlock.instruction : null
  ...
  if (selected) {
    ...
    const selectStudyTopic = async (roadmap: Roadmap, module: RoadmapModule, topicIndex: number) => { const selectionEpoch = ++studySelectionEpoch.current; const topic = module.topics[topicIndex] ?? module.title; const topicId = `${module.id}:${topic}`; const lesson = await window.coach.studyLesson.getOrCreate({ workspaceId: selected.id, roadmapId: roadmap.id, moduleId: module.id, topicId }); const progress = await window.coach.studyProgress.select({ workspaceId: selected.id, roadmapId: roadmap.id, moduleId: module.id, topicId, lessonId: lesson.id, checkpointId: null }); if (studySelectionEpoch.current !== selectionEpoch) return; setStudyRoadmap(roadmap); setStudyModule(module); setStudyLesson(lesson); setStudyProgress(progress) }
    const openStudies = (moduleId?: string) => { const selectionEpoch = ++studySelectionEpoch.current; setWorkspacePage('studies'); setLearningPathOpen(false); setWorkspaceError(null); setLearningPathState(null); setStudyRoadmap(null); setStudyModule(null); setStudyLesson(null); setStudyProgress(null); void window.coach.roadmap.getLearningPathState(selected.id).then(async (pathState) => { if (studySelectionEpoch.current !== selectionEpoch) return; setLearningPathState(pathState); const [roadmap, saved] = await Promise.all([window.coach.roadmap.get(selected.id), window.coach.studyProgress.get(selected.id)]); if (studySelectionEpoch.current !== selectionEpoch) return; if (!roadmap) { if (pathState.status === 'ready') setWorkspaceError('A Trilha está pronta, mas não foi possível carregar seu conteúdo.'); return } const restoredModule = saved?.roadmapId === roadmap.id ? roadmap.modules.find((item) => item.id === saved.moduleId) : null; const requested = roadmap.modules.find((item) => item.id === moduleId); const module = requested ?? restoredModule ?? roadmap.modules.find((item) => item.status === 'active' || item.status === 'available') ?? roadmap.modules[0]; if (!module) { setWorkspaceError('A Trilha está pronta, mas não possui módulos disponíveis.'); return } if (saved?.roadmapId === roadmap.id && saved.moduleId === module.id && module.topics.some((topic) => `${module.id}:${topic}` === saved.topicId)) { const lesson = await window.coach.studyLesson.getOrCreate({ workspaceId: selected.id, roadmapId: roadmap.id, moduleId: module.id, topicId: saved.topicId }); if (studySelectionEpoch.current !== selectionEpoch) return; setStudyRoadmap(roadmap); setStudyModule(module); setStudyLesson(lesson); setStudyProgress(saved) } else if (studySelectionEpoch.current === selectionEpoch) await selectStudyTopic(roadmap, module, 0) }).catch(() => { if (studySelectionEpoch.current === selectionEpoch) setWorkspaceError('Não foi possível abrir os estudos agora.') }) }
    ...
    return (
      <WorkspaceShell name={selected.name} objective={selected.objective} page={workspacePage} timerLabel={timerLabel} timerRunning={studyState?.timerStatus === 'running'} finishing={sessionCompleting} shareContext={studyState?.shareContextWithAi ?? false} coachMessages={workspaceMessages} streamedMessage={workspaceStreamedContent} coachInput={workspaceInput} coachBusy={workspaceSending || workspaceLoading} coachError={workspaceError} conversationRef={workspaceConversationPane} messageEndRef={workspaceMessageEnd} onPage={selectPage} onHome={() => { setSelected(null); setHomeSection('home'); void window.coach.planning.listPriorities().then(setPriorities) }} onSettings={() => setProviderDialogOpen(true)} onToggleTimer={() => { if (!studyState || timerUpdating) return; const workspaceId = selected.id; const epoch = workspaceLoadEpoch.current; const action = studyState.timerStatus === 'running' ? 'pause' : 'start'; setTimerUpdating(true); const configure = action === 'start' && studyState.timerStatus === 'idle' && studyState.timerDurationSeconds !== adaptiveMinutes * 60 ? window.coach.studyWorkspace.setTimerDuration({ workspaceId, durationSeconds: adaptiveMinutes * 60 }) : Promise.resolve(studyState); void configure.then(() => window.coach.studyWorkspace.updateTimer({ workspaceId, action })).then(applyStudyState(workspaceId, epoch)).catch(() => setWorkspaceError('Não foi possível atualizar o foco.')).finally(() => setTimerUpdating(false)) }} onFinish={() => { if (sessionCompleting) return; setSessionCompleting(true); void window.coach.studyWorkspace.completeSession(selected.id).then(setStudyState).finally(() => setSessionCompleting(false)) }} onShareContext={(enabled) => { void window.coach.studyWorkspace.updateContextSharing({ workspaceId: selected.id, enabled }).then(setStudyState).catch(() => setWorkspaceError('Não foi possível atualizar o compartilhamento de contexto.')) }} onCoachInput={setWorkspaceInput} onCoachSend={sendWorkspaceMessage} onNotes={() => setNotesOpen(true)}>
        {workspacePage === 'studies' && <div className="relative flex h-full min-h-0 flex-col overflow-hidden">{livePriority?.eventPhase === 'today' && <div className="shrink-0 border-b border-coach-orange/30 bg-coach-orange/10 px-6 py-3 text-xs text-coach-muted"><strong className="text-coach-orange">Revisão para a prova hoje.</strong> Priorizando explicações objetivas, exemplos e aplicações curtas; evite conteúdo pesado novo.</div>}{studyModule && studyRoadmap && studyProgress && studyLesson ? <><div className="flex shrink-0 items-center justify-between gap-4 border-b border-coach-line px-6 py-3"><p className="min-w-0 truncate text-xs text-coach-muted"><span className="text-coach-ink">{selected.name}</span><span className="mx-2">›</span>{studyModule.title}<span className="mx-2">›</span><span className="text-coach-ink">{studyModule.topics.find((topic) => `${studyModule.id}:${topic}` === studyProgress.topicId) ?? studyModule.title}</span></p><button aria-expanded={learningPathOpen} aria-controls="learning-path-drawer" onClick={() => setLearningPathOpen(true)} className="shrink-0 rounded-lg border border-coach-line px-3 py-2 text-xs font-black text-coach-green hover:bg-white/[.03]">Trilha de aprendizado</button></div><div className="min-h-0 flex-1"><StudyLessonView workspaceId={selected.id} module={studyModule} roadmap={studyRoadmap} lesson={studyLesson} progress={studyProgress} reviewMode={livePriority?.eventPhase === 'today'} onPosition={(position) => { void window.coach.studyProgress.updatePosition({ workspaceId: selected.id, position }).then(setStudyProgress) }} onCheckpoint={(evaluation, attempt) => { void window.coach.studyProgress.record({ workspaceId: selected.id, type: 'CHECKPOINT_ANSWERED', moduleId: studyProgress.moduleId, topicId: studyProgress.topicId, lessonId: studyProgress.lessonId, checkpointId: studyProgress.checkpointId, correct: evaluation.correct, attempt, hintUsed: evaluation.hint !== null, reinforcementUsed: evaluation.reinforcement !== null }).then((event) => event.shouldReplan ? window.coach.studyWorkspace.recalculatePlan({ workspaceId: selected.id }).then(setStudyState) : undefined) }} onComplete={(exerciseCompleted) => { void window.coach.studyProgress.record({ workspaceId: selected.id, type: 'TOPIC_COMPLETED', moduleId: studyProgress.moduleId, topicId: studyProgress.topicId, lessonId: studyProgress.lessonId, checkpointId: studyProgress.checkpointId, exerciseCompleted }).then((event) => event.shouldReplan ? window.coach.studyWorkspace.recalculatePlan({ workspaceId: selected.id }).then(setStudyState) : undefined).then(() => window.coach.studyProgress.get(selected.id)).then((value) => { if (value) setStudyProgress(value); return window.coach.studyWorkspace.recalculatePlan({ workspaceId: selected.id }) }).then(setStudyState) }} onPractice={() => setWorkspacePage('practice')} /></div><LearningPathDrawer open={learningPathOpen} roadmap={studyRoadmap} progress={studyProgress} onClose={() => setLearningPathOpen(false)} onSelect={(module, topicIndex) => selectStudyTopic(studyRoadmap, module, topicIndex).catch((error) => { setWorkspaceError('Não foi possível abrir este tópico agora.'); throw error })} /></> : <div className="grid min-h-0 flex-1 place-items-center p-8 text-center"><div><p className="font-display text-xl font-black text-coach-ink">{learningPathState?.status === 'waiting_for_provider' ? 'A Trilha será preparada quando a IA estiver disponível.' : learningPathState?.status === 'failed_retryable' ? 'Não foi possível concluir a Trilha agora. O Coach tentará novamente automaticamente.' : learningPathState?.status === 'ready' ? 'A Trilha está pronta, mas não foi possível carregar seu conteúdo.' : 'Preparando sua Trilha de Aprendizado…'}</p><p className="mt-2 text-xs text-coach-muted">Você pode continuar usando o Coach enquanto isso.</p></div></div>}</div>}

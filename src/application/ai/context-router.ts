...
export interface RoutedContext {
  ...
  readonly context: StreamWorkspaceMessageInput['studyContext']
...
export class ContextRouter {
  route(input: StreamWorkspaceMessageInput, observer?: ObserverState | null): RoutedContext {
    const text = input.content.trim()
    const intervention = Boolean(observer?.interventionSuggested)
    const context = input.studyContext

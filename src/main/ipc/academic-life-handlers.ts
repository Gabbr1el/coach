...
import { academicLifeDeleteInputSchema, academicLifeMutationInputSchema, academicLifeTransitionInputSchema } from '../../shared/contracts/academic-life-contract'
...
export function registerAcademicLifeHandlers(service: AcademicLifeService): void {
  ...
  ipcMain.handle(ACADEMIC_LIFE_CHANNELS.delete, (event, payload) => { assertTrustedSender(event); const input = academicLifeDeleteInputSchema.parse(payload); return service.delete(input.id, input.confirmCompleted) })
}

...
export const studyCheckpointHistoryEntrySchema = z.object({ answerId: z.string().min(1).max(120).optional(), learningAttemptId: z.string().min(1).max(420).nullable().optional(), signature: z.string().min(1).max(128).optional(), selectedOptionId: z.string().min(1).max(120), studentJustification: z.string().max(1000), attempt: z.number().int().min(1).max(1000), correct: z.boolean(), feedback: z.string().max(4000), rationale: z.string().max(1000), reasoningAssessment: checkpointReasoningAssessmentSchema.optional(), answeredAt: z.number().int().nonnegative() }).strict()
...
export interface StudyProgressApi {
  ...
  answerCheckpoint(input: z.infer<typeof answerStudyCheckpointSchema>): Promise<{ readonly state: StudyProgressState; readonly evaluation: import('./study-lesson-contract').StudyCheckpointEvaluation; readonly shouldReplan: boolean; readonly learningAttemptId: string | null }>
  retryCheckpointReasoning(input: z.infer<typeof retryCheckpointReasoningSchema>): Promise<{ readonly state: StudyProgressState; readonly evaluation: import('./study-lesson-contract').StudyCheckpointEvaluation; readonly shouldReplan: boolean; readonly learningAttemptId: string | null }>

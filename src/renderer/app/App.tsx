...
export function App() {
  ...
  const [homeSection, setHomeSection] = useState<'home' | 'workspaces' | 'history' | 'reports'>('home')
  ...
  if (selected) {
    ...
    return (
      <WorkspaceShell name={selected.name} objective={selected.objective} page={workspacePage} timerLabel={timerLabel} timerRunning={studyState?.timerStatus === 'running'} coachMessages={workspaceMessages} streamedMessage={workspaceStreamedContent} coachInput={workspaceInput} coachBusy={workspaceSending || workspaceLoading} coachError={workspaceError} conversationRef={workspaceConversationPane} messageEndRef={workspaceMessageEnd} onPage={selectPage} onHome={() => { setSelected(null); setHomeSection('home'); void window.coach.planning.listPriorities().then(setPriorities) }} onSettings={() => setProviderDialogOpen(true)} onToggleTimer={() => { if (!studyState || timerUpdating) return; setTimerUpdating(true); void window.coach.studyWorkspace.updateTimer({ workspaceId: selected.id, action: studyState.timerStatus === 'running' ? 'pause' : timerRemaining === 0 ? 'reset' : 'start' }).then(setStudyState).finally(() => setTimerUpdating(false)) }} onFinish={() => { setSessionCompleting(true); void window.coach.studyWorkspace.completeSession(selected.id).then(setStudyState).finally(() => setSessionCompleting(false)) }} onCoachInput={setWorkspaceInput} onCoachSend={sendWorkspaceMessage} onNotes={() => setNotesOpen(true)}>
        ...
        {workspacePage === 'videos' && <div className="coach-scroll-pane h-full p-5 lg:p-8"><div className="mx-auto max-w-5xl"><p className="text-xs font-black uppercase tracking-[0.18em] text-coach-green">Vídeos focados</p><h2 className="mt-2 font-display text-3xl font-black">Aprenda sem sair do contexto</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-coach-muted">Use uma busca direcionada ao objetivo deste Workspace. O Coach mantém a conversa e o plano visíveis sem abrir o feed tradicional.</p><div className="mt-7 rounded-3xl border border-coach-line bg-white p-6"><label className="text-xs font-black uppercase text-coach-muted">Buscar no YouTube</label><div className="mt-3 flex gap-2"><input value={materialQuery} onChange={(event) => setMaterialQuery(event.target.value)} placeholder={`Ex.: ${activePlan?.title ?? selected.name}`} className="min-w-0 flex-1 rounded-xl border border-coach-line bg-coach-paper p-3" /><button onClick={() => { const query = encodeURIComponent(`${materialQuery || activePlan?.title || selected.name} aula`); window.open(`https://www.youtube.com/results?search_query=${query}`, '_blank', 'noopener,noreferrer') }} className="rounded-xl bg-coach-orange px-5 text-sm font-black text-white">Pesquisar</button></div><p className="mt-3 text-xs text-coach-muted">Links externos abrem somente após sua ação. Nenhum vídeo é enviado automaticamente ao provedor de IA.</p></div></div></div>}
        {workspacePage === 'reports' && <div className="coach-scroll-pane h-full p-5 lg:p-8"><div className="mx-auto max-w-5xl"><p className="text-xs font-black uppercase tracking-[0.18em] text-coach-green">Evolução</p><h2 className="mt-2 font-display text-3xl font-black">Relatórios de aprendizagem</h2><div className="mt-6 grid gap-4 md:grid-cols-2">{sessionHistory.length === 0 && <p className="rounded-2xl bg-white p-5 text-sm text-coach-muted">Finalize sua primeira sessão para gerar um relatório.</p>}{sessionHistory.map((session) => <article key={session.id} className="rounded-2xl border border-coach-line bg-white p-5"><div className="flex justify-between"><strong>{new Date(session.startedAt).toLocaleDateString('pt-BR')}</strong><span className="rounded-full bg-coach-green/10 px-3 py-1 text-xs font-black text-coach-green">{Math.floor(session.focusSeconds / 60)} min</span></div><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl bg-coach-paper p-3"><strong className="text-xl">{session.successRate ?? 0}%</strong><span className="block text-[10px] text-coach-muted">Sucesso</span></div><div className="rounded-xl bg-coach-paper p-3"><strong className="text-xl">{session.focusRetentionPercent ?? 0}%</strong><span className="block text-[10px] text-coach-muted">Retenção</span></div></div><p className="mt-4 text-xs leading-5 text-coach-muted">{session.recommendation}</p></article>)}</div></div></div>}
        {notesOpen && <div className="fixed inset-0 z-20 flex justify-end bg-black/30" onMouseDown={() => setNotesOpen(false)}><section className="flex h-full w-full max-w-md flex-col bg-coach-paper p-6 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}><div className="flex items-center justify-between"><h2 className="font-display text-2xl font-black">Notas rápidas</h2><button onClick={() => setNotesOpen(false)}><X /></button></div><textarea value={studyNotes} onChange={(event) => setStudyNotes(event.target.value)} className="mt-6 min-h-0 flex-1 resize-none rounded-2xl border border-coach-line bg-white p-4" /></section></div>}
      </WorkspaceShell>
    )
  }

  return (
    <main className="coach-app-shell flex bg-[#ece9df] text-coach-ink"><AppRail active={homeSection} onHome={() => setHomeSection('home')} onWorkspaces={() => setHomeSection('workspaces')} onHistory={() => setHomeSection('history')} onReports={() => setHomeSection('reports')} onSettings={() => setProviderDialogOpen(true)} /><div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden p-5 lg:p-7">
      ...
      <section className="mx-auto mt-20 max-w-7xl">
        ...
        <div className="mt-12 grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_390px]">
          ...
          <aside className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-[2rem] bg-coach-ink text-white shadow-soft">
            ...
            <div className="coach-scroll-pane min-h-0 flex-1 space-y-4 p-5">
              ...
              {messages.map((message) => <div key={message.id} className={`coach-chat-message max-w-[90%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === 'user' ? 'ml-auto bg-coach-green text-white' : 'bg-white/8 text-white/75'}`}>{message.content}</div>)}

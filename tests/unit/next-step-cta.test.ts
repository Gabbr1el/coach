...
describe('deriveNextStepCta', () => {
  it.each([
    ...
    ['practice', 'practice', 'Abrir Prática'], ['coding', 'practice', 'Abrir Prática'], ['material', 'materials', 'Abrir Material'], ['video', 'videos', 'Abrir Vídeos'],
  ...
  it('keeps legacy exact-target activities unavailable when their persisted identity is null', () => {
    expect(deriveNextStepCta({ ...item('exercise'), exerciseSetId: undefined })).toBeNull()
    expect(deriveNextStepCta({ ...item('material'), materialId: undefined })).toBeNull()
  })
})

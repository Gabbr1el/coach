...
export const streamWorkspaceMessageInputSchema = sendHomeMessageInputSchema.extend({
  ...
  activeInteractiveCode: z.object({ lessonId: z.string().max(360), blockId: z.string().max(420), interactionType: z.enum(['PREDICT_AND_RUN', 'EDIT_AND_RUN', 'FIX_AND_RUN']), instruction: z.string().max(2000), language: z.enum(['python', 'c']), code: z.string().max(20_000), prediction: z.string().max(2000).nullable(), attempts: z.number().int().nonnegative(), lastExecution: z.object({ stdout: z.string().max(8000), stderr: z.string().max(8000), exitCode: z.number().int().nullable(), timedOut: z.boolean() }).nullable(), validationResult: z.object({ status: z.enum(['not_applicable', 'passed', 'failed']), message: z.string().max(1000) }).nullable() }).optional(),

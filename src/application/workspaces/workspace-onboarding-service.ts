...
export class WorkspaceOnboardingService {
  ...
  async analyze(topic: string, diagnosticAnswer?: string): Promise<WorkspaceTopicAnalysis> {
    ...
    if (!provider) return { topic: normalizedTopic, objective: `Aprender ${normalizedTopic}`, needsDiagnostic: !diagnosticAnswer, question: diagnosticAnswer ? null : `O que você já sabe sobre ${normalizedTopic}, e onde sente mais dificuldade?`, contextSource: diagnosticAnswer ? 'diagnostic' : 'none' }

...
import { executeCodeInputSchema, executeInteractiveCodeInputSchema, executeProjectInputSchema, interactiveCodeStateSchema, listInteractiveCodeStatesInputSchema, saveInteractiveCodeStateInputSchema, type CodeExecutionResult, type InteractiveCodeBlock, type InteractiveCodeState } from '../../shared/contracts/code-execution-contract'
...
function normalizeOutput(value: string): string { return value.replaceAll('\r\n', '\n').trimEnd() }

export function validateInteractiveExecution(block: InteractiveCodeBlock, result: CodeExecutionResult, now: number): NonNullable<InteractiveCodeState['validationResult']> {
  if (block.evidenceMode !== 'validated') return { status: 'not_applicable', message: block.evidenceMode === 'observation' ? 'Execução registrada como observação, sem evidência avaliativa.' : 'Execução livre, sem evidência pedagógica.', validatedAt: now }
  const passed = result.exitCode === 0 && !result.timedOut && block.expectedOutput !== null && normalizeOutput(result.stdout) === normalizeOutput(block.expectedOutput)
  return { status: passed ? 'passed' : 'failed', message: passed ? 'Saída validada contra o resultado esperado.' : 'A execução terminou, mas não corresponde ao resultado esperado.', validatedAt: now }
}

function inlineProject(workspaceId: string, block: InteractiveCodeBlock, content: string): WorkspaceProject {
  const extension = block.language === 'python' ? 'py' : 'c'
  const path = `main.${extension}`
...
export function registerCodeExecutionHandlers(workspaceExists: (id: string) => Promise<boolean>, observer: ObserverService, projects?: DrizzleProjectRepository, database?: CoachDatabase, toolchains = new ToolchainManager()): void {
  ...
  ipcMain.handle(CODE_EXECUTION_CHANNELS.listInteractiveStates, (event, payload: unknown) => {
    ...
    return (database.sqlite.prepare('SELECT lesson_id AS lessonId, block_id AS blockId, current_code AS currentCode, prediction, attempts, last_execution_json AS lastExecutionJson, validation_result_json AS validationResultJson, updated_at AS updatedAt FROM study_interactive_code_states WHERE workspace_id = ? AND lesson_id = ? ORDER BY updated_at').all(input.workspaceId, input.lessonId) as Array<{ lessonId: string; blockId: string; currentCode: string; prediction: string | null; attempts: number; lastExecutionJson: string | null; validationResultJson: string | null; updatedAt: number }>).map((row) => interactiveCodeStateSchema.parse({ ...row, lastExecution: row.lastExecutionJson ? JSON.parse(row.lastExecutionJson) : null, validationResult: row.validationResultJson ? JSON.parse(row.validationResultJson) : null }))
  })
  ipcMain.handle(CODE_EXECUTION_CHANNELS.saveInteractiveState, async (event, payload: unknown) => {
    ...
    const now = Date.now()
    const state = interactiveCodeStateSchema.parse({ lessonId: input.lessonId, blockId: input.blockId, currentCode: input.currentCode, prediction: input.prediction, attempts: previous?.attempts ?? 0, lastExecution: previous?.lastExecutionJson ? JSON.parse(previous.lastExecutionJson) : null, validationResult: previous?.validationResultJson ? JSON.parse(previous.validationResultJson) : null, updatedAt: now })
    database.sqlite.prepare(`INSERT INTO study_interactive_code_states (workspace_id, lesson_id, block_id, current_code, prediction, attempts, last_execution_json, validation_result_json, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(workspace_id, lesson_id, block_id) DO UPDATE SET current_code=excluded.current_code,prediction=excluded.prediction,updated_at=excluded.updated_at`).run(input.workspaceId, input.lessonId, input.blockId, input.currentCode, input.prediction, state.attempts, previous?.lastExecutionJson ?? null, previous?.validationResultJson ?? null, now)
    return state
  })
  ipcMain.handle(CODE_EXECUTION_CHANNELS.executeInteractive, async (event, payload: unknown) => {
    ...
    try {
      ...
      const now = Date.now()
      const validationResult = validateInteractiveExecution(block, execution, now)
      ...
      const state = interactiveCodeStateSchema.parse({ lessonId: input.lessonId, blockId: input.blockId, currentCode: input.currentCode, prediction: input.prediction, attempts: (previous?.attempts ?? 0) + 1, lastExecution: execution, validationResult, updatedAt: now })
      database.sqlite.transaction(() => {
        database.sqlite.prepare(`INSERT INTO study_interactive_code_states (workspace_id, lesson_id, block_id, current_code, prediction, attempts, last_execution_json, validation_result_json, evidence_granted_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(workspace_id, lesson_id, block_id) DO UPDATE SET current_code=excluded.current_code,prediction=excluded.prediction,attempts=excluded.attempts,last_execution_json=excluded.last_execution_json,validation_result_json=excluded.validation_result_json,evidence_granted_at=COALESCE(study_interactive_code_states.evidence_granted_at,excluded.evidence_granted_at),updated_at=excluded.updated_at`).run(input.workspaceId, input.lessonId, input.blockId, input.currentCode, input.prediction, state.attempts, JSON.stringify(execution), JSON.stringify(validationResult), validationResult.status === 'passed' ? now : null, now)

...
void app.whenReady().then(async () => {
  try {
    ...
    const workspaceService = new WorkspaceService({ repository: workspaceRepository, academicContext: academicSubjectContext })
    ...
    const workspaceContextHub = new WorkspaceContextHub({ getCurrent: (id) => currentWorkspaceContext.get(id), read: async (workspaceId, resource) => { if (resource === 'materials') return materialService.list(workspaceId); if (resource === 'roadmap') return roadmapRepository.findCurrent(workspaceId); if (resource === 'lesson') return database!.sqlite.prepare('SELECT id, topic_id AS topicId, content_json AS content FROM study_lessons WHERE workspace_id = ? ORDER BY updated_at DESC LIMIT 1').get(workspaceId); if (resource === 'progress') return database!.sqlite.prepare('SELECT * FROM study_progress WHERE workspace_id = ?').get(workspaceId); if (resource === 'plan') return studyWorkspaceService.getState(workspaceId).then((state) => state.plan); if (resource === 'academic') { const workspace = await workspaceRepository.findById(workspaceId); return workspace ? academicSubjectContext.get(workspace.name) : null } if (resource === 'workspace') return workspaceRepository.findById(workspaceId); return null } })
    ...
    const plannerActionService = new PlannerActionService({ repository: new DrizzlePlannerActionRepository(database), createWorkspace: (input) => workspaceService.create({ ...input, declaredLevel: 'beginner', declaredKnowledge: [], declaredDifficulties: [], goals: [] }), createProject: (workspaceId, name, language) => new ProjectService(projectRepository, async (id) => Boolean(await workspaceRepository.findById(id))).create(workspaceId, name, language), createDeadline: (input) => planningService.createDeadline(input), addRoutine: (content) => planningService.addRoutineNote(content) })
    registerPlannerActionHandlers(plannerActionService)
    registerConversationHandlers(homePlannerService, workspaceCoachService, new HomeOrganizerService(homePlannerService, planningService, plannerActionService, () => workspaceRepository.listActive(), (id) => studyWorkspaceService.recalculatePlan(id), Date.now, academicSubjectContext), workspaceActions)
    registerReportHandlers(new ReportService(new DrizzleReportRepository(database)))
    registerWorkspaceOnboardingHandlers(new WorkspaceOnboardingService({ repository: new DrizzleConversationRepository(database), providerManager, academicContext: academicSubjectContext }))

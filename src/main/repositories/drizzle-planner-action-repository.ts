...
export class DrizzlePlannerActionRepository implements PlannerActionRepository {
  ...
  listPending(): PlannerAction[] { return (this.database.sqlite.prepare(`SELECT ${columns} FROM planner_actions WHERE status IN ('proposed','applying') ORDER BY created_at DESC`).all() as Row[]).map((row) => this.map(row)) }
  ...
  resolveAtomically(id: string, decision: 'apply' | 'reject', now: number, execute: (action: PlannerAction) => unknown): PlannerAction {
    return this.database.sqlite.transaction(() => {
      ...
      if (current.status !== 'proposed' && current.status !== 'applying') throw new Error('Planner action is not recoverable')
      const result = decision === 'apply' ? execute(current) : null
      const status = decision === 'apply' ? 'applied' : 'rejected'
      const changed = this.database.sqlite.prepare("UPDATE planner_actions SET status=?,result_json=?,resolved_at=? WHERE id=? AND status IN ('proposed','applying')").run(status, JSON.stringify(result), now, id)
      if (changed.changes !== 1) throw new Error('Planner action terminal state was not persisted')
      return this.find(id)!
    })()
  }
}

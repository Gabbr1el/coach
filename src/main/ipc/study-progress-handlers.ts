...
function responseFor(state: StudyProgressState, checkpoint: LessonCheckpoint, correct: boolean, attempt: number, input: { selectedOptionId: string; studentJustification: string }, reasoningAssessment: CheckpointReasoningAssessment, shouldReplan: boolean, learningAttemptId: string | null) {
  ...
  return { state, evaluation: { correct, selectedOptionId: input.selectedOptionId, attempt, studentJustification: input.studentJustification, rationale: selected.rationale, misconceptionTag: selected.misconceptionTag ?? null, feedback, hint: correct ? null : checkpoint.hint, reinforcement, reasoningAssessment }, shouldReplan, learningAttemptId }
}
...
export function registerStudyProgressHandlers(database: CoachDatabase, getToolchains: () => ToolchainStatus[] = () => [], providerManager?: AIProviderManager, prefetch?: PedagogicalPrefetchScheduler, evidenceRecorder?: LearningEvidenceRecorder): void {
  ...
  const evaluateAndPersist = async (input: { workspaceId: string; lessonId: string; checkpointId: string; selectedOptionId: string; studentJustification: string }, retry: boolean) => {
    ...
    if (!retry && replay) return responseFor(get(input.workspaceId)!, checkpoint, replay.correct, replay.attempt, input, replay.reasoningAssessment ?? previous?.reasoningAssessment ?? pendingAssessment(0, Date.now()), false, replay.learningAttemptId ?? null)
    ...
    if (retry && pendingPrevious && Date.now() < pendingPrevious.nextRetryAt) return responseFor(active, checkpoint, previous!.correct, previous!.attempt, input, pendingPrevious, false, previous!.history.at(-1)?.learningAttemptId ?? null)
    ...
    const now = Date.now()
    let replan = false
    let learningAttemptId: string | null = previous?.history.at(-1)?.learningAttemptId ?? null
    database.sqlite.transaction(() => {
      ...
      if (assessment.status !== 'reasoning_evaluation_pending') {
        ...
        if (inserted.changes && evidenceRecorder) {
          ...
          learningAttemptId = authoritative ? recorded.attemptId : null
        }
      }
      const historyEntry = { answerId, learningAttemptId, signature, selectedOptionId: input.selectedOptionId, studentJustification: input.studentJustification, attempt, correct, feedback: alternativeFeedback, rationale: selected.rationale, reasoningAssessment: assessment, answeredAt: retry ? (freshPrevious?.history.at(-1)?.answeredAt ?? now) : now }
    ...
    return responseFor(persisted, checkpoint, persisted.checkpointStates?.[input.checkpointId]?.correct ?? correct, persisted.checkpointStates?.[input.checkpointId]?.attempt ?? attempt, input, persisted.checkpointStates?.[input.checkpointId]?.reasoningAssessment ?? assessment, replan, persisted.checkpointStates?.[input.checkpointId]?.history.at(-1)?.learningAttemptId ?? learningAttemptId)
  }

...
export interface StudyWorkspaceRepository {
  ...
  createStateFromAuthoritative(input: Omit<StudyWorkspaceState, 'plan' | 'timerDurationSeconds' | 'timerRemainingSeconds'>, calculatePlan: () => StudyPlanItem[]): Promise<StudyWorkspaceState>
  ...
  setPlanItemCompletion(workspaceId: string, sessionId: string, itemId: string, completed: boolean, statuses: ReadonlyArray<{ id: string; status: StudyPlanItem['status'] }>, timer: Pick<StudyWorkspaceState, 'timerDurationSeconds' | 'timerStatus' | 'timerRemainingSeconds' | 'timerStartedAt' | 'timerStartedMonotonicMs' | 'timerBootId' | 'accumulatedFocusSeconds'>, now: number): Promise<boolean>
  replacePlan(workspaceId: string, sessionId: string, plan: StudyPlanItem[], now: number, dayKey?: string): Promise<void>
  recalculatePlanAtomically(workspaceId: string, sessionId: string, calculate: () => StudyPlanItem[], now: number, dayKey?: string): Promise<void>

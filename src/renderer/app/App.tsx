...
function useDialogFocus<T extends HTMLElement>(open: boolean, onClose: () => void) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (isEditableTarget(event.target) && event.key !== 'Escape' && event.key !== 'Tab') return
...
function WorkspaceCreationScreen({ open, submitting, initial, onClose, onSubmit }: {
  ...
  const creationInput = (): CreateWorkspaceInput => ({ name, objective, analysisToken: analysis?.analysisToken, analysisRevision: analysis?.revision, canonicalFocus: analysis?.canonicalFocus, canonicalContext: analysis?.canonicalContext, localKnowledgeProjection: analysis?.localKnowledgeProjection, declaredLevel: analysis?.declaredLevel ?? undefined, declaredKnowledge: analysis ? [...analysis.declaredKnowledge] : [], declaredDifficulties: analysis ? [...analysis.declaredDifficulties] : [], goals: analysis ? [...analysis.goals] : [], duplicateOverride: duplicateConfirmed && duplicateDifference.trim().length >= 8 ? { confirmed: true, meaningfulDifference: duplicateDifference.trim() } : undefined, relatedSubjects: analysis?.relatedContexts.map(({ subject, relation }) => ({ subject, relation })) ?? [] })
  ...
  const dialogRef = useDialogFocus<HTMLDivElement>(open, () => { void closeCreation() })
  if (!open) return null
    ...
    <div ref={dialogRef} className="fixed inset-0 z-20 overflow-y-auto bg-[#0d0e12] p-5" role="dialog" aria-modal="true" aria-labelledby="create-workspace-title">
      ...
      >
        <div className="flex items-start justify-between">
          ...
          {analysis && <section className={`mt-5 rounded-2xl border p-5 ${analysis.status === 'INVALID' ? 'border-red-500/50 bg-red-500/10' : analysis.status === 'NEEDS_CONFIRMATION' ? 'border-coach-orange/60 bg-coach-orange/10' : 'border-coach-green/40 bg-coach-green/[.06]'}`}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-coach-muted">Interpretação do Coach</p><h3 className="mt-1 font-display text-xl font-black">{analysis.canonicalSubject || 'Tema não reconhecido'}</h3></div><span className="rounded-full border border-current px-3 py-1 text-[10px] font-black tracking-wide">{analysis.status}</span></div><p className="mt-3 text-sm leading-6 text-coach-muted">{analysis.explanation}</p>{analysis.canonicalFocus && analysis.canonicalFocus !== analysis.canonicalSubject && <p className="mt-2 text-xs"><strong>Foco:</strong> {analysis.canonicalFocus}</p>}{analysis.relatedContexts.length > 0 && <div className="mt-4 border-t border-coach-line pt-3"><p className="text-xs font-black uppercase tracking-wide">Relações automáticas</p>{analysis.relatedContexts.map((item) => <p key={`${item.relation}:${item.subject}`} className="mt-2 text-xs text-coach-muted"><strong className="text-coach-ink">{item.subject}</strong> — {item.explanation} Isso adiciona contexto, não evidência de domínio.</p>)}</div>}</section>}
          ...
          {question && <label className="mt-5 block rounded-xl border border-[#39334f] bg-[#181622] p-4 text-sm font-bold"><span className="text-[#aa9cff]">Pergunta contextual opcional</span><span className="mt-2 block font-normal leading-6 text-[#c8cad0]">{question}</span><textarea maxLength={1000} value={diagnosticAnswer} onChange={(event) => { setDiagnosticAnswer(event.target.value); setAnalyzed(event.target.value.trim().length === 0) }} placeholder="Você pode responder ou criar o Workspace sem responder." className="mt-3 min-h-24 w-full resize-none rounded-lg border border-[#39334f] bg-[#111217] px-4 py-3 outline-none" />{diagnosticAnswer.trim() && <button type="button" disabled={analyzing} onClick={() => void analyze()} className="mt-3 rounded-lg border border-[#aa9cff] px-3 py-2 text-xs font-bold text-[#aa9cff]">Usar esta resposta</button>}</label>}
  ...
  if (selected) {
    ...
    return (
      <WorkspaceShell name={selected.name} objective={selected.objective} page={workspacePage} timerLabel={timerLabel} timerRunning={studyState?.timerStatus === 'running'} finishing={sessionCompleting} coachMessages={workspaceMessages} streamedMessage={workspaceStreamedContent} coachInput={workspaceInput} coachBusy={workspaceSending || workspaceLoading} coachStage={workspaceSendStage} coachContextSummary={workspaceContextSummary} coachError={workspaceError} plannerActions={workspacePlannerActions} conversationRef={workspaceConversationPane} messageEndRef={workspaceMessageEnd} onConversationScroll={() => { const pane = workspaceConversationPane.current; if (pane) workspaceFollowLatest.current = isNearChatBottom(pane) }} onPage={selectPage} onHome={() => { workspaceOpenEpoch.current += 1; stopWorkspaceStream(); void (workspacePage === 'exercises' ? leaveExercises() : Promise.resolve()).then(() => { setSelected(null); setHomeSection('home'); void window.coach.planning.listPriorities().then(setPriorities) }) }} onSettings={() => setProviderDialogOpen(true)} onToggleTimer={() => { if (!studyState || timerUpdating) return; const workspaceId = selected.id; const epoch = workspaceLoadEpoch.current; const action = studyState.timerStatus === 'running' ? 'pause' : 'start'; setTimerUpdating(true); const configure = action === 'start' && studyState.timerStatus === 'idle' && studyState.timerDurationSeconds !== adaptiveMinutes * 60 ? window.coach.studyWorkspace.setTimerDuration({ workspaceId, durationSeconds: adaptiveMinutes * 60 }) : Promise.resolve(studyState); void configure.then(() => window.coach.studyWorkspace.updateTimer({ workspaceId, action })).then(applyStudyState(workspaceId, epoch)).catch(() => setWorkspaceError('Não foi possível atualizar o foco.')).finally(() => setTimerUpdating(false)) }} onFinish={() => { if (sessionCompleting) return; setSessionCompleting(true); void window.coach.studyWorkspace.completeSession(selected.id).then(setStudyState).finally(() => setSessionCompleting(false)) }} onCoachInput={setWorkspaceInput} onCoachSend={sendWorkspaceMessage} onResolveAction={(actionId, decision) => { if (workspaceSending) return; setWorkspaceSending(true); setWorkspaceError(null); void window.coach.plannerAction.resolve({ actionId, decision }).then(async (action) => { setWorkspacePlannerActions((current) => current.filter((item) => item.id !== action.id)); if (decision === 'apply') { const [state, plan] = await Promise.all([window.coach.studyWorkspace.getState(selected.id), window.coach.planning.getWeeklyPlan()]); setStudyState(state); setWeeklyPlan(plan); setWorkspaceMessages((current) => [...current, { id: crypto.randomUUID(), role: 'assistant', content: `Ação aplicada e persistida: ${action.label}.`, createdAt: Date.now(), sequence: current.length + 1, providerId: 'coach-local', modelId: 'planner-action-v1' }]) } }).catch(() => setWorkspaceError('Não foi possível resolver a ação; nenhuma confirmação de sucesso foi registrada.')).finally(() => setWorkspaceSending(false)) }} onNotes={() => setNotesOpen(true)}>
        ...
        {workspacePage === 'exercises' && exerciseRoadmap && exerciseTopicId && <ExercisesWorkspace workspaceId={selected.id} roadmap={exerciseRoadmap} initialTopicId={exerciseTopicId} expectedSetId={exerciseSetId} onBack={() => { void leaveExercises().then(() => openStudies()) }} onContext={setActiveExerciseContext} />}

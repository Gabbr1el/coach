...
export const academicSubjectContexts = sqliteTable('academic_subject_contexts', {
  ...
  observedStrengthsJson: text('observed_strengths_json').notNull().default('[]'),
  observedDifficultiesJson: text('observed_difficulties_json').notNull().default('[]'),
  misconceptionsJson: text('misconceptions_json').notNull().default('[]'),
  masteredConceptsJson: text('mastered_concepts_json').notNull().default('[]'),
  reviewConceptsJson: text('review_concepts_json').notNull().default('[]'),
  lastConsolidatedAt: integer('last_consolidated_at'),

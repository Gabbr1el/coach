...
import { workspaceCoachDecisionSchema, type ConversationMessage } from '../../src/shared/contracts/conversation-contract'
...
describe('WorkspaceCoachService', () => {
  it('makes today-budget explicitly impossible as a provider-selected Tutor action', () => { expect(workspaceCoachDecisionSchema.safeParse({ kind: 'workspace_action', action: { type: 'plan.today-budget.set', arguments: { dateKey: '2026-09-10', timezone: 'UTC', minutes: 60 } } }).success).toBe(false) })
  ...
  it('proposes an explicit +4h today action without writing on the initial Tutor request', async () => { const repository = new MemoryConversationRepository(); const manager = new AIProviderManager(); const execute = vi.fn(); const propose = vi.fn((input) => ({ id: crypto.randomUUID(), ...input, status: 'proposed', result: null, createdAt: 1, resolvedAt: null })); manager.register({ id: 'stream', name: 'Stream', testConnection: async () => {}, sendMessage: async () => { throw new Error('provider must not decide an explicit supported command') }, streamMessage: async function* () {}, getCapabilities: () => ({ streaming: true, usageInformation: false, supportedInput: ['text'] }) }); manager.select('stream'); const metadata = vi.fn(); const service = new WorkspaceCoachService({ repository, providerManager: manager, getWorkspace: async () => workspace, contextHub: { immediate: async () => ({ todayDateKey: '2026-09-10', planningTimezone: 'UTC', weeklyPlanRevision: 3 }), read: async () => null } as any, workspaceActions: { execute } as any, plannerActions: { propose } as any }); const output: string[] = []; for await (const delta of service.streamMessage(workspace.id, { requestId: crypto.randomUUID(), workspaceId: workspace.id, content: 'Aumente +4h no estudo de hoje', activePage: 'plan' }, new AbortController().signal, metadata)) output.push(delta); expect(execute).not.toHaveBeenCalled(); expect(propose).toHaveBeenCalledWith(expect.objectContaining({ type: 'plan.workspace-day-load.adjust', payload: { workspaceId: workspace.id, dateKey: '2026-09-10', timezone: 'UTC', deltaMinutes: 240 } })); expect(metadata).toHaveBeenCalledWith({ plannerAction: expect.objectContaining({ status: 'proposed' }) }); expect(output.join('')).toContain('Confirme ou descarte') })
  it('does not mutate today when the user requests tomorrow or the provider selects an arbitrary date', async () => { const repository = new MemoryConversationRepository(); const manager = new AIProviderManager(); const execute = vi.fn(); const propose = vi.fn(); manager.register({ id: 'stream', name: 'Stream', testConnection: async () => {}, sendMessage: async () => ({ content: JSON.stringify({ kind: 'workspace_action', action: { type: 'plan.load.adjust', arguments: { dateKey: '2026-09-10', deltaMinutes: 240 } } }), providerId: 'stream', modelId: 'model' }), streamMessage: async function* () { yield { type: 'completed' as const, response: { content: 'Nenhuma alteração foi feita.', providerId: 'stream', modelId: 'model' } } }, getCapabilities: () => ({ streaming: true, usageInformation: false, supportedInput: ['text'] }) }); manager.select('stream'); const service = new WorkspaceCoachService({ repository, providerManager: manager, getWorkspace: async () => workspace, contextHub: { immediate: async () => ({ todayDateKey: '2026-09-10', planningTimezone: 'UTC', weeklyPlanRevision: 3 }), read: async () => null } as any, workspaceActions: { execute } as any, plannerActions: { propose } as any }); for await (const _ of service.streamMessage(workspace.id, { requestId: crypto.randomUUID(), workspaceId: workspace.id, content: 'Aumente 4h amanhã', activePage: 'plan' }, new AbortController().signal)) {} expect(execute).not.toHaveBeenCalled(); expect(propose).not.toHaveBeenCalled() })
  it('rejects global availability without explicit global authorization', async () => { const repository = new MemoryConversationRepository(); const manager = new AIProviderManager(); const execute = vi.fn(); manager.register({ id: 'stream', name: 'Stream', testConnection: async () => {}, sendMessage: async () => ({ content: JSON.stringify({ kind: 'workspace_action', action: { type: 'plan.availability.set', arguments: { weekday: 5, minutes: 240, confirmedGlobal: true } } }), providerId: 'stream', modelId: 'model' }), streamMessage: async function* () { yield { type: 'completed' as const, response: { content: 'não alterei', providerId: 'stream', modelId: 'model' } } }, getCapabilities: () => ({ streaming: true, usageInformation: false, supportedInput: ['text'] }) }); manager.select('stream'); const service = new WorkspaceCoachService({ repository, providerManager: manager, getWorkspace: async () => workspace, contextHub: { immediate: async () => ({ todayDateKey: '2026-09-10' }), read: async () => null } as any, workspaceActions: { execute } as any }); const output: string[] = []; for await (const delta of service.streamMessage(workspace.id, { requestId: crypto.randomUUID(), workspaceId: workspace.id, content: 'Sexta tenho 4h disponíveis', activePage: 'plan' }, new AbortController().signal)) output.push(delta); expect(execute).not.toHaveBeenCalled(); expect(output.join('')).not.toMatch(/aplicad|salv|sucesso/i) })
})

...
describe('authoritative learning acceptance flow', () => {
  it('creates, studies and reviews only through public services and IPC handlers, then survives restart', async () => {
    ...
    let topicId = ''
    const provider: AIProvider = { id: 'fixture', name: 'Fixture', async testConnection() {}, getCapabilities: () => ({ streaming: false, usageInformation: false, supportedInput: ['text'] }), async sendMessage(request) { const system = request.messages[0]?.content ?? ''; const content = system.includes('trilha curricular') ? roadmapResponse : system.includes('aula específica') ? lessonResponse(topicId) : { exercises: [codeExercise('PROGRAMMING_PROBLEM', 'loops-check', 'loops', 1), codeExercise('FIX_CODE', 'conditions-check', 'conditionals', 2), codeExercise('COMPLETE_CODE', 'variables-run', 'variables', 3), predictionExercise('output-predict', 'output', '4'), predictionExercise('iteration-predict', 'iteration', '5'), predictionExercise('output-predict', 'output', '6'), predictionExercise('iteration-predict', 'iteration', '7')] }; return { content: JSON.stringify(content), providerId: 'fixture', modelId: 'fixture-model' } } }

...
export class AcademicLifeService {
  ...
  delete(id: string, confirmCompleted: boolean): AcademicLifeDeleteResult {
    const item = this.find(id)
    if (!item) throw new Error('Academic item no longer exists')
    if (item.status === 'resolved' && !confirmCompleted) throw new Error('Completed academic item deletion requires explicit confirmation')
    return this.repository.deleteLineage(id, this.now())
  }

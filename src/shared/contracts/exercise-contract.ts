...
export const exerciseSchema = z.object({ id: entityIdSchema, position: z.number().int().positive(), assessmentIntentKey: z.string().min(1).max(120).optional(), conceptKeys: z.array(z.string().min(1).max(120)).max(12).optional(), kind: exerciseKindSchema, difficulty: exerciseDifficultySchema, title: z.string().min(1).max(180), statement: z.string().min(1).max(5000), inputDescription: z.string().max(2000), outputDescription: z.string().max(2000), language: projectLanguageSchema, starterCode: z.string().max(20_000), predictionPrompt: z.string().min(1).max(1000).nullable(), codeToObserve: z.string().min(1).max(20_000).nullable(), requiredForTopicCompletion: z.boolean(), publicTests: z.array(publicExerciseTestSchema).max(8) }).strict().superRefine((value, context) => {

...
export class DrizzleStudyWorkspaceRepository implements StudyWorkspaceRepository {
  ...
  private validateTargets(workspaceId: string, item: StudyPlanItem): void {
    if (item.exerciseSetId && !this.database.sqlite.prepare('SELECT 1 FROM exercise_sets WHERE id=? AND workspace_id=? AND topic_id=?').get(item.exerciseSetId, workspaceId, item.topicId ?? '')) throw new Error('Study plan exercise target does not belong to its workspace and topic')
    if (item.materialId && !this.database.sqlite.prepare('SELECT 1 FROM materials WHERE id=? AND workspace_id=?').get(item.materialId, workspaceId)) throw new Error('Study plan material target does not belong to its workspace')
  }
  private resolveTargets(workspaceId: string, item: StudyPlanItem): StudyPlanItem {
    if ((item.activityType === 'exercise' || item.activityType === 'assessment') && item.topicId && !item.exerciseSetId) {
      const row = this.database.sqlite.prepare("SELECT id FROM exercise_sets WHERE workspace_id=? AND topic_id=? AND status='ready'").get(workspaceId, item.topicId) as { id: string } | undefined
      return row ? { ...item, exerciseSetId: row.id } : item
    }
    if (item.activityType === 'material' && !item.materialId) {
      const row = this.database.sqlite.prepare("SELECT id FROM materials WHERE workspace_id=? AND status='ready' ORDER BY CASE role WHEN 'priority' THEN 0 WHEN 'base' THEN 1 ELSE 2 END,created_at,id LIMIT 1").get(workspaceId) as { id: string } | undefined
      return row ? { ...item, materialId: row.id } : item
    }
    return item
  }
  ...
  async createState(input: StudyWorkspaceState): Promise<StudyWorkspaceState> {
    this.database.sqlite.transaction(() => {
      ...
      this.database.orm.insert(workspaceStudyStates).values({ workspaceId: input.workspaceId, activeSessionId: input.sessionId, fileName: input.fileName, language: input.language, editorContent: input.editorContent, notes: input.notes, shareContextWithAi: input.shareContextWithAi, timerDurationSeconds: input.timerDurationSeconds, timerRemainingSeconds: input.timerRemainingSeconds, timerStatus: input.timerStatus, timerStartedAt: input.timerStartedAt, timerStartedMonotonicMs: input.timerStartedMonotonicMs, timerBootId: input.timerBootId, updatedAt: input.updatedAt, documentRevision: input.documentRevision, notesRevision: input.notesRevision, accumulatedFocusSeconds: input.accumulatedFocusSeconds }).run()
      const plan = input.plan.map((item) => this.resolveTargets(input.workspaceId, item)); for (const item of plan) this.validateTargets(input.workspaceId, item)
      if (plan.length > 0) this.database.orm.insert(studyPlanItems).values(plan.map((item) => ({ ...item, workspaceId: input.workspaceId, sessionId: input.sessionId, createdAt: input.updatedAt, updatedAt: input.updatedAt }))).run()
    })()
    return input
  }

  async createStateFromAuthoritative(input: Omit<StudyWorkspaceState, 'plan' | 'timerDurationSeconds' | 'timerRemainingSeconds'>, calculatePlan: () => StudyPlanItem[]): Promise<StudyWorkspaceState> { return this.database.sqlite.transaction(() => { const plan = calculatePlan().map((item) => this.resolveTargets(input.workspaceId, item)); for (const item of plan) this.validateTargets(input.workspaceId, item); const duration = (plan.find((item) => item.status === 'active')?.durationMinutes ?? 1) * 60; const state: StudyWorkspaceState = { ...input, plan, timerDurationSeconds: duration, timerRemainingSeconds: duration }; this.database.orm.insert(studySessions).values({ id: state.sessionId, workspaceId: state.workspaceId, status: 'active', startedAt: state.sessionStartedAt, focusSeconds: 0 }).run(); this.database.orm.insert(workspaceStudyStates).values({ workspaceId: state.workspaceId, activeSessionId: state.sessionId, fileName: state.fileName, language: state.language, editorContent: state.editorContent, notes: state.notes, shareContextWithAi: state.shareContextWithAi, timerDurationSeconds: state.timerDurationSeconds, timerRemainingSeconds: state.timerRemainingSeconds, timerStatus: state.timerStatus, timerStartedAt: state.timerStartedAt, timerStartedMonotonicMs: state.timerStartedMonotonicMs, timerBootId: state.timerBootId, updatedAt: state.updatedAt, documentRevision: state.documentRevision, notesRevision: state.notesRevision, accumulatedFocusSeconds: state.accumulatedFocusSeconds }).run(); if (plan.length) this.database.orm.insert(studyPlanItems).values(plan.map((item) => ({ ...item, workspaceId: state.workspaceId, sessionId: state.sessionId, createdAt: state.updatedAt, updatedAt: state.updatedAt }))).run(); return state })() }
  ...
  async replacePlan(workspaceId: string, sessionId: string, plan: StudyPlanItem[], now: number, dayKey?: string): Promise<void> { this.database.sqlite.transaction(() => { const resolvedPlan = plan.map((item) => this.resolveTargets(workspaceId, item)); for (const item of resolvedPlan) this.validateTargets(workspaceId, item); this.database.sqlite.prepare("DELETE FROM study_plan_items WHERE workspace_id = ? AND session_id = ? AND status != 'completed'").run(workspaceId, sessionId); const insert = this.database.sqlite.prepare('INSERT OR REPLACE INTO study_plan_items (id, workspace_id, session_id, title, duration_minutes, position, status, module_id, topic_id, activity_type, exercise_set_id, material_id, scheduled_start_minutes, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'); for (const item of resolvedPlan) insert.run(item.id, workspaceId, sessionId, item.title, item.durationMinutes, item.position, item.status, item.moduleId, item.topicId, item.activityType, item.exerciseSetId, item.materialId, item.scheduledStartMinutes, now, now); if (dayKey) this.database.sqlite.prepare('UPDATE workspace_study_states SET last_planned_day_key = ? WHERE workspace_id = ?').run(dayKey, workspaceId) })() }
  async recalculatePlanAtomically(workspaceId: string, sessionId: string, calculate: () => StudyPlanItem[], now: number, dayKey?: string): Promise<void> { this.database.sqlite.transaction(() => { const plan = calculate(); this.database.sqlite.prepare("DELETE FROM study_plan_items WHERE workspace_id = ? AND session_id = ? AND status != 'completed'").run(workspaceId, sessionId); const insert = this.database.sqlite.prepare('INSERT OR REPLACE INTO study_plan_items (id, workspace_id, session_id, title, duration_minutes, position, status, module_id, topic_id, activity_type, scheduled_start_minutes, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'); for (const item of plan) insert.run(item.id, workspaceId, sessionId, item.title, item.durationMinutes, item.position, item.status, item.moduleId, item.topicId, item.activityType, item.scheduledStartMinutes, now, now); const active = plan.find((item) => item.status === 'active'); const duration = active ? active.durationMinutes * 60 : 60; const result = this.database.sqlite.prepare("UPDATE workspace_study_states SET timer_duration_seconds = ?, timer_remaining_seconds = ?, timer_status = 'idle', timer_started_at = NULL, timer_started_monotonic_ms = NULL, timer_boot_id = NULL, last_planned_day_key = COALESCE(?, last_planned_day_key), updated_at = ? WHERE workspace_id = ? AND active_session_id = ?").run(duration, duration, dayKey ?? null, now, workspaceId, sessionId); if (result.changes !== 1) throw new Error('Study workspace state not found') })() }

  async updateTimer(workspaceId: string, sessionId: string, timer: Pick<StudyWorkspaceState, 'timerStatus' | 'timerRemainingSeconds' | 'timerStartedAt' | 'timerStartedMonotonicMs' | 'timerBootId' | 'accumulatedFocusSeconds'>, now: number): Promise<void> {
  ...
  async setTimerDuration(workspaceId: string, sessionId: string, durationSeconds: number, now: number): Promise<void> {
    const result = this.database.sqlite.prepare("UPDATE workspace_study_states SET timer_duration_seconds = ?, timer_remaining_seconds = ?, timer_status = 'idle', timer_started_at = NULL, timer_started_monotonic_ms = NULL, timer_boot_id = NULL, updated_at = ? WHERE workspace_id = ? AND active_session_id = ?").run(durationSeconds, durationSeconds, now, workspaceId, sessionId)
  ...
  completeAndCreateSession(workspaceId: string, currentSessionId: string, nextSessionId: string, plan: StudyPlanItem[], focusSeconds: number, timerDurationSeconds: number, now: number): void {
    this.database.sqlite.transaction(() => {
      ...
      const changed = this.database.orm.update(workspaceStudyStates).set({ activeSessionId: nextSessionId, timerStatus: 'idle', timerStartedAt: null, timerStartedMonotonicMs: null, timerBootId: null, timerRemainingSeconds: timerDurationSeconds, accumulatedFocusSeconds: 0, updatedAt: now }).where(and(eq(workspaceStudyStates.workspaceId, workspaceId), eq(workspaceStudyStates.activeSessionId, currentSessionId))).run()
  ...
  listSessionHistory(workspaceId: string, limit: number): DailyStudyReport[] {
    ...
    const enriched = sessions.map((session) => {
      ...
      return { ...session, successRate, focusRetentionPercent: null, recommendation }
    })
    ...
    return [...reports.entries()].map(([date, day]) => { const focusSeconds = day.reduce((sum, item) => sum + item.focusSeconds, 0); const executions = day.reduce((sum, item) => sum + item.executions, 0); const errors = day.reduce((sum, item) => sum + item.errors, 0); const successRate = executions ? Math.round(Math.max(0, executions - errors) / executions * 100) : null; const focusExits = day.reduce((sum, item) => sum + item.focusExits, 0); return { date, startedAt: Math.min(...day.map((item) => item.startedAt)), endedAt: Math.max(...day.map((item) => item.endedAt)), focusSeconds, executions, errors, interventions: day.reduce((sum, item) => sum + item.interventions, 0), focusExits, completedPlanItems: day.reduce((sum, item) => sum + item.completedPlanItems, 0), successRate, focusRetentionPercent: null, sessionCount: day.length, recommendation: focusExits >= 3 ? 'Faça o próximo sprint em 15 minutos e elimine uma distração.' : null } })
  }
}

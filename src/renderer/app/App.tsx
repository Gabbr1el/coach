...
function StudyModuleView({ module, roadmap, studyProgress, onSelect, onStage, onCheckpoint, onComplete, onBack, onAskCoach, onPractice }: { module: RoadmapModule; roadmap: Roadmap; studyProgress: StudyProgressState; onSelect(module: RoadmapModule, topicIndex: number): void; onStage(checkpointId: string | null): void; onCheckpoint(correct: boolean): void; onComplete(): void; onBack(): void; onAskCoach(prompt: string): void; onPractice(): void }) {
  const topicIndex = Math.max(0, module.topics.findIndex((topic) => `${module.id}:${topic}` === studyProgress.topicId))
  const [stage, setStage] = useState<StudyStage>('explanation')
  ...
  const resetTopic = (index: number) => { setStage('explanation'); setSelected(null); setAttempts(0); onSelect(module, index) }
  const choose = (index: number) => {
    setSelected(index)
    setAttempts((value) => value + 1)
    setStage(nextStudyStage('verification', index === lesson.verification.correct))
    onCheckpoint(index === lesson.verification.correct)
  ...
  return <div className="grid h-full min-h-0 grid-cols-[210px_minmax(0,1fr)] bg-[#0b0c10]"><aside className="coach-scroll-pane border-r border-coach-line bg-[#0e0f13] p-4"><button onClick={onBack} className="text-xs font-black text-coach-muted">Voltar ao plano</button><p className="mt-6 text-[10px] font-black uppercase tracking-wider text-coach-muted">Roteiro</p><p className="mt-1 text-xs text-coach-green">{progressPercent}% concluido</p><div className="mt-2 h-1 rounded bg-coach-line"><div className="h-full rounded bg-coach-green" style={{ width: `${progressPercent}%` }} /></div><div className="mt-5 space-y-2">{roadmap.modules.map((item) => <div key={item.id}><button onClick={() => onSelect(item, 0)} className={`w-full rounded-lg px-3 py-2 text-left text-xs font-black ${item.id === module.id ? 'bg-[#1a1c22] text-white' : 'text-coach-muted'}`}>{item.position}. {item.title}</button>{item.id === module.id && <div className="ml-3 mt-1 border-l border-coach-line pl-3">{item.topics.map((entry, index) => { const id = `${item.id}:${entry}`; const status = studyProgress.topicStatuses[id] ?? 'NOT_STARTED'; return <button key={entry} onClick={() => resetTopic(index)} className={`block w-full py-1.5 text-left text-[11px] ${index === topicIndex ? 'font-black text-coach-orange' : 'text-coach-muted'}`}>{index === topicIndex ? '>' : status === 'COMPLETED' ? 'x' : status === 'IN_PROGRESS' ? '-' : 'o'} {entry}</button> })}</div>}</div>)}</div></aside><article className="coach-scroll-pane h-full px-6 py-7 lg:px-12"><div className="mx-auto max-w-4xl"><div className="flex items-center justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-coach-green">Tutor adaptativo - ensino antes da avaliacao</p><h2 className="mt-2 font-display text-3xl font-black">{topic}</h2></div><span className="rounded-full border border-coach-line px-3 py-2 text-[10px] font-black text-coach-muted">ESTUDO - {Math.max(3, Math.round(module.estimatedMinutes / Math.max(1, module.topics.length)))} MIN</span></div><div className="mt-6 flex flex-wrap gap-2">{stages.map(([id, label], index) => <span key={id} className={`rounded-full px-3 py-1.5 text-[10px] font-black ${id === stage ? 'bg-coach-green text-[#0c0d10]' : activeStage > index ? 'bg-coach-green/15 text-coach-green' : 'border border-coach-line text-coach-muted'}`}>{index + 1}. {label}</span>)}</div>{stage === 'explanation' && <section className="coach-enter mt-6 rounded-xl border border-coach-line bg-[#111217] p-6"><p className="text-[10px] font-black uppercase tracking-wider text-coach-orange">Primeiro, vamos construir o conceito</p><h3 className="mt-2 font-display text-xl font-black">Como e por que funciona</h3><p className="mt-4 text-sm leading-7 text-coach-muted">{lesson.concept}</p><p className="mt-3 text-sm leading-7 text-coach-muted">{lesson.whyItWorks}</p><ol className="mt-5 space-y-3">{lesson.steps.map((step, index) => <li key={step} className="rounded-lg bg-[#17191f] p-3 text-sm leading-6 text-[#d5d8df]"><strong className="mr-2 text-coach-green">{index + 1}.</strong>{step}</li>)}</ol><button onClick={() => setStage(nextStudyStage(stage))} className="mt-6 rounded-lg bg-coach-green px-5 py-3 text-xs font-black text-[#0c0d10]">Ver exemplo passo a passo</button></section>}{stage === 'example' && <section className="coach-enter mt-6 rounded-xl border border-coach-line bg-[#111217] p-6"><p className="text-[10px] font-black uppercase tracking-wider text-coach-orange">Exemplo guiado</p><pre className="mt-4 overflow-x-auto rounded-lg bg-[#090a0d] p-4 text-xs text-[#d5d8df]"><code>{lesson.example}</code></pre><ol className="mt-5 space-y-3">{lesson.walkthrough.map((step, index) => <li key={step} className="flex gap-3 text-sm leading-6 text-coach-muted"><strong className="text-coach-green">{index + 1}</strong><span>{step}</span></li>)}</ol><button onClick={() => { onStage(lesson.verification.id); setStage(nextStudyStage(stage)) }} className="mt-6 rounded-lg bg-coach-green px-5 py-3 text-xs font-black text-[#0c0d10]">Verificar meu entendimento</button></section>}{stage === 'verification' && <section className="coach-enter mt-6 rounded-xl border border-coach-line bg-[#111217] p-6"><p className="text-[10px] font-black uppercase tracking-wider text-coach-orange">Verificacao apos a explicacao</p><h3 className="mt-2 font-display text-xl font-black">{lesson.verification.question}</h3><div className="mt-4 grid gap-2">{lesson.verification.options.map((option, index) => <button key={option} onClick={() => choose(index)} className="rounded-lg border border-coach-line bg-[#17191f] p-3 text-left text-xs hover:border-coach-green">{String.fromCharCode(65 + index)}. {option}</button>)}</div></section>}{stage === 'exercise' && <section className="coach-enter mt-6 rounded-xl border border-coach-green/40 bg-[#111217] p-6"><p className="text-[10px] font-black uppercase tracking-wider text-coach-green">Exercicio de aplicacao</p><h3 className="mt-2 font-display text-xl font-black">Agora faca, nao apenas reconheca</h3><p className="mt-3 text-sm leading-7 text-coach-muted">{lesson.exercise}</p><div className="mt-5 flex flex-wrap gap-3"><button onClick={onPractice} className="rounded-lg bg-coach-green px-5 py-3 text-xs font-black text-[#0c0d10]">Abrir Pratica</button><button onClick={() => setStage(nextStudyStage(stage))} className="rounded-lg border border-coach-line px-5 py-3 text-xs font-black text-coach-muted">Conclui o exercicio</button></div><p className="mt-4 text-xs text-coach-muted">Abrir o topico ou a pratica nao significa dominio; o Coach usa verificacao e exercicio como evidencias separadas.</p></section>}{stage === 'feedback' && <section className={`coach-enter mt-6 rounded-xl border p-6 ${correct ? 'border-coach-green/40 bg-coach-green/10' : 'border-coach-orange/40 bg-coach-orange/10'}`}><p className="text-[10px] font-black uppercase tracking-wider text-coach-orange">Feedback da tentativa {attempts}</p><h3 className="mt-2 font-display text-xl font-black">{correct ? 'Compreensao aplicada' : 'Vamos reconstruir um ponto especifico'}</h3><p className="mt-3 text-sm leading-7 text-coach-muted">{correct ? lesson.success : lesson.reinforcement}</p>{!correct && <p className="mt-3 text-xs text-coach-muted">Ainda nao mostrarei a alternativa correta. Releia esta explicacao por outra perspectiva e tente novamente.</p>}<button onClick={() => { if (correct) advance(); else { setSelected(null); setStage(nextStudyStage(stage, false)) } }} className="mt-5 rounded-lg bg-coach-green px-5 py-3 text-xs font-black text-[#0c0d10]">{correct ? 'Ir para o proximo conteudo' : 'Tentar novamente'}</button></section>}</div></article></div>
}

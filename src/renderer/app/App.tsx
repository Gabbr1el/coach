...
function StudyModuleView({ module, roadmap, onBack, onAskCoach, onPractice }: { module: RoadmapModule; roadmap: Roadmap | null; onBack(): void; onAskCoach(prompt: string): void; onPractice(): void }) {
  return <div className="coach-scroll-pane h-full p-5 lg:p-8"><article className="mx-auto max-w-4xl"><button onClick={onBack} className="text-xs font-black text-coach-muted">← Voltar à visão geral</button><p className="mt-6 text-[10px] font-black uppercase tracking-[.18em] text-coach-green">Estudo · Etapa {module.position} de {roadmap?.modules.length ?? 1}</p><h2 className="mt-2 font-display text-3xl font-black">{module.title}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-coach-muted">{module.objective}</p><div className="mt-7 grid gap-5 lg:grid-cols-[minmax(0,1fr)_280px]"><section className="rounded-xl border border-coach-line bg-[#111217] p-6"><h3 className="font-display text-xl font-black">Conteúdo desta etapa</h3><ol className="mt-5 space-y-4">{module.topics.map((topic, index) => <li key={topic} className="flex gap-3"><span className="grid size-7 shrink-0 place-items-center rounded-full bg-coach-green/10 text-xs font-black text-coach-green">{index + 1}</span><div><strong className="text-sm">{topic}</strong><button onClick={() => onAskCoach(`Estou estudando a etapa “${module.title}”. Ensine “${topic}” com uma explicação curta, um exemplo e uma pergunta para verificar se entendi.`)} className="mt-1 block text-[10px] font-black text-coach-orange">Aprender com o Coach</button></div></li>)}</ol></section><aside className="space-y-4"><div className="rounded-xl bg-[#181622] p-5"><span className="text-[10px] font-black uppercase text-coach-orange">Prática da etapa</span><p className="mt-2 text-xs leading-5 text-coach-muted">{module.practice}</p><button onClick={onPractice} className="mt-4 w-full rounded-lg bg-coach-orange px-3 py-2 text-xs font-black text-[#0c0d10]">Abrir ambiente de prática</button></div><div className="rounded-xl border border-coach-line bg-[#111217] p-5"><span className="text-[10px] font-black uppercase text-coach-muted">Critérios de domínio</span>{module.completionCriteria.map((criterion) => <p key={criterion} className="mt-2 text-xs text-coach-muted">✓ {criterion}</p>)}</div>{module.resources.map((resource) => <a key={resource.url} href={resource.url} target="_blank" rel="noreferrer" className="block rounded-xl border border-coach-line bg-[#111217] p-4 text-xs font-black text-coach-green">{resource.title} ↗</a>)}</aside></div></article></div>
}

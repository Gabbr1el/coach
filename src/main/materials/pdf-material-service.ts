...
export class PdfMaterialService {
  ...
  async importPdf(workspaceId: string, path: string): Promise<MaterialSummary> {
    ...
    const material: MaterialSummary = { id: crypto.randomUUID(), name: basename(path).slice(0, 240), pageCount: pages.length, createdAt: Date.now() }
    ...
    this.database.sqlite.transaction(() => {
      this.database.sqlite.prepare('INSERT INTO materials (id, workspace_id, name, media_type, page_count, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(material.id, workspaceId, material.name, 'application/pdf', material.pageCount, material.createdAt)
  ...
  list(workspaceId: string): MaterialSummary[] { return this.database.sqlite.prepare('SELECT id, name, page_count AS pageCount, created_at AS createdAt FROM materials WHERE workspace_id = ? ORDER BY created_at DESC').all(workspaceId) as MaterialSummary[] }
  search(workspaceId: string, query: string): MaterialSearchResult[] {
    ...
    const rows = this.database.sqlite.prepare(`SELECT m.id AS materialId, m.name AS materialName, c.page_number AS pageNumber, c.content FROM material_chunks c JOIN materials m ON m.id = c.material_id WHERE m.workspace_id = ? AND (${clauses.replaceAll('LIKE ?', "LIKE ? ESCAPE '\\'")}) LIMIT 50`).all(workspaceId, ...terms.map((term) => `%${escapeLike(term)}%`)) as MaterialSearchResult[]

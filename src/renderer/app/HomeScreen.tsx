import { BarChart3, Brain, CalendarDays, ChevronRight, Clock3, FolderKanban, Home, MoreHorizontal, Plus, Settings, Sparkles } from 'lucide-react'
...
import type { AcademicOverview, WeeklyPlan, WeeklyPlanDay, WorkspacePriority } from '../../shared/contracts/planning-contract'
...
import { AIConnectionPage, type AIConnectionPageProps } from './AIConnectionPage'
import { ChatMessage } from './ChatMessage'
import { ChatComposer } from './ChatComposer'
import { isNearChatBottom, scrollChatToLatest } from './chat-experience'

export type HomeSection = 'home' | 'academic' | 'workspaces' | 'reports' | 'ai'
...
function Rail({ active, onSection, onSettings }: { active: HomeSection; onSection(section: HomeSection): void; onSettings(): void }) {
  ...
  return <aside className="flex h-full w-[64px] shrink-0 flex-col border-r border-[#202229] bg-[#0d0e12] px-2 py-5 text-[#f4f5f7] sm:w-[70px] sm:px-3 lg:w-[196px] lg:px-4 lg:py-6"><div className="flex items-center gap-3 px-2"><span className="grid size-8 place-items-center rounded-lg bg-[#8c7cff] text-[#0d0e12]"><Brain size={17} /></span><strong className="hidden text-[15px] lg:inline">Coach</strong></div><nav className="mt-8 space-y-1 lg:mt-10">{items.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => onSection(id)} aria-current={active === id ? 'page' : undefined} aria-label={label} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-[13px] font-semibold ${active === id ? 'bg-[#22252e] text-white' : 'text-[#9297a3] hover:bg-[#17191f] hover:text-white'}`}><Icon size={16} aria-hidden="true" /><span className="hidden lg:inline">{label}</span></button>)}</nav><button onClick={onSettings} aria-current={active === 'ai' ? 'page' : undefined} aria-label="Configurações" className={`mt-auto flex items-center gap-3 rounded-lg px-3 py-2.5 text-[13px] font-semibold ${active === 'ai' ? 'bg-[#22252e] text-white' : 'text-[#9297a3] hover:bg-[#17191f] hover:text-white'}`}><Settings size={16} aria-hidden="true" /><span className="hidden lg:inline">Configurações</span></button><div className="mt-4 flex items-center gap-3 border-t border-[#202229] px-2 pt-5" aria-label="Perfil local"><span className="grid size-8 place-items-center rounded-full bg-[#282b35] text-[11px] font-bold" aria-hidden="true">U</span><div className="hidden lg:block"><p className="text-xs font-semibold">Usuário</p><p className="text-[10px] text-[#9297a3]">Perfil local</p></div></div></aside>
}

function WorkspaceRow({ workspace, priority, onOpen, onArchive, onRetry, onContinuationResolved }: { workspace: WorkspaceSummary; priority: WorkspacePriority | undefined; onOpen(id: string): void; onArchive?: (id: string) => void; onRetry(id: string): void; onContinuationResolved?: () => void }) {
  ...
  const status = workspace.status === 'completed' ? 'Concluído' : workspace.status === 'archived' ? 'Arquivado' : stage ?? (priority?.level === 'urgent' ? 'Urgente' : priority?.level === 'attention' ? 'Atenção' : 'No ritmo')
  ...
  const detail = workspace.repairConflict ? `Conflito duplicado preservado: ambos continham evidências · Workspace canônico ${workspace.repairConflict.canonicalWorkspace.name}` : provisioning && !ready ? [provisioning.errorMessage ?? provisioning.eta?.label ?? 'Calculando estimativa', progress].filter(Boolean).join(' · ') : provisioning?.errorMessage ?? (provisioning?.progress.background ? `Segundo plano · ${progress}` : priority?.reason || workspace.objective || 'Sem objetivo definido')
  return <><article className="group grid grid-cols-[minmax(0,1fr)_76px_34px] items-center gap-3 border-b border-[#202229] px-1 py-4 sm:grid-cols-[minmax(0,1fr)_110px_110px_34px]"><div className="min-w-0"><button disabled={!ready} onClick={() => onOpen(workspace.id)} className="min-w-0 text-left disabled:cursor-wait disabled:opacity-70"><strong className="block truncate text-[13px] text-[#eceef2]">{workspace.name}</strong><span className="mt-1 block truncate text-[11px] text-[#626773]">{detail}</span></button>{canRetry && <button type="button" onClick={() => onRetry(workspace.id)} className="mt-1 text-[10px] font-bold text-[#8c7cff] hover:text-white">Tentar agora</button>}</div><span className={`text-[11px] font-semibold ${stage ? 'text-[#8c7cff]' : priority?.level === 'urgent' ? 'text-[#ef8d93]' : priority?.level === 'attention' ? 'text-[#e8b96d]' : 'text-[#72d7aa]'}`}>{status}</span><span className="hidden text-right text-[11px] text-[#747986] sm:block">{workspace.lastOpenedAt ? new Intl.DateTimeFormat('pt-BR', { day: '2-digit', month: 'short' }).format(workspace.lastOpenedAt) : 'Novo'}</span>{onArchive ? <button onClick={() => onArchive(workspace.id)} className="grid size-8 place-items-center rounded-md text-[#626773] opacity-100 hover:bg-[#22252e] hover:text-white lg:opacity-0 lg:focus-visible:opacity-100 lg:group-hover:opacity-100" aria-label={`Arquivar ${workspace.name}`}><MoreHorizontal size={16} /></button> : <span />}</article>{workspace.continuationRecommendation && !continuationDismissed && <section className="mb-3 rounded-lg border border-[#8c7cff]/30 bg-[#8c7cff]/[.08] p-4"><strong className="text-xs text-white">Próximo passo: {workspace.continuationRecommendation.suggestedName}</strong><p className="mt-1 text-[11px] leading-5 text-[#9297a3]">{workspace.continuationRecommendation.rationale}</p><div className="mt-3 flex gap-2"><button onClick={() => void window.coach.workspace.acceptContinuation(workspace.id).then((next) => { onContinuationResolved?.(); onOpen(next.id) })} className="rounded-md bg-[#8c7cff] px-3 py-2 text-[10px] font-bold text-[#0c0d10]">{workspace.continuationRecommendation.action === 'open_existing' ? 'Abrir existente' : 'Criar continuação'}</button><button onClick={() => void window.coach.workspace.declineContinuation(workspace.id).then(() => { setContinuationDismissed(true); onContinuationResolved?.() })} className="rounded-md border border-[#343842] px-3 py-2 text-[10px] font-bold text-[#9297a3]">Agora não</button></div></section>}</>
}
...
export function HomeScreen({ section, loading, error, report, weeklyPlan, academicLife, workspaces, workspaceHistory, priorities, messages, streamedContent, plannerActions, plannerInput, plannerBusy, plannerError, providerLabel, providerConnected, providerUnavailable, aiConnection, onSection, onSettings, onCreate, onOpen, onArchive, onRetryProvisioning, onPlanNavigate, onPlannerInput, onPlannerSend, onResolveAction, onRefreshAcademicLife }: { section: HomeSection; loading: boolean; error: string | null; report: GlobalReportOverview | null; schedule: import('../../shared/contracts/planning-contract').StudyScheduleItem[]; weeklyPlan: WeeklyPlan | null; academicOverview: AcademicOverview | null; academicLife: AcademicLifeProjection | null; workspaces: WorkspaceSummary[]; workspaceHistory: WorkspaceSummary[]; priorities: WorkspacePriority[]; messages: ConversationMessage[]; streamedContent: string; plannerActions: PlannerAction[]; plannerInput: string; plannerBusy: boolean; plannerError: string | null; providerLabel: string; providerConnected: boolean; providerUnavailable: boolean; aiConnection: AIConnectionPageProps; onSection(section: HomeSection): void; onSettings(): void; onCreate(): void; onOpen(id: string): void; onArchive(id: string): void; onRetryProvisioning(id: string): void; onPlanNavigate(dateKey?: string): void; onPlannerInput(value: string): void; onPlannerSend(): void; onResolveAction(id: string, decision: 'apply' | 'reject'): void; onRefreshAcademicLife(affectsPlanning: boolean): void | Promise<void> }) {
  ...
  else if (section === 'workspaces') content = <div className="mx-auto w-full max-w-6xl"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#626773]">Biblioteca</p><h1 className="mt-2 text-3xl font-semibold text-white">Seus Workspaces</h1></div><button onClick={onCreate} className="flex items-center gap-2 rounded-lg bg-[#8c7cff] px-4 py-2.5 text-xs font-bold text-[#0c0d10]"><Plus size={15} />Novo Workspace</button></div><h2 className="mt-8 text-xs font-bold uppercase tracking-[.16em] text-[#72d7aa]">Ativos</h2><div className="mt-2 border-t border-[#292c35]">{workspaces.map((workspace) => <WorkspaceRow key={workspace.id} workspace={workspace} priority={priorities.find((item) => item.workspaceId === workspace.id)} onOpen={onOpen} onArchive={onArchive} onRetry={onRetryProvisioning} />)}</div>{(['completed', 'archived'] as const).map((status) => { const rows = workspaceHistory.filter((item) => item.status === status); return rows.length ? <section key={status}><h2 className="mt-8 text-xs font-bold uppercase tracking-[.16em] text-[#747986]">{status === 'completed' ? 'Concluídos' : 'Arquivados'}</h2><div className="mt-2 border-t border-[#292c35]">{rows.map((workspace) => <WorkspaceRow key={workspace.id} workspace={workspace} priority={undefined} onOpen={onOpen} onRetry={onRetryProvisioning} />)}</div></section> : null })}</div>

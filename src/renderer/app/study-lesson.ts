...
export interface StudyLesson {
  ...
  example: string
  walkthrough: string[]
  verification: { question: string; options: string[]; correct: number; gaps: string[] }
  exercise: string
  success: string
...
export function lessonFor(topic: string, module: RoadmapModule): StudyLesson {
  if (/ponteiro|pointer/i.test(topic)) return {
    ...
    verification: { question: 'Depois de int n = 7; int *p = &n;, o que representa p?', options: ['O valor 7', 'O endereco de n', 'Uma copia independente de n'], correct: 1, gaps: ['diferenca entre valor e endereco', 'significado do operador &', 'relacao entre copia e referencia'] },
  ...
  if (/sintaxe|syntax/i.test(topic)) return {
    ...
    verification: { question: 'Qual opcao tem uma estrutura que o interpretador nao consegue concluir?', options: ['print("Ola")', 'idade = 20', 'print("Ola"'], correct: 2, gaps: ['fechamento de parenteses', 'atribuicao valida', 'delimitacao de texto'] },
  ...
  return {
    ...
    verification: { question: `Qual evidencia demonstra compreensao de ${topic}?`, options: [module.completionCriteria[0] ?? 'Aplicar e explicar o conceito', 'Somente abrir o topico', 'Copiar uma resposta sem verificar'], correct: 0, gaps: ['aplicacao verificavel', 'abertura sem aprendizagem', 'dependencia de resposta pronta'] },

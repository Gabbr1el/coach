...
export class HomeOrganizerService {
  ...
  async organize(input: SendHomeMessageInput): Promise<{ messages: ConversationMessage[]; result: HomeOrganizerResult }> {
    const content = input.content.trim(); const clock = this.clock?.() ?? currentClock(this.now); const version = clock.currentTime; const pending = this.actions.listPending()

...
function parseDiagnostics(language: ProjectLanguage, stderr: string): CodeDiagnostic[] {
...
export class ToolchainManager {
  ...
  async execute(project: WorkspaceProject, signal?: AbortSignal): Promise<CodeExecutionResult> {
    ...
    try {
      if (project.language === 'python') return { ...await spawnLimited(BWRAP_PATH, sandboxArguments(['/usr/bin/python3', '-I', '-B', `/project/${project.entryFilePath}`], [{ source: directory, destination: '/project' }], '/project'), `python3 -I -B ${project.entryFilePath}`, 5_000, signal), phase: 'run', diagnostics: [] }

...
export function registerRoadmapHandlers(service: RoadmapService): void { ipcMain.handle(ROADMAP_CHANNELS.get, (event, payload: unknown) => { assertTrustedSender(event); return service.get(workspaceRoadmapInputSchema.parse(payload).workspaceId) }); ipcMain.handle(ROADMAP_CHANNELS.generate, async (event, payload: unknown) => { assertTrustedSender(event); const input = workspaceRoadmapInputSchema.parse(payload); await service.ensureLearningPath(input.workspaceId); return service.get(input.workspaceId) }); ipcMain.handle(ROADMAP_CHANNELS.accept, (event, payload: unknown) => { assertTrustedSender(event); const input = acceptRoadmapInputSchema.parse(payload); return service.accept(input.workspaceId, input.roadmapId) }) }

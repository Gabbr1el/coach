...
import type { CurricularScope } from '../../shared/contracts/workspace-onboarding-contract'
import { z } from 'zod'
...
const compactEvidenceSchema = z.enum(['mc', 'code', 'predict', 'write'])
const compactDifficultySchema = z.enum(['intro', 'standard', 'challenge'])
const compactTopicSchema = z.tuple([z.string().trim().min(1).max(240), z.array(z.tuple([z.string().trim().min(1).max(120).regex(/^[a-z0-9][a-z0-9._-]*$/), z.string().trim().min(1).max(240), z.string().trim().min(1).max(120), z.string().trim().min(1).max(600), compactEvidenceSchema, compactDifficultySchema, z.array(z.string().trim().min(1).max(120).regex(/^[a-z0-9][a-z0-9._-]*$/)).max(30)])).min(1).max(2)])
const compactRoadmapProposalSchema = z.object({ t: z.string().trim().min(1).max(160), m: z.array(z.tuple([z.string().trim().min(1).max(160), z.string().trim().min(1).max(500), z.number().int().min(10).max(2400), z.array(compactTopicSchema).min(1).max(3), z.string().trim().min(1).max(240), z.string().trim().min(1).max(600), z.string().trim().min(1).max(240), z.array(z.string().trim().min(1)).max(4)])).min(1).max(16) }).strict()
const evidenceTypes = { mc: 'multiple_choice', code: 'code_execution', predict: 'prediction', write: 'constructed_response' } as const
const difficulties = { intro: 'introductory', standard: 'standard', challenge: 'challenge' } as const

export function expandCompactRoadmapProposal(value: unknown): unknown {
  const compact = compactRoadmapProposalSchema.parse(value)
  const intentKeys = new Set<string>()
  return { title: compact.t, modules: compact.m.map(([title, objective, estimatedMinutes, compactTopics, outcome, practice, completionCriterion, sourceIds]) => ({ title, objective, estimatedMinutes, topics: compactTopics.map(([topic]) => topic), curricularTopics: compactTopics.map(([topic, tuples]) => ({ topic, concepts: tuples.map(([key, name, domain]) => ({ key, name, aliases: [], domain })), assessmentIntents: tuples.map(([conceptKey, , , assessmentObjective, evidenceType, difficulty, prerequisiteConceptKeys]) => { const key = `${conceptKey}.assessment`; if (intentKeys.has(key)) throw new Error(`Duplicate compact assessment key ${key}`); intentKeys.add(key); return { key, conceptKey, objective: assessmentObjective, evidenceType: evidenceTypes[evidenceType], difficulty: difficulties[difficulty], prerequisiteConceptKeys } }) })), outcomes: [outcome], practice, completionCriteria: [completionCriterion], sourceIds })) }
}

function normalizeGeneratedRoadmapProposal(value: unknown): unknown {
  return value && typeof value === 'object' && ('t' in value || 'm' in value) ? expandCompactRoadmapProposal(value) : value
}
...
export interface RoadmapAcademicContext { difficulties: string[]; deadline: number | null; availability: Array<{ weekday: number; minutes: number }>; knownContext: string[]; curricularScope?: Partial<CurricularScope> }
...
export function hasCompressedModules(proposal: ReturnType<typeof roadmapProposalSchema.parse>): boolean {
  return proposal.modules.some((module) => {
    const distinctTopics = new Set(module.topics.map(semanticKey)).size
    return distinctTopics > 1 && module.estimatedMinutes > 720 && module.estimatedMinutes / distinctTopics > 240
  })
}

export type LearningPathErrorCode = 'PROVIDER_UNAVAILABLE' | 'PROVIDER_TIMEOUT' | 'PROVIDER_REQUEST_FAILED' | 'PROVIDER_INVALID_RESPONSE' | 'JSON_EXTRACTION_FAILED' | 'ROADMAP_SCHEMA_INVALID' | 'ROADMAP_GENERIC_REJECTED' | 'ROADMAP_COMPRESSED' | 'ROADMAP_SCOPE_MISALIGNED' | 'ROADMAP_OBVIOUSLY_TRUNCATED' | 'CURRICULUM_SOURCE_FAILED' | 'ROADMAP_PERSISTENCE_FAILED' | 'UNKNOWN_GENERATION_ERROR'
...
export const ROADMAP_GENERATION_MAX_OUTPUT_TOKENS = 1800
export const ROADMAP_REPAIR_MAX_OUTPUT_TOKENS = 1800
...
function schemaDescription(): string { return '{"t":"title","m":[1..16 [moduleTitle,objective,minutes,topics,outcome,practice,completion,sourceIds]]}; topics=1..3 [topicName,concepts]; concepts=1..2 [conceptKey,conceptName,domain,assessmentObjective,evidence,difficulty,prerequisiteKeys]; evidence=mc|code|predict|write; difficulty=intro|standard|challenge' }
...
function scopeText(value: string): string { return value.normalize('NFD').replace(/\p{M}/gu, '').toLocaleLowerCase('pt-BR').replace(/[^a-z0-9+#]+/g, ' ').trim() }
function semanticallyIncludes(value: string, expected: string): boolean { const haystack = scopeText(value); const needles = scopeText(expected).split(' ').filter((item) => item.length > 2); return needles.length > 0 && needles.every((needle) => haystack.split(' ').some((word) => word === needle || word.startsWith(needle) || needle.startsWith(word))) }
function roadmapScopeIssue(proposal: ReturnType<typeof roadmapProposalSchema.parse>, scope?: Partial<CurricularScope>): string | null {
  const requested = scope?.requestedStart?.trim() || scope?.focusFrom?.trim()
  if (!requested) return null
  const modules = proposal.modules.slice(0, 2)
  const direct = semanticallyIncludes(`${modules[0]?.title ?? ''} ${modules[0]?.objective ?? ''} ${(modules[0]?.topics ?? []).join(' ')}`, requested)
  if (direct) return null
  const second = modules[1]
  const reachesSecond = Boolean(second && semanticallyIncludes(`${second.title} ${second.objective} ${second.topics.join(' ')}`, requested))
  const firstIsBriefReview = Boolean(reachesSecond && modules[0]!.estimatedMinutes <= 90 && /\b(?:revis[aã]o|pr[eé]-requisito|ponte|recapitula)/i.test(`${modules[0]!.title} ${modules[0]!.objective}`))
  return firstIsBriefReview ? null : `O início solicitado “${requested}” não aparece no começo da Trilha nem após uma revisão breve e justificada.`
}
function roadmapIsObviouslyTruncated(proposal: ReturnType<typeof roadmapProposalSchema.parse>, workspace: Workspace, scope?: Partial<CurricularScope>): boolean {
  const target = scope?.coveredThrough || workspace.objective
  if (!target?.trim()) return false
  const final = proposal.modules.at(-1)!
  const all = proposal.modules.map((module) => `${module.title} ${module.objective} ${module.topics.join(' ')} ${module.outcomes.join(' ')} ${module.completionCriteria.join(' ')}`).join(' ')
  if (semanticallyIncludes(all, target) || semanticallyIncludes(`${final.title} ${final.objective} ${final.outcomes.join(' ')}`, target)) return false
  if (scope?.coveredThrough) return true
  return /\b(?:at[eé]|through|completo|completa|todo|toda|prova|certifica|avançad|avancad)\b/i.test(target)
}
...
export class RoadmapService {
  ...
  private async ensureOnce(workspaceId: string, options: { forceProviderRetry?: boolean }): Promise<LearningPathState> {
    ...
    if (current?.generationKind === 'ai_generated' && validRoadmap(current)) {
      const academic = this.getAcademicContext(workspaceId)
      const persistedProposal = roadmapProposalSchema.parse({ title: current.title, modules: current.modules.map(({ id: _id, position: _position, status: _status, ...module }) => module) })
      if (roadmapIsObviouslyTruncated(persistedProposal, workspace, academic.curricularScope) && !this.repository.findLatestRebuildPreview?.(workspaceId) && this.repository.saveRebuildPreview) {
        const provider = this.providers.route('roadmap')
        if (provider) try { await this.previewCompletenessReconciliation(workspace, current, provider) } catch {}
      }
      return this.repository.setLearningPathState({ workspaceId, status: 'ready', activeRoadmapId: current.id, lastAttemptAt: state?.lastAttemptAt ?? null, retryAfter: null, lastErrorCode: null, updatedAt: now })
    }
  ...
  private async previewCompletenessReconciliation(workspace: Workspace, current: Roadmap, provider: AIProvider): Promise<RoadmapRebuildPreview> {
    const candidate = await this.generateCandidate(workspace, provider, 'Reconcilie a Trilha legada truncada: complete o escopo solicitado sem remover conteúdo existente. A resposta é somente uma prévia e não será aplicada automaticamente.')
    ...
    const modules = candidate.proposal.modules.map((module, index) => {
      const old = [...unused].find((item) => semanticKey(item.title) === semanticKey(module.title) || overlap(conceptKeys(item), conceptKeys(module)) > 0)
      if (old) unused.delete(old)
      const topics = module.topics.map((topic) => { const equivalent = old?.topics.find((item) => semanticKey(item) === semanticKey(topic)); if (old && equivalent) preservedTopicIds.push(`${old.id}:${equivalent}`); return equivalent ?? topic })
      return { id: old?.id ?? this.createId(), ...module, topics, curricularTopics: module.curricularTopics?.map((topic, topicIndex) => ({ ...topic, topic: topics[topicIndex]! })), position: index + 1, status: old?.status ?? (index === 0 ? 'active' as const : 'locked' as const) }
    })
    ...
    const progressed = new Set(this.repository.progressedTopicIds?.(workspace.id) ?? [])
    const preview: RoadmapRebuildPreview = { id: this.createId(), workspaceId: workspace.id, currentRoadmapId: current.id, title: candidate.proposal.title, modules, materialIds: [], impact: { relevance: 'compatible', summary: 'A Trilha legada parece terminar antes do escopo solicitado. Esta reconciliação completa a cobertura sem alterar nada até sua confirmação.', preservedModuleIds: modules.filter((module) => current.modules.some((old) => old.id === module.id)).map((module) => module.id), preservedTopicIds, changedTopicIds: [], addedTopics: nextTopicIds.filter((id) => !oldTopicIds.includes(id)), removedTopics, unsafeProgressTopicIds: removedTopics.filter((id) => progressed.has(id)), reasons: ['Cobertura final solicitada não aparece semanticamente na Trilha atual.', 'IDs, progresso e evidências equivalentes serão preservados na aplicação confirmada.'], requiresAcknowledgement: true }, status: 'pending', appliedRoadmapId: null, createdAt: this.now(), resolvedAt: null }
  ...
  private async generateCandidate(workspace: Workspace, provider: AIProvider, instruction?: string, materialIds: string[] = [], signal?: AbortSignal): Promise<{ proposal: ReturnType<typeof roadmapProposalSchema.parse>; response: AIResponse }> {
    ...
    const curricularScope = { requestedStart: academic.curricularScope?.requestedStart ?? null, coveredThrough: academic.curricularScope?.coveredThrough ?? null, focusFrom: academic.curricularScope?.focusFrom ?? null, skipFundamentals: academic.curricularScope?.skipFundamentals ?? false, preferences: academic.curricularScope?.preferences ?? [] }
    const request = { messages: [{ role: 'system' as const, content: `Crie a estrutura completa de uma trilha curricular até o objetivo final. A quantidade de módulos emerge do escopo: decomponha currículos amplos em etapas coesas, navegáveis e progressivas; nunca comprima uma disciplina completa em um módulo longo. Escopo estreito pode ter um módulo. Retorne JSON compacto, sem explicações nem texto redundante: ${schemaDescription()}. Cada tópico mantém Concepts canônicos e AssessmentIntents; use um conceito por tópico por padrão e um segundo somente quando semanticamente indispensável, sempre com um intent por conceito. Keys são estáveis; intent referencia seu conceito e apenas pré-requisitos anteriores. Respeite requestedStart/focusFrom no começo, salvo revisão breve justificada, e cubra coveredThrough. skipFundamentals e contexto DECLARED são preferência/auto-relato, nunca mastery ou evidência; somente OBSERVED é evidência. Sem setup, módulos genéricos, URLs ou sourceIds não fornecidos.` }, { role: 'user' as const, content: JSON.stringify({ subject: workspace.name, objective: workspace.objective || null, curricularScope, declaredContext: academic.knownContext.slice(0, 4), observedLearning: academic.difficulties.slice(0, 4), deadline: academic.deadline, availability: academic.availability.slice(0, 7), materialExcerpts: materialExcerpts.slice(0, 3).map((item) => ({ ...item, content: item.content.slice(0, 1200) })), providedSources: [...allowed.keys()].slice(0, 4), instruction: instruction ?? null }) }], maxOutputTokens: ROADMAP_GENERATION_MAX_OUTPUT_TOKENS, responseFormat: 'json_object' as const }
    let response: AIResponse
    try { response = await provider.sendMessage({ ...request, signal }); logProgress(workspaceId, 'provider_response', { providerId: response.providerId, modelId: response.modelId, contentLength: response.content.length }) } catch (error) { throw diagnosticError(error, 'provider_response') }
    ...
    try { proposal = this.parseProposal(workspaceId, response.content, allowed, setupObjective); const scopeIssue = roadmapScopeIssue(proposal, curricularScope); if (scopeIssue) throw new LearningPathGenerationError('ROADMAP_SCOPE_MISALIGNED', 'generic_validation', scopeIssue, { response: response.content }); if (roadmapIsObviouslyTruncated(proposal, workspace, curricularScope)) throw new LearningPathGenerationError('ROADMAP_OBVIOUSLY_TRUNCATED', 'generic_validation', 'A resposta termina antes de cobrir semanticamente o escopo curricular solicitado.', { response: response.content }) }
    catch (initialError) {
      ...
      const repairReason = diagnostic.code === 'ROADMAP_GENERIC_REJECTED' ? `Os módulos foram rejeitados porque são genéricos. Produza tópicos concretos específicos de ${workspace.name}.` : diagnostic.code === 'ROADMAP_COMPRESSED' ? 'A estrutura comprime conceitos distintos e carga extensa em módulos pedagogicamente largos demais. Redistribua todo o conteúdo em etapas coesas e navegáveis, preservando a cobertura e a ordem de pré-requisitos; a quantidade de módulos deve emergir dessa decomposição.' : `Erros encontrados: ${diagnostic.message}`
      let repaired: AIResponse
      try { repaired = await provider.sendMessage({ messages: [{ role: 'system', content: `Faça a única correção e retorne JSON compacto para ${schemaDescription()}. Preserve assunto, cobertura, keys, Concepts, AssessmentIntents e autoridade curricular. Decomponha por progressão quando houver compressão; não derive domínio de declarações. Sem explicações ou redundância.` }, { role: 'user', content: JSON.stringify({ subject: workspace.name, reason: repairReason, invalidResponse: response.content.slice(0, 14_000), allowedSourceIds: [...allowed.keys()].slice(0, 4) }) }], maxOutputTokens: ROADMAP_REPAIR_MAX_OUTPUT_TOKENS, responseFormat: 'json_object', signal }) } catch (error) { throw diagnosticError(error, 'provider_response') }
      try { proposal = this.parseProposal(workspaceId, repaired.content, allowed, setupObjective); const scopeIssue = roadmapScopeIssue(proposal, curricularScope); if (scopeIssue) throw new LearningPathGenerationError('ROADMAP_SCOPE_MISALIGNED', 'generic_validation', scopeIssue, { response: repaired.content }); if (roadmapIsObviouslyTruncated(proposal, workspace, curricularScope)) throw new LearningPathGenerationError('ROADMAP_OBVIOUSLY_TRUNCATED', 'generic_validation', 'A resposta reparada ainda termina antes do escopo solicitado.', { response: repaired.content }) } catch (repairError) { const repairedDiagnostic = diagnosticError(repairError, 'extract_json'); throw new LearningPathGenerationError(repairedDiagnostic.code, repairedDiagnostic.stage, repairedDiagnostic.message, { cause: repairedDiagnostic.cause, response: repaired.content }) }
      response = repaired
    }
  ...
  private parseProposal(workspaceId: string, content: string, allowed: Map<string, RoadmapResource>, setupObjective: boolean): ReturnType<typeof roadmapProposalSchema.parse> {
    ...
    try { expanded = normalizeGeneratedRoadmapProposal(raw) } catch (error) { throw new LearningPathGenerationError('ROADMAP_SCHEMA_INVALID', 'generated_schema', structuredErrorDetail(error), { cause: error, response: content }) }
    const generated = generatedRoadmapProposalSchema.safeParse(expanded)
    ...
    if (hasCompressedModules(proposal.data)) throw new LearningPathGenerationError('ROADMAP_COMPRESSED', 'generic_validation', 'Roadmap contains an overloaded module that must be decomposed into pedagogically navigable stages', { response: content })

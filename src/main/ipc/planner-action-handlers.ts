...
export function registerPlannerActionHandlers(service: PlannerActionService): void { ipcMain.handle(PLANNER_ACTION_CHANNELS.listPending, (event) => { assertTrustedSender(event); return service.listPending() }); ipcMain.handle(PLANNER_ACTION_CHANNELS.proposeFromText, (event, content: unknown) => { assertTrustedSender(event); if (typeof content !== 'string' || content.length > 4000) throw new Error('Invalid planner text'); return service.proposeFromText(content) }); ipcMain.handle(PLANNER_ACTION_CHANNELS.resolve, (event, payload: unknown) => { assertTrustedSender(event); const input = resolvePlannerActionInputSchema.parse(payload); return service.resolve(input.actionId, input.decision) }) }

...
import { CONTENT_GENERATOR_VERSIONS, contentJobKey } from '../../src/application/workspaces/workspace-content-repository'
...
describe('SqliteWorkspaceContentRepository', () => {
  it('obsoletes queued projections and rejects provisioning for an archived workspace', () => {
    const db = database(); const id = workspace(db); const repository = new SqliteWorkspaceContentRepository(db)
    const revision = repository.createRevision({ workspaceId: id, inputHash: hash('a'), now: 1 })
    repository.enqueue({ workspaceId: id, revision: revision.revision, kind: 'roadmap_generate', unitKey: 'roadmap', priority: 900, inputHash: revision.inputHash, generatorContractVersion: CONTENT_GENERATOR_VERSIONS.roadmap_generate }, 2)
    db.sqlite.prepare("UPDATE workspaces SET status='archived',archived_at=3 WHERE id=?").run(id)
    expect(repository.invalidateArchivedWorkspace(id, 3)).toBe(1)
    expect(db.sqlite.prepare('SELECT status FROM content_jobs WHERE workspace_id=?').get(id)).toEqual({ status: 'obsolete' })
    new InitialProvisioningCoordinator(db, repository, () => undefined, () => 4).initialize(id)
    expect(repository.claimNext({ owner: 'worker', now: 5 })).toBeNull()
  })
  it('coalesces coordinator, startup reconciliation, and selection onto one immutable job identity', () => {
    const db = database(); const workspaceId = workspace(db); const repository = new SqliteWorkspaceContentRepository(db)
    repository.createRevision({ workspaceId, inputHash: hash('a'), now: 1 })
    const coordinator = new InitialProvisioningCoordinator(db, repository, () => undefined, () => 2)
    coordinator.initialize(workspaceId); coordinator.initialize(workspaceId)
    const revision = repository.getRevision(workspaceId)!
    const roadmap = roadmapSchema.parse({ id: 'roadmap', workspaceId, title: 'C', status: 'accepted', generationKind: 'ai_generated', version: 1, providerId: null, modelId: null, modules: [{ id: 'm', title: 'M', objective: 'O', estimatedMinutes: 30, position: 1, status: 'active', topics: ['one'], outcomes: ['O'], practice: 'P', completionCriteria: ['C'], resources: [] }], createdAt: 1, updatedAt: 1 })
    db.sqlite.prepare("INSERT INTO roadmaps (id,workspace_id,title,status,generation_kind,version,content_revision,content_hash,created_at,updated_at) VALUES (?,?,?,'accepted','ai_generated',1,?,?,1,1)").run(roadmap.id, workspaceId, roadmap.title, revision.revision, revision.inputHash)
    db.sqlite.prepare("INSERT INTO roadmap_modules (id,roadmap_id,title,objective,estimated_minutes,position,status,topics_json,outcomes_json,practice,completion_criteria_json,resources_json) VALUES ('m',?,'M','O',30,1,'active','[\"one\"]','[\"O\"]','P','[\"C\"]','[]')").run(roadmap.id)
    repository.adoptRoadmapRevision({ workspaceId, roadmapId: roadmap.id, inputHash: revision.inputHash, now: 3 })
    const scheduler = new PedagogicalPrefetchScheduler({ repository, getRoadmap: () => roadmap, now: () => 4 })
    scheduler.schedule({ type: 'topic_selected', workspaceId, topicId: 'm:one' }); scheduler.schedule({ type: 'topic_opened', workspaceId, topicId: 'm:one' })
    const jobs = db.sqlite.prepare("SELECT idempotency_key AS idempotencyKey FROM content_jobs WHERE workspace_id=? AND revision=? AND kind='lesson_generate' AND unit_key='m:one'").all(workspaceId, revision.revision) as Array<{ idempotencyKey: string }>
    expect(jobs).toHaveLength(1)
    expect(jobs[0]?.idempotencyKey).toBe(contentJobKey({ workspaceId, revision: revision.revision, kind: 'lesson_generate', unitKey: 'm:one', inputHash: revision.inputHash, generatorContractVersion: CONTENT_GENERATOR_VERSIONS.lesson_generate }))
    db.close()
  })

  it('rejects noncanonical immutable dependencies instead of weakening stale-write protection', () => {
    const db = database(); const workspaceId = workspace(db); const repository = new SqliteWorkspaceContentRepository(db)
    const revision = repository.createRevision({ workspaceId, inputHash: hash('a'), now: 1 })
    expect(() => repository.enqueue({ workspaceId, revision: revision.revision, kind: 'exercise_generate', unitKey: 'm:one', priority: 1, inputHash: revision.inputHash, generatorContractVersion: CONTENT_GENERATOR_VERSIONS.exercise_generate, dependencyKeys: ['wrong'] }, 2)).toThrow(/deterministic unit contract/)
    db.close()
  })

  it('keeps prior generator contracts immutable while allowing deterministic legacy reconciliation', () => {
    const db = database(); const workspaceId = workspace(db); const repository = new SqliteWorkspaceContentRepository(db)
    const revision = repository.createRevision({ workspaceId, inputHash: hash('a'), now: 1 })
    const legacyDependency = hash('b')
    const legacy = repository.enqueue({ workspaceId, revision: revision.revision, kind: 'exercise_generate', unitKey: 'm:one', priority: 1, inputHash: revision.inputHash, generatorContractVersion: 'exercise-v0', dependencyKeys: [legacyDependency] }, 2)
    const reconciled = repository.enqueue({ workspaceId, revision: revision.revision, kind: 'exercise_generate', unitKey: 'm:one', priority: 9, inputHash: revision.inputHash, generatorContractVersion: 'exercise-v0', dependencyKeys: [legacyDependency] }, 3)
    expect(reconciled).toMatchObject({ id: legacy.id, priority: 9, dependencyKeys: [legacyDependency] })
    expect(() => repository.enqueue({ workspaceId, revision: revision.revision, kind: 'exercise_generate', unitKey: 'm:one', priority: 10, inputHash: revision.inputHash, generatorContractVersion: 'exercise-v0' }, 4)).toThrow(/immutable inputs/)
    db.close()
  })
  ...
  it('keeps rebuild mutation authoritative through callback, repeated initialize, and later material change', async () => {
    ...
    const replacementModuleId = crypto.randomUUID(); const preview = roadmapRebuildPreviewSchema.parse({ id: crypto.randomUUID(), workspaceId: id, currentRoadmapId: roadmapA.id, title: 'B', modules: [{ ...roadmapA.modules[0], id: replacementModuleId, title: 'B', topics: ['B'] }], materialIds: [crypto.randomUUID()], impact: { preservedModuleIds: [], preservedTopicIds: [], addedTopics: [`${replacementModuleId}:B`], removedTopics: [`${moduleA}:A`], unsafeProgressTopicIds: [], requiresAcknowledgement: true }, status: 'pending', appliedRoadmapId: null, createdAt: now, resolvedAt: null }); roadmaps.saveRebuildPreview(preview)
    const atomicRoadmaps = new DrizzleRoadmapRepository(db, (roadmap) => coordinator.adoptApprovedRoadmap(roadmap)); const service = new RoadmapService(atomicRoadmaps, new AIProviderManager(), async () => ({ id, name: 'C', objective: 'Ponteiros', status: 'active', createdAt: 1, updatedAt: 1, lastOpenedAt: null, archivedAt: null })); service.setRoadmapChangedHandler((roadmap, atomicallyAdopted) => { if (atomicallyAdopted) coordinator.resumeApprovedRoadmap(roadmap.workspaceId) }); const roadmapB = await service.applyRebuild({ workspaceId: id, previewId: preview.id, acknowledgeUnsafeChanges: true }); const second = content.getRevision(id)!
  ...
  it('runs material-first provisioning to USABLE, repairs an empty plan, and leaves N+1 queued', async () => {
    ...
    const handlers: Partial<Record<import('../../src/shared/contracts/workspace-content-contract').ContentUnitKind, ContentJobHandler>> = {
      ...
      plan_recalculate: async () => { order.push('plan'); return { publish: () => { db.sqlite.prepare("INSERT INTO weekly_plans (id,week_start,timezone,revision,generated_at,updated_at) VALUES ('plan',date(?,'weekday 1','-7 days'),'UTC',1,?,?)").run(new Date(now).toISOString(), now, now); const parts = new Intl.DateTimeFormat('en-CA', { timeZone: 'UTC', year: 'numeric', month: '2-digit', day: '2-digit' }).formatToParts(now); const value = (type: Intl.DateTimeFormatPartTypes) => parts.find((item) => item.type === type)?.value ?? ''; const dateKey = `${value('year')}-${value('month')}-${value('day')}`; db.sqlite.prepare("INSERT INTO weekly_plan_items (id,plan_id,workspace_id,source_key,date_key,title,duration_minutes,position,status,module_id,topic_id,activity_type,scheduled_start_minutes,reason,created_at,updated_at) VALUES ('item','plan',?,'first',?,'Ponteiros',30,1,'pending','module','module:Ponteiros','introduction',600,'Primeiro',?,?)").run(id, dateKey, now, now) } } },
    }
    const worker = new ContentGenerationWorker({ repository, admission: new HeavyGenerationQueue(), handlers, pollMs: 5, onPublished: (job) => { publishedJobs.push(`${job.kind}:${job.unitKey}`); coordinator.onPublished(job) } }); coordinator = new InitialProvisioningCoordinator(db, repository, () => worker.wake(), Date.now, new PerformanceTimelineStore(db))
    ...
    expect(roadmapResponses).toBe(2)
    expect(publishedJobs.slice(0, 4)).toEqual(['roadmap_generate:roadmap', 'lesson_generate:module:Ponteiros', 'exercise_generate:module:Ponteiros', 'plan_recalculate:current-week'])
  ...
  it('converges to FULLY only when every required output is valid and keeps it after restart', () => {
    const db = database(); const workspaceId = workspace(db); const repository = new SqliteWorkspaceContentRepository(db); const now = Date.now(); const revision = 1; const inputHash = hash('f'); const moduleId = 'module'; const firstTopicId = `${moduleId}:Pointers`; const lessonId = `${firstTopicId}:lesson`
    repository.createRevision({ workspaceId, inputHash, now }); db.sqlite.prepare("INSERT INTO roadmaps (id,workspace_id,title,status,generation_kind,version,content_revision,content_hash,created_at,updated_at) VALUES ('roadmap',?,'C','accepted','ai_generated',1,?,?,?,?)").run(workspaceId, revision, inputHash, now, now); db.sqlite.prepare("INSERT INTO roadmap_modules (id,roadmap_id,title,objective,estimated_minutes,position,status,topics_json,outcomes_json,practice,completion_criteria_json,resources_json) VALUES (?,'roadmap','C','Learn',60,1,'active','[\"Pointers\"]','[\"Use pointers\"]','Practice','[\"Complete\"]','[]')").run(moduleId)
    const lesson = { title: 'Pointers', level: 'basic', objective: 'Use pointers', sources: [], blocks: [{ id: 'a', type: 'explanation', title: 'A', content: 'Pointers have observable address semantics.' }, { id: 'b', type: 'explanation', title: 'B', content: 'Dereference accesses the pointed value.' }, { id: 'c', type: 'explanation', title: 'C', content: 'Pointer types constrain valid access.' }, { id: 'd', type: 'checkpoint', title: 'D', questionType: 'multiple_choice', question: 'What is safe?', options: Array.from({ length: 5 }, (_, index) => ({ id: `o${index}`, text: `Option ${index}`, rationale: `Reason ${index}` })), correctOptionId: 'o0', reasoningRequirement: 'none', hint: 'Think', reinforcement: 'Review' }] }; db.sqlite.prepare("INSERT INTO study_lessons (id,workspace_id,roadmap_id,module_id,topic_id,generation_kind,content_revision,input_hash,content_json,created_at,updated_at) VALUES (?,?, 'roadmap',?,?,'ai_generated',?,?,?, ?,?)").run(lessonId, workspaceId, moduleId, firstTopicId, revision, inputHash, JSON.stringify(lesson), now, now)
    db.sqlite.prepare("INSERT INTO exercise_sets (id,workspace_id,roadmap_id,module_id,topic_id,lesson_id,status,content_revision,input_hash,created_at,updated_at) VALUES ('set',?,'roadmap',?,?,?,'ready',?,?,?,?)").run(workspaceId, moduleId, firstTopicId, lessonId, revision, inputHash, now, now); db.sqlite.prepare("INSERT INTO exercises (id,set_id,position,kind,difficulty,title,statement,input_description,output_description,language,starter_code,required_for_topic_completion,public_tests_json,private_tests_json,reference_solution,hint,created_at) VALUES ('exercise','set',1,'PROGRAMMING_PROBLEM','introductory','P','S','','','c','int main(){}',1,?,?,?,'H',?)").run(JSON.stringify([{ id: 'p', input: '', expectedOutput: '' }]), JSON.stringify(Array.from({ length: 3 }, (_, i) => ({ id: `h${i}`, input: '', expectedOutput: '' }))), 'int main(){}', now); db.sqlite.prepare("INSERT INTO weekly_plans (id,week_start,timezone,revision,generated_at,updated_at) VALUES ('plan','2026-09-14','UTC',1,?,?)").run(now, now); db.sqlite.prepare("INSERT INTO weekly_plan_items (id,plan_id,workspace_id,source_key,date_key,title,duration_minutes,position,status,module_id,topic_id,activity_type,scheduled_start_minutes,reason,created_at,updated_at) VALUES ('item','plan',?,'first','2026-09-14','Pointers',30,1,'pending',?,?,'introduction',600,'First',?,?)").run(workspaceId, moduleId, firstTopicId, now, now)
    const units = [{ kind: 'roadmap_generate' as const, unitKey: 'roadmap', inputHash }, { kind: 'lesson_generate' as const, unitKey: firstTopicId, inputHash }, { kind: 'exercise_generate' as const, unitKey: firstTopicId, inputHash }, { kind: 'plan_recalculate' as const, unitKey: 'current-week', inputHash }]; repository.replaceRequiredUnits(workspaceId, revision, units, now); for (const unit of units) { const queued = repository.enqueue({ workspaceId, revision, kind: unit.kind, unitKey: unit.unitKey, inputHash, generatorContractVersion: CONTENT_GENERATOR_VERSIONS[unit.kind], priority: 1, dependencyKeys: unit.kind === 'lesson_generate' ? [contentJobKey({ workspaceId, revision, kind: 'roadmap_generate', unitKey: 'roadmap', inputHash, generatorContractVersion: CONTENT_GENERATOR_VERSIONS.roadmap_generate })] : unit.kind === 'exercise_generate' ? [contentJobKey({ workspaceId, revision, kind: 'lesson_generate', unitKey: firstTopicId, inputHash, generatorContractVersion: CONTENT_GENERATOR_VERSIONS.lesson_generate })] : [], availableAt: now }, now); db.sqlite.prepare("UPDATE content_jobs SET status='ready' WHERE id=?").run(queued.id) }
    const required = repository.listRequiredUnits(workspaceId, revision)
    expect(required.length).toBeGreaterThan(0)
    expect(repository.evaluateReadiness({ workspaceId, expectedRevision: revision, todayDateKey: '2026-09-14', now }).state).toBe('FULLY_PROVISIONED')
    const requiredJob = db.sqlite.prepare("SELECT id FROM content_jobs WHERE workspace_id=? AND revision=? AND unit_key=?").get(workspaceId, revision, firstTopicId) as { id: string }
    db.sqlite.prepare("UPDATE content_jobs SET status='failed',last_error_code='GENERATION_FAILED' WHERE id=?").run(requiredJob.id)
    expect(repository.evaluateReadiness({ workspaceId, expectedRevision: revision, todayDateKey: '2026-09-14', now: now + 1 }).state).toBe('PROVISIONING')
    db.sqlite.prepare("UPDATE content_jobs SET status='ready',last_error_code=NULL WHERE id=?").run(requiredJob.id)
    const optional = repository.enqueue({ workspaceId, revision, kind: 'lesson_generate', unitKey: 'optional-topic', inputHash, generatorContractVersion: CONTENT_GENERATOR_VERSIONS.lesson_generate, priority: 1, dependencyKeys: [contentJobKey({ workspaceId, revision, kind: 'roadmap_generate', unitKey: 'roadmap', inputHash, generatorContractVersion: CONTENT_GENERATOR_VERSIONS.roadmap_generate })], availableAt: now }, now); db.sqlite.prepare("UPDATE content_jobs SET status='failed',last_error_code='GENERATION_FAILED' WHERE id=?").run(optional.id)
    expect(repository.evaluateReadiness({ workspaceId, expectedRevision: revision, todayDateKey: '2026-09-14', now: now + 2 }).state).toBe('FULLY_PROVISIONED')
    const databasePath = db.sqlite.name; db.close()
    const reopened = openCoachDatabase({ databasePath, migrationsFolder: resolve('drizzle/migrations') })
    expect(new SqliteWorkspaceContentRepository(reopened).getRevision(workspaceId)?.state).toBe('FULLY_PROVISIONED')
    reopened.close()
  })

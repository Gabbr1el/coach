...
export function ProjectWorkspace({ workspaceId, workspaceName, onError }: { workspaceId: string; workspaceName: string; onError(message: string): void }) {
  ...
  function open(fileId: string): void { if (project) void window.coach.project.openFile({ projectId: project.id, fileId }).then((next) => apply(next, fileId)).catch(() => onError('Não foi possível abrir o arquivo.')) }

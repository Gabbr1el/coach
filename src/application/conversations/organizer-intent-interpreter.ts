...
function eventEntities(content: string) {
  ...
  const subject = /(?:prova|exame|trabalho|atividade|prazo|deadline)\s+(?:de|da|do)\s+(.+?)(?=\s+(?:foi\s+|mudou\s+|passou\s+|ao\s+workspace\b|do\s+workspace\b|sem\s+workspace\b|no\s+dia|dia|em\s+\d|para\s+(?:o\s+)?dia|amanh[ãa]|hoje|depois\s+de\s+amanh[ãa]|na\s+(?:pr[oó]xima\s+)?(?:segunda|terça|quarta|quinta|sexta|sábado|domingo)|daqui\s+a)\b|[,.;]|$)/i.exec(content)?.[1]?.trim() ?? null

...
export function App() {
  ...
  if (selected) {
    ...
    const selectPage = (page: WorkspacePage) => { setStudyModule(null); if (page === 'materials') openMaterials(); else if (page === 'reports') openReports(); else setWorkspacePage(page) }
    const openStudyModule = (moduleId: string) => { void window.coach.roadmap.get(selected.id).then((roadmap) => { if (!roadmap) return; setStudyRoadmap(roadmap); setStudyModule(roadmap.modules.find((item) => item.id === moduleId) ?? null); setWorkspacePage('overview') }).catch(() => setWorkspaceError('Não foi possível abrir esta etapa de estudo.')) }
    return (

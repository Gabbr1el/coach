...
describe('PlanningService', () => {
  ...
  it.each([7, 31, 183, 365, 730])('reads a completed future frontier at +%i days without materializing a plan', (days) => { const now = Date.parse('2026-09-07T12:00:00Z'); let saves = 0; let planningReads = 0; const repository = new Repository() as Repository & PlanningRepository; repository.getCanonicalPlanningTimezone = () => 'UTC'; repository.findWeeklyPlan = () => null; repository.listWorkspaces = () => []; repository.listWeeklyAvailability = () => []; repository.listTodayBudgets = () => []; repository.listWeeklyPlanningTopics = () => { planningReads += 1; return [] }; repository.saveWeeklyPlan = () => { saves += 1 }; const anchor = new Date(now + days * 86_400_000).toISOString().slice(0, 10); const plan = new PlanningService(repository, () => now).getWeeklyPlanWindow(anchor, 'UTC'); expect(plan.days.flatMap((day) => day.items)).toEqual([]); expect(saves).toBe(0); expect(planningReads).toBe(0) })
})

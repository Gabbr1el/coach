...
describe('RoadmapService', () => { it('builds and accepts a detailed offline Java roadmap', async () => { const repository = new MemoryRoadmaps(); const service = new RoadmapService(repository, new AIProviderManager(), async () => workspace, () => 10); const roadmap = await service.generate(workspace.id); expect(roadmap.modules.map((module) => module.title)).toContain('Classes, objetos e encapsulamento'); expect(roadmap.modules.every((module) => module.topics.length >= 2 && module.practice && module.completionCriteria.length)).toBe(true); expect(roadmap.modules[0]?.resources[0]?.url).toBe('https://roadmap.sh/java'); expect((await service.accept(workspace.id, roadmap.id)).status).toBe('accepted') }) })

...
export class WorkspaceOnboardingService {
  private readonly analyses = new Map<string, { revision: number; subject: string; focus: string; context: string }>()
  ...
  validate(token: string, revision: number, subject: string, focus = '', context = ''): boolean { const value = this.analyses.get(token); return Boolean(value && value.revision === revision && value.subject === normalizeSubject(subject).subject && value.focus === focus && value.context === context) }
}

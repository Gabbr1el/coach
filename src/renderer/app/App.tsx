...
function lessonFor(topic: string, module: RoadmapModule) {
  if (/sintaxe|syntax/i.test(topic)) return { lead: 'Um erro de sintaxe acontece quando o código não segue a estrutura que Python consegue interpretar.', example: 'print("Olá"', explanation: 'O parêntese foi aberto e não foi fechado. O interpretador interrompe antes de executar o programa e indica SyntaxError.', question: 'Qual opção contém um erro de sintaxe?', options: ['print("Olá")', 'idade = 20', 'print("Olá"', 'nome = input("Nome: ")'], correct: 2, reinforcement: 'Observe pares que precisam fechar: aspas, parênteses, colchetes e chaves. Confira a estrutura antes de pensar no resultado.' }
  if (/interpretador|arquivo \.py/i.test(topic)) return { lead: 'O interpretador Python lê as instruções de um arquivo .py e as executa em ordem.', example: 'idade = 20\nprint(idade)', explanation: 'A primeira linha guarda um valor. A segunda produz saída. Executar um arquivo não significa que ele sempre mostrará algo.', question: 'Qual arquivo executa sem produzir saída?', options: ['idade = 20', 'print(20)', 'print("20")'], correct: 0, reinforcement: 'Uma atribuição é executada, mas somente uma operação de saída torna o resultado visível no terminal.' }
  return { lead: module.objective, example: module.practice, explanation: module.outcomes.join(' · '), question: `Qual alternativa melhor demonstra ${topic}?`, options: [module.completionCriteria[0] ?? 'Aplicar o conceito corretamente', 'Somente abrir o tópico', 'Ignorar o resultado'], correct: 0, reinforcement: `Revise o objetivo: ${module.objective}` }
}
...
function StudyModuleView({ module, roadmap, onBack, onAskCoach, onPractice }: { module: RoadmapModule; roadmap: Roadmap | null; onBack(): void; onAskCoach(prompt: string): void; onPractice(): void }) {
  const [topicIndex, setTopicIndex] = useState(0); const [selected, setSelected] = useState<number | null>(null); const topic = module.topics[topicIndex] ?? module.title; const lesson = lessonFor(topic, module); const completed = roadmap?.modules.filter((item) => item.status === 'completed').length ?? 0; const progress = Math.round((completed + topicIndex / Math.max(1, module.topics.length)) / Math.max(1, roadmap?.modules.length ?? 1) * 100)
  const choose = (index: number) => { setSelected(index); onAskCoach(`No checkpoint de “${topic}”, escolhi: ${lesson.options[index]}. ${index === lesson.correct ? 'Valide meu entendimento e ofereça continuar.' : 'Explique o equívoco e ofereça opções de reforço.'}`) }
  return <div className="grid h-full min-h-0 grid-cols-[210px_minmax(0,1fr)] bg-[#0b0c10]"><aside className="coach-scroll-pane border-r border-coach-line bg-[#0e0f13] p-4"><button onClick={onBack} className="text-xs font-black text-coach-muted">← Voltar ao plano</button><p className="mt-6 text-[10px] font-black uppercase tracking-wider text-coach-muted">Roteiro</p><p className="mt-1 text-xs text-coach-green">{progress}% concluído</p><div className="mt-2 h-1 rounded bg-coach-line"><div className="h-full rounded bg-coach-green" style={{ width: `${progress}%` }} /></div><div className="mt-5 space-y-2">{roadmap?.modules.map((item) => <div key={item.id}><button className={`w-full rounded-lg px-3 py-2 text-left text-xs font-black ${item.id === module.id ? 'bg-[#1a1c22] text-white' : 'text-coach-muted'}`}>{item.position}. {item.title}</button>{item.id === module.id && <div className="ml-3 mt-1 border-l border-coach-line pl-3">{item.topics.map((entry, index) => <button key={entry} onClick={() => { setTopicIndex(index); setSelected(null) }} className={`block w-full py-1.5 text-left text-[11px] ${index === topicIndex ? 'font-black text-coach-orange' : 'text-coach-muted'}`}>{index === topicIndex ? '→' : '○'} {entry}</button>)}</div>}</div>)}</div></aside><article className="coach-scroll-pane h-full px-6 py-7 lg:px-12"><div className="mx-auto max-w-4xl"><div className="flex items-center justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-coach-green">Próximo passo escolhido pelo Coach</p><h2 className="mt-2 font-display text-3xl font-black">{topic}</h2></div><span className="rounded-full border border-coach-line px-3 py-2 text-[10px] font-black text-coach-muted">ESTUDO · {Math.max(3, Math.round(module.estimatedMinutes / Math.max(1, module.topics.length)))} MIN</span></div><section className="mt-8 rounded-xl border border-coach-line bg-[#111217] p-6"><h3 className="font-display text-xl font-black">O que é?</h3><p className="mt-3 text-sm leading-7 text-coach-muted">{lesson.lead}</p><pre className="mt-5 overflow-auto rounded-lg bg-[#090a0d] p-4 text-sm text-coach-green"><code>{lesson.example}</code></pre><p className="mt-4 text-sm leading-7 text-coach-muted">{lesson.explanation}</p></section><section className="mt-5 rounded-xl border border-coach-line bg-[#111217] p-6"><p className="text-[10px] font-black uppercase tracking-wider text-coach-orange">Antes de continuar…</p><h3 className="mt-2 font-display text-lg font-black">{lesson.question}</h3><div className="mt-4 grid gap-2">{lesson.options.map((option, index) => <button key={option} onClick={() => choose(index)} className={`rounded-lg border p-3 text-left text-sm ${selected === index ? 'border-coach-orange bg-coach-orange/10' : 'border-coach-line bg-[#0d0e12]'}`}><span className="mr-2 font-black text-coach-muted">{String.fromCharCode(65 + index)}.</span>{option}</button>)}</div>{selected !== null && selected !== lesson.correct && <div className="mt-4 rounded-lg border border-coach-orange/40 bg-coach-orange/10 p-4 text-sm text-coach-muted"><strong className="text-coach-orange">Vamos reforçar antes de avançar.</strong><p className="mt-2">{lesson.reinforcement}</p></div>}<div className="mt-4 flex flex-wrap gap-2"><button onClick={() => onAskCoach(`Dê outro exemplo de “${topic}” adaptado ao meu contexto atual.`)} className="rounded-lg border border-coach-line px-4 py-2 text-xs font-black">Ver outro exemplo</button>{selected === lesson.correct && topicIndex < module.topics.length - 1 && <button onClick={() => { setTopicIndex((value) => value + 1); setSelected(null) }} className="rounded-lg bg-coach-green px-4 py-2 text-xs font-black text-[#0c0d10]">Continuar</button>}<button onClick={onPractice} className="rounded-lg border border-coach-orange px-4 py-2 text-xs font-black text-coach-orange">Praticar {topic}</button></div></section></div></article></div>
}
  ...
  if (selected) {
    ...
    return (
      <WorkspaceShell name={selected.name} objective={selected.objective} page={workspacePage} timerLabel={timerLabel} timerRunning={studyState?.timerStatus === 'running'} finishing={sessionCompleting} shareContext={studyState?.shareContextWithAi ?? false} coachMessages={workspaceMessages} streamedMessage={workspaceStreamedContent} coachInput={workspaceInput} coachBusy={workspaceSending || workspaceLoading} coachError={workspaceError} conversationRef={workspaceConversationPane} messageEndRef={workspaceMessageEnd} onPage={selectPage} onHome={() => { setSelected(null); setHomeSection('home'); void window.coach.planning.listPriorities().then(setPriorities) }} onSettings={() => setProviderDialogOpen(true)} onToggleTimer={() => { if (!studyState || timerUpdating) return; const workspaceId = selected.id; const epoch = workspaceLoadEpoch.current; const action = studyState.timerStatus === 'running' ? 'pause' : 'start'; setTimerUpdating(true); const configure = action === 'start' && studyState.timerStatus === 'idle' && studyState.timerDurationSeconds !== adaptiveMinutes * 60 ? window.coach.studyWorkspace.setTimerDuration({ workspaceId, durationSeconds: adaptiveMinutes * 60 }) : Promise.resolve(studyState); void configure.then(() => window.coach.studyWorkspace.updateTimer({ workspaceId, action })).then(applyStudyState(workspaceId, epoch)).catch(() => setWorkspaceError('Não foi possível atualizar o foco.')).finally(() => setTimerUpdating(false)) }} onFinish={() => { if (sessionCompleting) return; setSessionCompleting(true); void window.coach.studyWorkspace.completeSession(selected.id).then(setStudyState).finally(() => setSessionCompleting(false)) }} onShareContext={(enabled) => { void window.coach.studyWorkspace.updateContextSharing({ workspaceId: selected.id, enabled }).then(setStudyState).catch(() => setWorkspaceError('Não foi possível atualizar o compartilhamento de contexto.')) }} onCoachInput={setWorkspaceInput} onCoachSend={sendWorkspaceMessage} onNotes={() => setNotesOpen(true)}>
        {workspacePage === 'studies' && livePriority?.eventPhase === 'today' && <div className="border-b border-coach-orange/30 bg-coach-orange/10 px-6 py-3 text-xs text-coach-muted"><strong className="text-coach-orange">Revisão para a prova hoje.</strong> Priorizando tópicos atuais, erros recorrentes e checkpoints curtos; evite conteúdo pesado novo.</div>}{workspacePage === 'studies' && studyModule && <StudyModuleView module={studyModule} roadmap={studyRoadmap} onBack={() => setWorkspacePage('plan')} onAskCoach={(prompt) => setWorkspaceInput(prompt)} onPractice={() => setWorkspacePage('practice')} />}

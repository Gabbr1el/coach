...
import type { PriorSubjectMemory } from '../../shared/contracts/workspace-contract'
...
export class WorkspaceOnboardingService {
  ...
  constructor(private readonly dependencies: { repository: ConversationRepository; providerManager: AIProviderManager; academicContext?: AcademicSubjectContextService; listObservedLearning?: (subject: string) => Promise<readonly string[]> | readonly string[]; listContextCandidates?: (subject: string) => Promise<readonly WorkspaceOnboardingContextCandidate[]> | readonly WorkspaceOnboardingContextCandidate[]; listPriorSubjectMemory?: (subject: string, limit: number) => Promise<readonly PriorSubjectMemory[]> | readonly PriorSubjectMemory[]; providerTimeoutMs?: number }) {}

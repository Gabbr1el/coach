...
export function inferPlannerActions(content: string): Array<{ type: PlannerActionType; payload: Record<string, unknown> }> {
  ...
  const explicitWorkspace = /(?:crie|criar|cria|monte|montar|abra|abrir)(?:\s+(?:um|uma))?\s+workspace(?:\s+(?:de|para|sobre))?\s+(.+)/i.exec(text)?.[1]
  const study = (explicitWorkspace ?? /(?:quero|preciso|vou)\s+(?:estudar|aprender|praticar)\s+(.+)/i.exec(text)?.[1])?.replace(/[.!?]+$/, '').trim()

...
import { interactiveCodeStateSchema, type InteractiveCodeBlock } from '../../shared/contracts/code-execution-contract'
  ...
  ipcMain.handle(CONVERSATION_CHANNELS.streamWorkspaceMessage, async (event, payload: unknown) => {
    ...
    if (input.activeInteractiveCode) {
      ...
      if (!database || !input.activeStudy || progress?.lessonId !== input.activeStudy.lessonId || input.activeInteractiveCode.lessonId !== input.activeStudy.lessonId || input.activeInteractiveCode.blockId !== input.activeStudy.currentBlockId) input = { ...input, activeInteractiveCode: undefined }
        ...
        const row = database.sqlite.prepare('SELECT l.content_json AS contentJson, s.current_code AS currentCode, s.prediction, s.attempts, s.last_execution_json AS lastExecutionJson, s.validation_result_json AS validationResultJson, s.updated_at AS updatedAt FROM study_lessons l JOIN study_interactive_code_states s ON s.lesson_id = l.id AND s.workspace_id = l.workspace_id WHERE l.id = ? AND l.workspace_id = ? AND s.block_id = ?').get(input.activeStudy.lessonId, input.workspaceId, input.activeInteractiveCode.blockId) as { contentJson: string; currentCode: string; prediction: string | null; attempts: number; lastExecutionJson: string | null; validationResultJson: string | null; updatedAt: number } | undefined
        ...
        if (!row || !block) input = { ...input, activeInteractiveCode: undefined }
          ...
          const state = interactiveCodeStateSchema.parse({ lessonId: input.activeStudy.lessonId, blockId: block.id, currentCode: row.currentCode, prediction: row.prediction, attempts: row.attempts, lastExecution: row.lastExecutionJson ? JSON.parse(row.lastExecutionJson) : null, validationResult: row.validationResultJson ? JSON.parse(row.validationResultJson) : null, updatedAt: row.updatedAt })

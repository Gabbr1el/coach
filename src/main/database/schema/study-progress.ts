...
export const checkpointReasoningEvidence = sqliteTable('checkpoint_reasoning_evidence', {
  answerId: text('answer_id').primaryKey(), workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }), topicId: text('topic_id').notNull(), lessonId: text('lesson_id').notNull(), checkpointId: text('checkpoint_id').notNull(), alternativeCorrect: integer('alternative_correct', { mode: 'boolean' }).notNull(), reasoningStatus: text('reasoning_status', { enum: ['coherent', 'partial', 'misconception', 'insufficient', 'off_topic', 'reasoning_evaluation_pending'] }).notNull(), summary: text('summary'), misconception: text('misconception'), feedback: text('feedback'), evaluatedAt: integer('evaluated_at'), retryCount: integer('retry_count').notNull().default(0), nextRetryAt: integer('next_retry_at'), createdAt: integer('created_at').notNull(), updatedAt: integer('updated_at').notNull(),
}, (table) => [index('checkpoint_reasoning_workspace_topic_idx').on(table.workspaceId, table.topicId, table.updatedAt), index('checkpoint_reasoning_pending_retry_idx').on(table.reasoningStatus, table.nextRetryAt)])

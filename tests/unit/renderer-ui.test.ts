...
describe('renderer UI safeguards', () => {
  ...
  it('keeps timer expiry separate from learning and offers explicit actions', () => { const app = read('src/renderer/app/App.tsx'); expect(app).toContain('O tempo terminou. A aprendizagem continua sob seu controle.'); expect(app).toContain("action: 'extend'"); expect(app).toContain('Concluir atividade'); expect(app).toContain('completePlanItem'); expect(app.match(/setTimerExpired\(true\)/g)).toHaveLength(1) })
})

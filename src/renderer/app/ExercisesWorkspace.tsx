...
import type { ExerciseAdaptation, ExerciseExecution, ExerciseSet, ExerciseSetProjection } from '../../shared/contracts/exercise-contract'
...
import { stableEditorPath } from './editor-reconciliation'
...
export function ExercisesWorkspace({ workspaceId, roadmap, initialTopicId, expectedSetId, progressState, onBack, onContext }: {
  workspaceId: string
  roadmap: Roadmap
  initialTopicId: string
  expectedSetId?: string | null
  progressState: StudyProgressState | null
  onBack(): void
  ...
  const [setProjection, setSetProjection] = useState<Record<string, ExerciseSetProjection | undefined>>({})
  ...
  const [adaptations, setAdaptations] = useState<ExerciseAdaptation[]>([])
  ...
  const curriculum = useMemo(() => projectCurriculum(roadmap, progressState, setProjection), [roadmap, progressState, setProjection])

  useEffect(() => {
    let current = true
    void window.coach.exercise.projectSets({ workspaceId, roadmapId: roadmap.id }).then((entries) => { if (current) setSetProjection(Object.fromEntries(entries.map((item) => [item.topicId, item]))) })
  ...
  useEffect(() => {
    ...
    void window.coach.exercise.getSet({ workspaceId, topicId }).then((next) => {
      ...
      if (first) { setCode(next.progress.find((item) => item.exerciseId === first.id)?.currentCode ?? first.starterCode); setPrediction(''); submitIdentity.current = null; void loadAdaptations(first.id) }
  ...
  async function selectExercise(exercise: Exercise) {
    ...
    setPrediction('')
    void loadAdaptations(exercise.id)
  ...
  async function loadAdaptations(exerciseId: string) { try { setAdaptations(await window.coach.exercise.listAdaptations({ workspaceId, exerciseId })) } catch { setAdaptations([]) } }
  ...
  async function requestHelp() { if (!active || busy) return; setBusy('help'); setError(null); try { const response = await window.coach.exercise.requestHelp({ workspaceId, exerciseId: active.id, requestId: crypto.randomUUID(), type: 'hint_requested', currentSource: active.kind === 'PREDICT_OUTPUT' ? '' : code, currentPrediction: active.kind === 'PREDICT_OUTPUT' ? prediction : null }); setHint(response.hint); await refreshSet() } catch (cause) { setError(cause instanceof Error ? cause.message : 'Não foi possível carregar a dica.') } finally { setBusy(null) } }
  async function simplify() { if (!active || busy) return; setBusy('help'); setError(null); try { await window.coach.exercise.simplify({ workspaceId, exerciseId: active.id, requestId: crypto.randomUUID() }); await refreshSet(); await loadAdaptations(active.id) } catch (cause) { setError(cause instanceof Error ? cause.message : 'Não foi possível simplificar o enunciado.') } finally { setBusy(null) } }
  async function toggleWording() { if (!active || busy || !adaptations.length) return; setBusy('help'); setError(null); try { const current=adaptations.find((item)=>item.isActive); const refreshed=current ? await window.coach.exercise.restoreOriginal({workspaceId,exerciseId:active.id}) : await window.coach.exercise.activateAdaptation({workspaceId,exerciseId:active.id,adaptationId:adaptations[0]!.id}); setSets((sets)=>({...sets,[topicId]:refreshed})); await loadAdaptations(active.id) } catch(cause){ setError(cause instanceof Error?cause.message:'Não foi possível alternar o enunciado.') } finally { setBusy(null) } }
  ...
  return <div className="flex h-full min-h-0 bg-[#0b0c10] text-coach-ink">
    <aside className="hidden w-72 shrink-0 overflow-y-auto border-r border-coach-line bg-[#0e1014] p-4 lg:block"><p className="px-2 text-[10px] font-black uppercase tracking-[.18em] text-coach-green">Trilha de exercicios</p>{roadmap.modules.map((module) => <section key={module.id} className="mt-4"><div className="flex items-center gap-2 px-2"><strong className="min-w-0 flex-1 truncate text-xs text-white">{module.position}. {module.title}</strong>{module.status === 'locked' && <LockKeyhole size={13} aria-label="Modulo bloqueado" className="text-coach-muted" />}</div>{curriculum.filter((item) => item.module.id === module.id).map((item) => { const selected = item.topicId === topicId; const topicSet = sets[item.topicId]; const count = topicSet?.status === 'ready' ? `${topicSet.progress.filter((progressItem) => progressItem.status === 'passed').length}/${topicSet.exercises.length}` : null; return <button key={item.topicId} disabled={item.state === 'locked'} onClick={() => { void selectTopic(item.topicId) }} className={`mt-1 flex w-full items-center justify-between gap-2 rounded-lg px-2 py-2 text-left text-xs ${selected ? 'bg-coach-green/15 text-coach-green' : 'text-coach-muted hover:bg-white/5'} disabled:cursor-not-allowed disabled:opacity-35`}><span className="truncate">{item.topic}</span><span className="shrink-0 text-[9px] font-black uppercase" aria-label={count ? `${count} exercicios aprovados` : curriculumStateLabel[item.state]}>{count ?? curriculumStateLabel[item.state]}</span></button> })}</section>)}</aside>
    <main className="coach-scroll-pane min-w-0 flex-1 p-5 lg:p-7"><button onClick={onBack} className="flex items-center gap-2 text-xs font-black text-coach-green"><ArrowLeft size={14} /> Voltar aos Estudos</button><p className="mt-6 text-xs font-black uppercase tracking-[.18em] text-coach-orange">Exercícios · {location?.topic ?? 'Tópico'}</p><h2 className="mt-2 font-display text-3xl font-black">Prática estruturada</h2><p className="mt-2 text-sm text-coach-muted"><strong className="text-coach-ink">Executar</strong> testa livremente com a entrada informada. <strong className="text-coach-ink">Enviar</strong> registra uma tentativa e avalia a solução em testes públicos e ocultos.</p>
      {error && <p role="alert" className="mt-6 rounded-lg border border-red-400/35 bg-red-500/10 p-4 text-sm text-red-200">{error}</p>}{busy === 'load' && !set && <p className="mt-6 rounded-lg border border-coach-line bg-[#111217] p-5 text-sm text-coach-muted">Consultando a preparação em segundo plano…</p>}{!busy && !set && <p className="mt-6 rounded-lg border border-coach-line bg-[#111217] p-5 text-sm text-coach-muted">Os exercícios estão na fila de preparação. Você pode continuar estudando enquanto o Coach trabalha.</p>}{set && set.status !== 'ready' && <p className="mt-6 rounded-lg border border-coach-line bg-[#111217] p-5 text-sm text-coach-muted">{set.status === 'generating' ? 'Preparando exercícios…' : set.status === 'waiting_for_provider' ? 'Aguardando o provedor de IA. O Coach retomará automaticamente.' : set.retryAfter ? 'Falha temporária. O Coach tentará novamente automaticamente.' : 'A preparação falhou definitivamente. A aula já pronta permanece disponível.'}</p>}
      {set?.status === 'ready' && active && <div className="mt-6 grid gap-5 xl:grid-cols-[220px_minmax(0,1fr)]"><nav aria-label="Exercícios do tópico" className="space-y-2">{set.exercises.map((exercise) => { const state = set.progress.find((item) => item.exerciseId === exercise.id); return <button key={exercise.id} onClick={() => selectExercise(exercise)} aria-current={exercise.id === active.id ? 'true' : undefined} className={`flex w-full items-start gap-2 rounded-lg border p-3 text-left text-xs ${exercise.id === active.id ? 'border-coach-green bg-coach-green/10' : 'border-coach-line bg-[#111217]'}`}>{exerciseStatus(state)}<span><strong>{exercise.position}. {exercise.title}</strong><small className="mt-1 block text-coach-muted">{state?.attempts ? `${state.attempts} envio${state.attempts === 1 ? '' : 's'}` : 'Ainda não enviado'}</small></span></button>})}</nav><section className="min-w-0 rounded-xl border border-coach-line bg-[#111217] p-5"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase text-coach-green">Problema {active.position} · {active.language}</p><h3 className="mt-1 text-xl font-black">{active.title}</h3></div>{active.requiredForTopicCompletion && <span className="rounded-full bg-coach-orange/10 px-3 py-1 text-[10px] font-black text-coach-orange">Obrigatório</span>}</div><div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={Boolean(busy)} onClick={() => void simplify()} className="rounded-full border border-white/10 px-3 py-1.5 text-[10px] font-black text-white/65 hover:text-coach-green disabled:opacity-40">Simplificar</button>{adaptations.length > 0 && <button type="button" disabled={Boolean(busy)} onClick={() => void toggleWording()} className="rounded-full border border-white/10 px-3 py-1.5 text-[10px] font-black text-white/65 hover:text-coach-green disabled:opacity-40">{adaptations.some((item) => item.isActive) ? 'Ver original' : 'Ver versão simplificada'}</button>}</div><p className="mt-4 whitespace-pre-line text-sm leading-7 text-coach-muted">{active.statement}</p>{active.kind === 'PREDICT_OUTPUT' ? <><div className="mt-4 rounded-lg border border-white/10 bg-black/20 p-4"><p className="text-xs font-black uppercase text-coach-muted">Código para observar</p><pre className="mt-3 max-h-72 overflow-auto whitespace-pre-wrap font-mono text-sm text-coach-ink">{active.codeToObserve}</pre></div><label className="mt-4 block text-xs font-black uppercase text-coach-muted">{active.predictionPrompt}<textarea value={prediction} disabled={Boolean(busy)} onChange={(event) => updatePrediction(event.target.value.slice(0, 20_000))} className="mt-2 min-h-24 w-full rounded-lg border border-coach-line bg-[#0b0c10] p-3 font-mono text-sm font-normal normal-case text-coach-ink" /></label></> : <><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-lg bg-black/20 p-3 text-xs"><strong>Entrada</strong><p className="mt-2 whitespace-pre-line text-coach-muted">{active.inputDescription}</p></div><div className="rounded-lg bg-black/20 p-3 text-xs"><strong>Saída</strong><p className="mt-2 whitespace-pre-line text-coach-muted">{active.outputDescription}</p></div></div><div className="mt-4 rounded-lg border border-white/10 bg-black/20 p-3 text-xs"><strong>Exemplo público</strong><div className="mt-2 grid gap-3 sm:grid-cols-2"><pre className="whitespace-pre-wrap text-coach-muted">Entrada{`\n`}{active.publicTests[0]?.input || '(sem entrada)'}</pre><pre className="whitespace-pre-wrap text-coach-muted">Saída esperada{`\n`}{active.publicTests[0]?.expectedOutput}</pre></div></div><div className="mt-4 h-72 overflow-hidden rounded-lg border border-white/10"><EmbeddedCodeEditor value={code} language={active.language} path={stableEditorPath('exercise', `${workspaceId}:${active.id}`, active.language === 'java' ? 'Main.java' : active.language === 'python' ? 'main.py' : 'main.c')} disabled={Boolean(busy)} maxLength={20_000} onChange={updateCode} /></div><label className="mt-3 block text-xs font-black uppercase text-coach-muted">Entrada para executar<textarea value={stdin} disabled={Boolean(busy)} onChange={(event) => setStdin(event.target.value.slice(0, 20_000))} className="mt-2 min-h-16 w-full rounded-lg border border-coach-line bg-[#0b0c10] p-3 font-mono text-xs font-normal normal-case text-coach-ink" /></label></>}<div className="mt-3 flex flex-wrap justify-end gap-2"><button type="button" disabled={Boolean(busy)} onClick={() => void requestHelp()} className="flex items-center gap-2 rounded-lg border border-coach-line px-4 py-2 text-xs font-black disabled:opacity-50"><HelpCircle size={14} /> {busy === 'help' ? 'Carregando…' : 'Pedir dica'}</button><button type="button" disabled={Boolean(busy) || (active.kind === 'PREDICT_OUTPUT' ? !prediction.trim() : !code.trim())} onClick={() => void run()} className="flex items-center gap-2 rounded-lg border border-coach-green px-4 py-2 text-xs font-black text-coach-green disabled:opacity-50"><Play size={14} /> {busy === 'run' ? 'Executando…' : active.kind === 'PREDICT_OUTPUT' ? 'Observar execução' : 'Executar'}</button><button type="button" disabled={Boolean(busy) || (active.kind === 'PREDICT_OUTPUT' ? !prediction.trim() : !code.trim())} onClick={() => void submit()} className="flex items-center gap-2 rounded-lg bg-coach-orange px-4 py-2 text-xs font-black text-white disabled:opacity-50"><Send size={14} /> {busy === 'submit' ? 'Avaliando…' : active.kind === 'PREDICT_OUTPUT' ? 'Enviar previsão' : 'Enviar solução'}</button></div>{hint && <p className="mt-4 rounded-lg border border-coach-yellow/40 bg-coach-yellow/[.07] p-4 text-sm text-coach-muted"><strong className="text-coach-yellow">Dica</strong><span className="mt-1 block">{hint}</span></p>}{result && <div className={`mt-4 rounded-lg border p-4 text-sm ${result.passed ? 'border-coach-green/40 bg-coach-green/10' : result.mode === 'run' && result.status === 'executed' ? 'border-white/10 bg-black/20' : 'border-coach-orange/40 bg-coach-orange/10'}`}><strong>{result.mode === 'run' ? 'Resultado da execução' : 'Resultado do envio'}</strong><p className="mt-1 text-coach-muted">{executionMessage(result)}</p>{result.mode === 'submit' && result.cases.length > 0 && <p className="mt-2 text-xs text-coach-muted">{result.cases.filter((item) => item.passed).length}/{result.cases.length} resultados de teste aprovados</p>}{result.stdout && <pre className="mt-3 max-h-44 overflow-auto whitespace-pre-wrap rounded bg-black/25 p-3 text-xs">{result.stdout}</pre>}{result.stderr && <pre className="mt-3 max-h-44 overflow-auto whitespace-pre-wrap rounded bg-black/25 p-3 text-xs text-red-200">{result.stderr}</pre>}{result.passed && nextExercise && <button type="button" onClick={() => selectExercise(nextExercise)} className="mt-4 rounded-lg bg-coach-green px-4 py-2 text-xs font-black text-white">Próximo exercício</button>}{result.passed && !nextExercise && <p className="mt-3 text-xs font-black text-coach-green">Todos os exercícios disponíveis deste tópico foram percorridos.</p>}</div>}</section></div>}
    </main>
  </div>
}

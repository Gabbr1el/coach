...
export class ReviewService {
  ...
  submit(input: { workspaceId: string; sessionId: string; itemId: string; answer: string; idempotencyKey: string }): ReviewSession {
    return this.database.sqlite.transaction(() => {
      ...
      const evaluator = JSON.parse(row.evaluatorJson) as { type?: string; expected?: string; correctOptionId?: string }
      const expected = evaluator.type === 'exact_text' ? evaluator.expected : evaluator.correctOptionId
      if (typeof expected !== 'string') throw new Error('Review variant has no authoritative evaluator')
      const correct = canonicalAnswer(input.answer) === canonicalAnswer(expected)

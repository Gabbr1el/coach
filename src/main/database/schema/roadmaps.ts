...
export const roadmaps = sqliteTable('roadmaps', {
  id: text('id').primaryKey(), workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }), title: text('title').notNull(), status: text('status', { enum: ['proposed', 'accepted', 'archived'] }).notNull().default('proposed'), generationKind: text('generation_kind', { enum: ['ai_generated', 'provisional_fallback'] }).notNull().default('ai_generated'), version: integer('version').notNull(), contentRevision: integer('content_revision').notNull().default(1), contentHash: text('content_hash').notNull().default('legacy-unavailable'), providerId: text('provider_id'), modelId: text('model_id'), createdAt: integer('created_at').notNull(), updatedAt: integer('updated_at').notNull(),
...
export const roadmapRebuildPreviews = sqliteTable('roadmap_rebuild_previews', {
  id: text('id').primaryKey(), workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }), currentRoadmapId: text('current_roadmap_id').notNull().references(() => roadmaps.id, { onDelete: 'cascade' }), proposalJson: text('proposal_json').notNull(), materialIdsJson: text('material_ids_json').notNull(), impactJson: text('impact_json').notNull(), status: text('status', { enum: ['pending', 'applied', 'stale'] }).notNull().default('pending'), appliedRoadmapId: text('applied_roadmap_id').references(() => roadmaps.id, { onDelete: 'set null' }), createdAt: integer('created_at').notNull(), resolvedAt: integer('resolved_at'),

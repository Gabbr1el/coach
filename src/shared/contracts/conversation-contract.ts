...
export const streamWorkspaceMessageInputSchema = sendHomeMessageInputSchema.extend({
  ...
  activeExercise: z.object({ exerciseId: z.string().max(420), kind: z.enum(['PROGRAMMING_PROBLEM', 'FIX_CODE', 'COMPLETE_CODE', 'PREDICT_OUTPUT']), title: z.string().max(180), statement: z.string().max(5000), language: z.enum(['python', 'c', 'java']), currentCode: z.string().max(20_000), attempts: z.number().int().nonnegative(), helpUsed: z.boolean(), lastRun: z.object({ status: z.string().max(40), passedTests: z.number().int().nonnegative(), totalTests: z.number().int().nonnegative(), message: z.string().max(1000) }).strict().nullable(), lastSubmission: z.object({ status: z.string().max(40), passedTests: z.number().int().nonnegative(), totalTests: z.number().int().nonnegative(), message: z.string().max(1000) }).strict().nullable() }).strict().optional(),

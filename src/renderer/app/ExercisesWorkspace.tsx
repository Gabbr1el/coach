...
export type ActiveExerciseContext = {
  exerciseId: string
  kind: Exercise['kind']
  title: string
  statement: string
  ...
  attempts: number
  helpUsed: boolean
  lastRun: { status: string; passedTests: number; totalTests: number; message: string } | null
  lastSubmission: { status: string; passedTests: number; totalTests: number; message: string } | null
}

type TopicLocation = { module: Roadmap['modules'][number]; topic: string; topicId: string }
const draftKey = (workspaceId: string, exerciseId: string) => `coach:exercise-draft:${workspaceId}:${exerciseId}`
const predictionKey = (workspaceId: string, exerciseId: string) => `coach:exercise-prediction:${workspaceId}:${exerciseId}`
...
const executionSummary = (execution: Pick<ExerciseExecution, 'status' | 'passedTests' | 'totalTests' | 'message'> | null | undefined) => execution ? { status: execution.status, passedTests: execution.passedTests, totalTests: execution.totalTests, message: execution.message } : null
...
export function ExercisesWorkspace({ workspaceId, roadmap, initialTopicId, onBack, onContext }: {
  ...
  useEffect(() => {
    let current = true
    void Promise.all(locations.map(async ({ topicId: id }) => [id, await window.coach.exercise.getSet({ workspaceId, topicId: id })] as const)).then((entries) => { if (current) setSets(Object.fromEntries(entries)) }).catch(() => undefined)
  ...
  useEffect(() => {
    ...
    void window.coach.exercise.ensureSet({ workspaceId, roadmapId: roadmap.id, moduleId: location.module.id, topicId, lessonId: `${topicId}:lesson` }).then((next) => {
      ...
      if (first) { setCode(localStorage.getItem(draftKey(workspaceId, first.id)) ?? next.progress.find((item) => item.exerciseId === first.id)?.currentCode ?? first.starterCode); setPrediction(localStorage.getItem(predictionKey(workspaceId, first.id)) ?? ''); submitIdentity.current = null }
    }).catch((cause: unknown) => { if (loadEpoch.current === epoch) setError(cause instanceof Error ? cause.message : 'Não foi possível preparar os exercícios.') }).finally(() => { if (loadEpoch.current === epoch) setBusy(null) })
  }, [workspaceId, roadmap.id, topicId, location?.module.id, location?.module.status])

  useEffect(() => {
    if (!active) { onContext(null); return }
    onContext({ exerciseId: active.id, kind: active.kind, title: active.title, statement: active.statement, language: active.language, currentCode: code, attempts: progress?.attempts ?? 0, helpUsed: progress?.helpUsed ?? false, lastRun: result?.mode === 'run' ? executionSummary(result) : executionSummary(progress?.lastRun), lastSubmission: result?.mode === 'submit' ? executionSummary(result) : executionSummary(progress?.lastSubmission) })
    return () => onContext(null)
  }, [active, code, progress?.attempts, progress?.helpUsed, progress?.lastRun, progress?.lastSubmission, result, onContext])

  function selectExercise(exercise: Exercise) {
    setActiveId(exercise.id)
    setCode(localStorage.getItem(draftKey(workspaceId, exercise.id)) ?? set?.progress.find((item) => item.exerciseId === exercise.id)?.currentCode ?? exercise.starterCode)
    setPrediction(localStorage.getItem(predictionKey(workspaceId, exercise.id)) ?? '')
    submitIdentity.current = null
    setResult(null); setHint(null); setError(null); setStdin('')
  }

  function updateCode(value: string) { setCode(value); if (active) localStorage.setItem(draftKey(workspaceId, active.id), value); submitIdentity.current = null }
  function updatePrediction(value: string) { setPrediction(value); if (active) localStorage.setItem(predictionKey(workspaceId, active.id), value); submitIdentity.current = null }
  ...
  return <div className="flex h-full min-h-0 bg-[#0b0c10] text-coach-ink">
    ...
    <main className="coach-scroll-pane min-w-0 flex-1 p-5 lg:p-7"><button onClick={onBack} className="flex items-center gap-2 text-xs font-black text-coach-green"><ArrowLeft size={14} /> Voltar aos Estudos</button><p className="mt-6 text-xs font-black uppercase tracking-[.18em] text-coach-orange">Exercícios · {location?.topic ?? 'Tópico'}</p><h2 className="mt-2 font-display text-3xl font-black">Prática estruturada</h2><p className="mt-2 text-sm text-coach-muted">Executar testa livremente com sua entrada. Enviar avalia a solução nos testes públicos e ocultos.</p>
      ...
      {set?.status === 'ready' && active && <div className="mt-6 grid gap-5 xl:grid-cols-[220px_minmax(0,1fr)]"><nav aria-label="Exercícios do tópico" className="space-y-2">{set.exercises.map((exercise) => { const state = set.progress.find((item) => item.exerciseId === exercise.id); return <button key={exercise.id} onClick={() => selectExercise(exercise)} aria-current={exercise.id === active.id ? 'true' : undefined} className={`flex w-full items-start gap-2 rounded-lg border p-3 text-left text-xs ${exercise.id === active.id ? 'border-coach-green bg-coach-green/10' : 'border-coach-line bg-[#111217]'}`}>{exerciseStatus(state)}<span><strong>{exercise.position}. {exercise.title}</strong><small className="mt-1 block text-coach-muted">{state?.attempts ? `${state.attempts} envio${state.attempts === 1 ? '' : 's'}` : 'Ainda não enviado'}</small></span></button>})}</nav><section className="min-w-0 rounded-xl border border-coach-line bg-[#111217] p-5"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase text-coach-green">Problema {active.position} · {active.language}</p><h3 className="mt-1 text-xl font-black">{active.title}</h3></div>{active.requiredForTopicCompletion && <span className="rounded-full bg-coach-orange/10 px-3 py-1 text-[10px] font-black text-coach-orange">Obrigatório</span>}</div><p className="mt-4 whitespace-pre-line text-sm leading-7 text-coach-muted">{active.statement}</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-lg bg-black/20 p-3 text-xs"><strong>Entrada</strong><p className="mt-2 whitespace-pre-line text-coach-muted">{active.inputDescription}</p></div><div className="rounded-lg bg-black/20 p-3 text-xs"><strong>Saída</strong><p className="mt-2 whitespace-pre-line text-coach-muted">{active.outputDescription}</p></div></div><div className="mt-4 rounded-lg border border-white/10 bg-black/20 p-3 text-xs"><strong>Exemplo público</strong><div className="mt-2 grid gap-3 sm:grid-cols-2"><pre className="whitespace-pre-wrap text-coach-muted">Entrada{`\n`}{active.publicTests[0]?.input || '(sem entrada)'}</pre><pre className="whitespace-pre-wrap text-coach-muted">Saída esperada{`\n`}{active.publicTests[0]?.expectedOutput}</pre></div></div><div className="mt-4 h-72 overflow-hidden rounded-lg border border-white/10"><EmbeddedCodeEditor value={active.kind === 'PREDICT_OUTPUT' ? active.codeToObserve ?? '' : code} language={active.language} path={active.language === 'java' ? 'Main.java' : active.language === 'python' ? 'main.py' : 'main.c'} disabled={Boolean(busy)} maxLength={20_000} onChange={updateCode} /></div><label className="mt-3 block text-xs font-black uppercase text-coach-muted">Entrada para executar<textarea value={stdin} disabled={Boolean(busy)} onChange={(event) => setStdin(event.target.value.slice(0, 20_000))} className="mt-2 min-h-16 w-full rounded-lg border border-coach-line bg-[#0b0c10] p-3 font-mono text-xs font-normal normal-case text-coach-ink" /></label><div className="mt-3 flex flex-wrap justify-end gap-2"><button type="button" disabled={Boolean(busy)} onClick={() => void requestHelp()} className="flex items-center gap-2 rounded-lg border border-coach-line px-4 py-2 text-xs font-black disabled:opacity-50"><HelpCircle size={14} /> {busy === 'help' ? 'Carregando…' : 'Pedir dica'}</button><button type="button" disabled={Boolean(busy) || !code.trim()} onClick={() => void run()} className="flex items-center gap-2 rounded-lg border border-coach-green px-4 py-2 text-xs font-black text-coach-green disabled:opacity-50"><Play size={14} /> {busy === 'run' ? 'Executando…' : 'Executar'}</button><button type="button" disabled={Boolean(busy) || !code.trim()} onClick={() => void submit()} className="flex items-center gap-2 rounded-lg bg-coach-orange px-4 py-2 text-xs font-black text-white disabled:opacity-50"><Send size={14} /> {busy === 'submit' ? 'Avaliando…' : 'Enviar solução'}</button></div>{hint && <p className="mt-4 rounded-lg border border-coach-yellow/40 bg-coach-yellow/[.07] p-4 text-sm text-coach-muted"><strong className="text-coach-yellow">Dica</strong><span className="mt-1 block">{hint}</span></p>}{result && <div className={`mt-4 rounded-lg border p-4 text-sm ${result.passed ? 'border-coach-green/40 bg-coach-green/10' : result.mode === 'run' && result.status === 'executed' ? 'border-white/10 bg-black/20' : 'border-coach-orange/40 bg-coach-orange/10'}`}><strong>{result.mode === 'run' ? 'Resultado da execução' : 'Resultado do envio'}</strong><p className="mt-1 text-coach-muted">{executionMessage(result)}</p>{result.mode === 'submit' && result.cases.length > 0 && <p className="mt-2 text-xs text-coach-muted">{result.cases.filter((item) => item.passed).length}/{result.cases.length} resultados de teste aprovados</p>}{result.stdout && <pre className="mt-3 max-h-44 overflow-auto whitespace-pre-wrap rounded bg-black/25 p-3 text-xs">{result.stdout}</pre>}{result.stderr && <pre className="mt-3 max-h-44 overflow-auto whitespace-pre-wrap rounded bg-black/25 p-3 text-xs text-red-200">{result.stderr}</pre>}{result.passed && nextExercise && <button type="button" onClick={() => selectExercise(nextExercise)} className="mt-4 rounded-lg bg-coach-green px-4 py-2 text-xs font-black text-white">Próximo exercício</button>}{result.passed && !nextExercise && <p className="mt-3 text-xs font-black text-coach-green">Todos os exercícios disponíveis deste tópico foram percorridos.</p>}</div>}</section></div>}
    </main>
  </div>
}

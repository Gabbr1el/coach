...
export function registerRoadmapHandlers(service: RoadmapService): void { ipcMain.handle(ROADMAP_CHANNELS.get, (event, payload: unknown) => { assertTrustedSender(event); return service.get(workspaceRoadmapInputSchema.parse(payload).workspaceId) }); ipcMain.handle(ROADMAP_CHANNELS.generate, (event, payload: unknown) => { assertTrustedSender(event); const input = workspaceRoadmapInputSchema.parse(payload); return service.generate(input.workspaceId, input.instruction) }); ipcMain.handle(ROADMAP_CHANNELS.accept, (event, payload: unknown) => { assertTrustedSender(event); const input = acceptRoadmapInputSchema.parse(payload); return service.accept(input.workspaceId, input.roadmapId) }) }

...
export const getWeeklyPlanInputSchema = z.object({ anchorDate: z.iso.date().optional() }).strict()
...
export interface PlanningApi { listPriorities(): Promise<WorkspacePriority[]>; createDeadline(input: z.infer<typeof createDeadlineInputSchema>): Promise<void>; addRoutineNote(content: string): Promise<void>; listRoutineNotes(): Promise<string[]>; getSchedule(): Promise<StudyScheduleItem[]>; getWeeklyPlan(anchorDate?: string): Promise<WeeklyPlan>; replanWeek(): Promise<WeeklyPlan>; applyAcademicMessage(content: string): Promise<AcademicMutationResult>; getAcademicOverview(): Promise<AcademicOverview> }

...
import type { ConceptMemory } from '../../shared/contracts/learning-evidence-contract'
...
export interface StudyLessonGenerationContext {
  ...
  getTopicConceptMemory?(workspaceId: string, topicId: string): Pick<ConceptMemory, 'performance' | 'retention' | 'confidence' | 'nextReviewAt'> | null
...
export class StudyLessonService {
  ...
  private async generate(provider: AIProvider, workspace: Workspace, roadmap: Roadmap, module: RoadmapModule, topic: string, topicId: string, signal?: AbortSignal): Promise<{ content: LessonContent; response: AIResponse; sources: RoadmapResource[] }> {
    ...
    const conceptMemory = this.generationContext?.getTopicConceptMemory?.(workspace.id, topicId) ?? null
    const topicLearningState = learningState ? {
      difficulty: learningState.difficulty,
      ...(conceptMemory ? { conceptMemory } : { needsReview: learningState.needsReview, confidence: learningState.confidence, ...(learningState.confidence === 'low' ? {} : { mastery: learningState.mastery }) }),
      assessmentCounts: {

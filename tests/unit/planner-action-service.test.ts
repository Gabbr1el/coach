...
describe('PlannerActionService', () => {
  ...
  it('applies workspace creation and its academic event as one intent', async () => { const repository = new MemoryActions(); const deadlines: any[] = []; const workspaceId = crypto.randomUUID(); const planner = service(repository, async (input) => ({ id: workspaceId, name: input.name }), (input) => deadlines.push(input)); const dueAt = 2_000; const action = planner.propose({ type: 'workspace.create', payload: { name: 'C', objective: 'Preparação acadêmica em C', academicEvent: { type: 'exam', title: 'Prova C', dueAt, estimatedMinutes: 240, masteryPercent: null } }, label: 'Criar C', originMessageId: crypto.randomUUID(), contextVersion: 1 }); await planner.resolve(action.id, 'apply'); expect(deadlines).toEqual([{ workspaceId, title: 'Prova C', dueAt, estimatedMinutes: 240, masteryPercent: null }]) })
})

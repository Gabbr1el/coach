...
void app.whenReady().then(async () => {
  try {
    ...
    let contentRepository: SqliteWorkspaceContentRepository | undefined
    let validateWorkspaceAnalysis: (token: string, revision: number, subject: string, focus?: string, context?: string) => boolean = () => false
    const createAcademicContexts = (workspaceId: string, workspaceName: string, related: readonly { subject: string; relation: 'implementation_language' | 'prerequisite' | 'user_selected' }[]) => {
      const createdSubjects: string[] = []
      database!.sqlite.transaction(() => { for (const item of [{ subject: workspaceName, relation: 'primary' as const }, ...related]) { const existing = academicSubjectContext.get(item.subject); if (!existing) { academicSubjectContext.record({ subject: item.subject, declaredLevel: null, declaredKnowledge: [], declaredDifficulties: [], goals: [], sourceEvidence: [] }); createdSubjects.push(academicSubjectContext.get(item.subject)!.subject) } database!.sqlite.prepare('INSERT OR IGNORE INTO workspace_academic_contexts (workspace_id, subject, relation) VALUES (?, ?, ?)').run(workspaceId, academicSubjectContext.get(item.subject)!.subject, item.relation) } })()
      return () => database!.sqlite.transaction(() => { database!.sqlite.prepare('DELETE FROM workspace_academic_contexts WHERE workspace_id=?').run(workspaceId); const removeUnused = database!.sqlite.prepare('DELETE FROM academic_subject_contexts WHERE subject=? AND NOT EXISTS (SELECT 1 FROM workspace_academic_contexts w WHERE w.subject=academic_subject_contexts.subject)'); for (const subject of createdSubjects) removeUnused.run(subject) })()
    }
    const workspaceService = new WorkspaceService({ repository: workspaceRepository, academicContext: academicSubjectContext, validateAnalysis: (...input) => validateWorkspaceAnalysis(...input), onArchived: (id, now) => { contentRepository?.invalidateArchivedWorkspace(id, now) }, findSemanticDuplicate: (key, excludedId) => database!.sqlite.prepare("SELECT id,name,objective,status,created_at AS createdAt,updated_at AS updatedAt,last_opened_at AS lastOpenedAt,archived_at AS archivedAt,completed_at AS completedAt,confirmed_at AS confirmedAt,equivalence_key AS equivalenceKey,meaningful_distinction AS meaningfulDistinction,predecessor_id AS predecessorId FROM workspaces WHERE status='active' AND confirmed_at IS NOT NULL AND equivalence_key=? AND id != COALESCE(?, '') LIMIT 1").get(key, excludedId ?? null) as Workspace | undefined ?? null, provisioning: { createDraft: (id) => workspaceProvisioning!.createDraft(id), start: (id) => workspaceProvisioning!.start(id), get: (id) => workspaceProvisioning!.get(id), retry: (id) => workspaceProvisioning!.retry(id), discardDraft: (id) => workspaceProvisioning!.discardDraft(id), reconcile: (id) => provisioningCoordinator.reconcileWorkspace(id) }, saveLearningOverrides: (workspaceId, subject, input, now) => { database!.sqlite.prepare('INSERT INTO workspace_learning_overrides (workspace_id,subject,canonical_focus,canonical_context,declared_level,declared_knowledge_json,declared_difficulties_json,goals_json,curricular_scope_json,onboarding_analysis_revision,onboarding_analysis_fingerprint,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(workspace_id) DO UPDATE SET subject=excluded.subject,canonical_focus=excluded.canonical_focus,canonical_context=excluded.canonical_context,declared_level=excluded.declared_level,declared_knowledge_json=excluded.declared_knowledge_json,declared_difficulties_json=excluded.declared_difficulties_json,goals_json=excluded.goals_json,curricular_scope_json=excluded.curricular_scope_json,onboarding_analysis_revision=COALESCE(workspace_learning_overrides.onboarding_analysis_revision,excluded.onboarding_analysis_revision),onboarding_analysis_fingerprint=COALESCE(workspace_learning_overrides.onboarding_analysis_fingerprint,excluded.onboarding_analysis_fingerprint),updated_at=excluded.updated_at').run(workspaceId, subject, input.canonicalFocus ?? subject, input.canonicalContext ?? '', input.declaredLevel ?? null, JSON.stringify(input.localKnowledgeProjection ? [input.localKnowledgeProjection] : input.declaredKnowledge ?? []), JSON.stringify(input.declaredDifficulties ?? []), JSON.stringify(input.goals ?? []), JSON.stringify(input.curricularScope ?? {}), input.analysisRevision ?? null, createHash('sha256').update(JSON.stringify({ subject, focus: input.canonicalFocus ?? subject, context: input.canonicalContext ?? '', revision: input.analysisRevision ?? null })).digest('hex'), now, now) }, createAcademicContexts })
    ...
    const priorSubjectMemory = (subject: string, limit = 3) => (database!.sqlite.prepare("SELECT w.name AS subject,o.goals_json AS goals,m.summary AS memory FROM workspaces w LEFT JOIN workspace_learning_overrides o ON o.workspace_id=w.id LEFT JOIN workspace_memories m ON m.workspace_id=w.id WHERE w.status IN ('completed','archived') AND (lower(trim(w.name))=lower(trim(?)) OR EXISTS (SELECT 1 FROM workspace_academic_contexts x WHERE x.workspace_id=w.id AND lower(trim(x.subject))=lower(trim(?)))) ORDER BY COALESCE(w.completed_at,w.archived_at,w.updated_at) DESC LIMIT ?").all(subject, subject, Math.min(limit, 3)) as Array<{ subject: string; goals: string | null; memory: string | null }>).map((row) => { let goals: string[] = []; try { const parsed = JSON.parse(row.goals ?? '[]'); goals = Array.isArray(parsed) ? parsed.filter((item): item is string => typeof item === 'string').slice(0, 5) : [] } catch {} return { subject: row.subject, outcomes: [], goals, prerequisites: [], memory: row.memory?.slice(0, 800) ?? null } })
    const currentWorkspaceContext = new CurrentWorkspaceContextService({ getWorkspace: (id) => workspaceRepository.findById(id), getStudyState: (id) => studyWorkspaceService.getState(id), getObserverState: (id) => observerService.getState(id), getWorkspaceMemory, getAcademicSubjectContext: (subject) => academicSubjectContext.get(subject), getPrimaryAcademicContext: (workspaceId) => { const row = database!.sqlite.prepare("SELECT subject FROM workspace_academic_contexts WHERE workspace_id = ? AND relation = 'primary'").get(workspaceId) as { subject: string } | undefined; return row ? academicSubjectContext.get(row.subject) : null }, getRelatedAcademicContexts: (workspaceId) => (database!.sqlite.prepare("SELECT subject, relation FROM workspace_academic_contexts WHERE workspace_id = ? AND relation != 'primary'").all(workspaceId) as Array<{ subject: string; relation: 'implementation_language' | 'prerequisite' | 'user_selected' }>).flatMap((item) => { const context = academicSubjectContext.get(item.subject); return context ? [{ ...context, relation: item.relation }] : [] }), getPriorSubjectMemory: (workspaceId, limit) => { const row = database!.sqlite.prepare('SELECT name FROM workspaces WHERE id=?').get(workspaceId) as { name: string } | undefined; return row ? priorSubjectMemory(row.name, limit) : [] } })
    ...
    let workspaceCoachService!: WorkspaceCoachService
    registerApplicationHandlers()
    registerExerciseHandlers(exerciseService, database)
    registerReviewHandlers(reviewService)
    contentRepository = new SqliteWorkspaceContentRepository(database)
    prefetch = new PedagogicalPrefetchScheduler({ repository: contentRepository, getRoadmap: (id) => roadmapRepository.findCurrent(id), isWorkspaceActive: (id) => (database!.sqlite.prepare('SELECT status FROM workspaces WHERE id=?').get(id) as { status: string } | undefined)?.status === 'active', onJobsChanged: () => contentWorker?.wake() })
    registerStudyProgressHandlers(database, () => toolchainManager.getStatuses(), providerManager, prefetch, learningEvidence, workspaceService)
    registerStudyLessonHandlers(studyLessonService, database)
    ...
    const resumeProviderWork = () => { roadmapService.retryWaitingForProvider(); contentWorker?.retryProviderUnavailable(); workspaceProvisioning?.resumePending() }
    providerManager.onAvailable(resumeProviderWork)
    if (providerManager.getActive()) resumeProviderWork()
    workspaceProvisioning.resumePending()
    provisioningCoordinator.reconcileAll()
    ...
    const workspaceOnboarding = new WorkspaceOnboardingService({ repository: new DrizzleConversationRepository(database), providerManager, academicContext: academicSubjectContext, listPriorSubjectMemory: priorSubjectMemory, listContextCandidates: () => {
      const workspaces = database!.sqlite.prepare("SELECT w.name AS subject,COUNT(DISTINCT c.id) AS concepts,COUNT(DISTINCT cm.concept_id) AS memories FROM workspaces w LEFT JOIN concepts c ON c.workspace_id=w.id LEFT JOIN concept_memories cm ON cm.workspace_id=w.id WHERE w.status='active' AND NOT EXISTS (SELECT 1 FROM workspace_provisioning p WHERE p.workspace_id=w.id AND p.status='draft') GROUP BY w.id,w.name").all() as Array<{ subject: string; concepts: number; memories: number }>
      const relations = database!.sqlite.prepare("SELECT subject,COUNT(*) AS uses FROM workspace_academic_contexts WHERE relation!='primary' GROUP BY subject").all() as Array<{ subject: string; uses: number }>
      return [...workspaces.map((item) => ({ subject: item.subject, source: item.memories > 0 ? 'concept-memory' as const : 'workspace' as const, strength: 70 + Math.min(item.concepts, 10) + Math.min(item.memories, 20), explanation: item.memories > 0 ? `O Workspace ${item.subject} tem experiência observada; ela orienta apenas a escolha de contexto e não é transferida como domínio.` : item.concepts > 0 ? `O Workspace ${item.subject} possui conceitos reais, ainda sem evidência transferível.` : `Você já possui um Workspace de ${item.subject}.` })), ...relations.map((item) => ({ subject: item.subject, source: 'workspace' as const, strength: 50 + Math.min(item.uses, 20), explanation: `${item.subject} já aparece em relações acadêmicas de ${item.uses} Workspace(s).` }))]
    } })

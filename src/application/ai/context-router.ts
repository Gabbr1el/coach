...
export interface AuthorizedStudyContext {
  ...
  readonly activeExercise?: StreamWorkspaceMessageInput['activeExercise']
...
export class ContextRouter {
  route(input: StreamWorkspaceMessageInput, observer?: ObserverState | null, authorizedContext?: AuthorizedStudyContext): RoutedContext {
    ...
    const context = authorizedContext
      ? { ...authorizedContext, activePage: input.activePage, activeMaterial: input.activeMaterial, activeStudy: input.activeStudy ? { ...input.activeStudy, currentExcerpt: input.activeStudy.currentExcerpt?.slice(0, 2000) ?? null } : undefined, activeInteractiveCode: input.activeInteractiveCode ? { ...input.activeInteractiveCode, code: input.activeInteractiveCode.code.slice(0, 12_000), lastExecution: input.activeInteractiveCode.lastExecution ? { ...input.activeInteractiveCode.lastExecution, stdout: input.activeInteractiveCode.lastExecution.stdout.slice(0, 4000), stderr: input.activeInteractiveCode.lastExecution.stderr.slice(0, 4000) } : null } : undefined, activeExercise: input.activeExercise ? { exerciseId: input.activeExercise.exerciseId, kind: input.activeExercise.kind, title: input.activeExercise.title, statement: input.activeExercise.statement, language: input.activeExercise.language, currentCode: input.activeExercise.currentCode.slice(0, 12_000), lastRun: input.activeExercise.lastRun, lastSubmission: input.activeExercise.lastSubmission, attempts: input.activeExercise.attempts, helpUsed: input.activeExercise.helpUsed } : undefined, practiceContext: input.practiceContext ? { fileName: input.practiceContext.fileName, language: input.practiceContext.language, code: input.practiceContext.code.slice(0, 12_000) } : undefined, ...(input.lastExecution ? { lastExecution: { ...input.lastExecution, stdout: input.lastExecution.stdout.slice(0, 4000), stderr: input.lastExecution.stderr.slice(0, 4000) } } : {}) }
      : undefined

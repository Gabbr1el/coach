...
describe('renderer UI safeguards', () => {
  ...
  it('keeps creation locked behind analysis and material role approval', () => { const app = read('src/renderer/app/App.tsx'); expect(app).toContain("!analysis?.analysisToken"); expect(app).toContain("item.status === 'staged'"); expect(app).toContain('Adicionar PDF/PPTX'); expect(app).toContain('Base curricular'); expect(app).toContain('Projeção local editável'); expect(app).not.toContain('onKeyDown={(event) => event.preventDefault()') })
  ...
  it('documents non-obvious fields and protects editable keyboard targets', () => {
    ...
    expect(app).toContain('Resultado que você quer alcançar')

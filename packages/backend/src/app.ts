...
export function buildApp(dependencies: { config: AppConfig; database: Database; auth: AuthBoundary; verifyAccessToken: VerifyAccessToken }) {
  ...
  const context = (request: { identity: { userId: string }; coachSession: { id: string; tenantId: string } }) => ({
    userId: request.identity.userId, tenantId: request.coachSession.tenantId, sessionId: request.coachSession.id
  });
  ...
  app.get('/ready', async (_request, reply) => {
    try {
      const [schema] = await dependencies.database<{ app_sessions: string | null; migrations: string | null }[]>`select to_regclass('public.app_sessions') as app_sessions, to_regclass('public.coach_migrations') as migrations`;
      if (!schema?.app_sessions || !schema.migrations) throw new Error('schema_not_ready');

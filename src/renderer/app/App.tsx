    ...
    return (
      <main className="min-h-screen bg-[#f5f3ec] text-coach-ink">
        ...
        <section className="grid min-h-[calc(100dvh-4rem)] xl:grid-cols-[285px_minmax(480px,1fr)_350px]">
          ...
          <section className="flex min-h-[560px] flex-col bg-[#102724] text-white"><div className="flex items-center justify-between border-b border-white/10 px-5 py-3"><div><p className="font-mono text-sm font-bold">{studyState?.fileName ?? 'main.py'}</p><p className="text-[10px] uppercase tracking-wider text-white/35">Monaco · arquivo local persistente</p></div><div className="flex gap-2"><button type="button" onClick={() => setWorkspaceInput('Analise meu código atual. Identifique primeiro o possível erro ou desvio e me dê apenas uma dica progressiva.')} className="rounded-lg border border-white/15 px-3 py-2 text-xs font-black">Preparar revisão</button><button disabled={executing || !editorContent.trim()} type="button" onClick={() => { setExecuting(true); setExecution(null); void window.coach.codeExecution.execute({ workspaceId: selected.id, language: 'python', content: editorContent }).then(setExecution).catch(() => setWorkspaceError('Não foi possível executar o Python. Confirme se python3 está instalado.')).finally(() => setExecuting(false)) }} className="rounded-lg bg-coach-green px-4 py-2 text-xs font-black disabled:opacity-40">{executing ? 'Executando…' : 'Executar'}</button></div></div><div className="min-h-[360px] flex-1"><WorkspaceEditor value={editorContent} onChange={(value) => { editorContentRef.current = value; setEditorContent(value) }} /></div>{execution && <div className="max-h-48 overflow-auto border-t border-white/10 bg-[#081613] p-4 font-mono text-xs leading-5"><div className="mb-2 flex justify-between text-white/35"><span>$ {execution.command}</span><span>exit {execution.exitCode ?? '—'} · {execution.durationMs}ms</span></div>{execution.stdout && <pre className="whitespace-pre-wrap text-[#b8e1c8]">{execution.stdout}</pre>}{execution.stderr && <pre className="whitespace-pre-wrap text-[#ffad9d]">{execution.stderr}</pre>}{!execution.stdout && !execution.stderr && <p className="text-white/35">Programa concluído sem saída.</p>}</div>}<div className="flex items-center justify-between border-t border-white/10 px-5 py-2 text-[11px] text-white/40"><span>{studyState?.language ?? 'python'} · UTF-8</span><span>{documentSavedAt ? 'Salvo agora' : 'Salvamento automático'}</span></div></section>

...
import { createSubjectLearningContext, normalizeSubject } from '../../src/application/workspaces/subject-normalizer'
...
describe('workspace subject normalization', () => {
  ...
  it('keeps a level qualifier as part of the persisted subject identity', () => { expect(normalizeSubject('JavaScript avançado')).toEqual({ subject: 'JavaScript avançado', userContext: null }) })
  it('deduplicates each evidence source without mixing declared and observed memory', () => { expect(createSubjectLearningContext('java', ['Domino classes', 'Domino classes'], ['Errou herança', 'Errou herança'])).toEqual({ subject: 'Java', declared: ['Domino classes'], observed: ['Errou herança'] }) })

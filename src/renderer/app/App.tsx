...
function StudyModuleView({ module, roadmap, onBack, onAskCoach, onPractice }: { module: RoadmapModule; roadmap: Roadmap | null; onBack(): void; onAskCoach(prompt: string): void; onPractice(): void }) {
  const [topicIndex, setTopicIndex] = useState(0); const [answer, setAnswer] = useState(''); const topic = module.topics[topicIndex] ?? module.title; const completed = roadmap?.modules.filter((item) => item.status === 'completed').length ?? 0; const progress = Math.round((completed + topicIndex / Math.max(1, module.topics.length)) / Math.max(1, roadmap?.modules.length ?? 1) * 100)
  return <div className="grid h-full min-h-0 grid-cols-[210px_minmax(0,1fr)] bg-[#0b0c10]"><aside className="coach-scroll-pane border-r border-coach-line bg-[#0e0f13] p-4"><button onClick={onBack} className="text-xs font-black text-coach-muted">← Voltar ao plano</button><p className="mt-6 text-[10px] font-black uppercase tracking-wider text-coach-muted">Roteiro</p><p className="mt-1 text-xs text-coach-green">{progress}% concluído</p><div className="mt-2 h-1 rounded bg-coach-line"><div className="h-full rounded bg-coach-green" style={{ width: `${progress}%` }} /></div><div className="mt-5 space-y-2">{roadmap?.modules.map((item) => <div key={item.id}><button className={`w-full rounded-lg px-3 py-2 text-left text-xs font-black ${item.id === module.id ? 'bg-[#1a1c22] text-white' : 'text-coach-muted'}`}>{item.position}. {item.title}</button>{item.id === module.id && <div className="ml-3 mt-1 border-l border-coach-line pl-3">{item.topics.map((entry, index) => <button key={entry} onClick={() => setTopicIndex(index)} className={`block w-full py-1.5 text-left text-[11px] ${index === topicIndex ? 'font-black text-coach-orange' : 'text-coach-muted'}`}>{index === topicIndex ? '●' : '○'} {entry}</button>)}</div>}</div>)}</div></aside><article className="coach-scroll-pane h-full px-6 py-7 lg:px-12"><div className="mx-auto max-w-4xl"><div className="flex items-center justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-coach-green">Próximo passo escolhido pelo Coach</p><h2 className="mt-2 font-display text-3xl font-black">{topic}</h2></div><span className="rounded-full border border-coach-line px-3 py-2 text-[10px] font-black text-coach-muted">ESTUDO · {Math.max(3, Math.round(module.estimatedMinutes / Math.max(1, module.topics.length)))} MIN</span></div><p className="mt-4 max-w-3xl text-sm leading-6 text-coach-muted">{module.objective}</p><section className="mt-8 rounded-xl border border-coach-line bg-[#111217] p-6"><div className="grid gap-4 md:grid-cols-3"><div className="rounded-lg bg-[#181a20] p-4"><span className="text-[10px] text-coach-muted">01 · Conceito</span><strong className="mt-2 block text-sm">{topic}</strong></div><div className="rounded-lg bg-[#181a20] p-4"><span className="text-[10px] text-coach-muted">02 · Relação</span><strong className="mt-2 block text-sm">{module.title}</strong></div><div className="rounded-lg bg-[#181a20] p-4"><span className="text-[10px] text-coach-muted">03 · Resultado</span><strong className="mt-2 block text-sm">{module.outcomes[0] ?? module.completionCriteria[0]}</strong></div></div><p className="mt-6 text-sm leading-7 text-coach-muted">O Coach conduz este tópico usando o objetivo da etapa, exemplos progressivos e o contexto já conhecido do Workspace. Peça um exemplo quando precisar aprofundar sem sair da sessão.</p><button onClick={() => onAskCoach(`Estou em “${topic}”, dentro de “${module.title}”. Explique este conteúdo com um exemplo curto e acompanhe minha compreensão.`)} className="mt-4 text-xs font-black text-coach-orange">Pedir exemplo ao Coach →</button></section><section className="mt-5 rounded-xl border border-coach-line bg-[#111217] p-6"><p className="text-[10px] font-black uppercase tracking-wider text-coach-orange">Antes de continuar…</p><h3 className="mt-2 font-display text-lg font-black">Explique com suas palavras qual é o papel de “{topic}” e como ele se relaciona com esta etapa.</h3><textarea value={answer} onChange={(event) => setAnswer(event.target.value)} rows={3} placeholder="Escreva seu entendimento…" className="mt-4 w-full resize-none rounded-lg border border-coach-line bg-[#0d0e12] p-3 text-sm outline-none focus:border-coach-orange" /><div className="mt-3 flex flex-wrap gap-2"><button disabled={!answer.trim()} onClick={() => onAskCoach(`Avalie minha resposta sobre “${topic}” sem avançar automaticamente. Resposta: ${answer}`)} className="rounded-lg bg-coach-orange px-4 py-2 text-xs font-black text-[#0c0d10] disabled:opacity-40">Responder e avaliar</button><button onClick={onPractice} className="rounded-lg border border-coach-line px-4 py-2 text-xs font-black">Ir para prática</button>{topicIndex < module.topics.length - 1 && <button onClick={() => { setTopicIndex((index) => index + 1); setAnswer('') }} className="rounded-lg border border-coach-green px-4 py-2 text-xs font-black text-coach-green">Próximo tópico</button>}</div></section></div></article></div>
}

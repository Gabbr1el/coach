...
import { SqliteEventWorkspaceLinkRepository } from '../../src/main/repositories/sqlite-event-workspace-link-repository'
...
function migrationsThrough0056(): string {
  const directory = mkdtempSync(join(tmpdir(), 'coach-migrations-0056-')); temporaryDirectories.push(directory); cpSync(migrationsFolder, directory, { recursive: true })
  const journalPath = join(directory, 'meta/_journal.json'); const journal = JSON.parse(readFileSync(journalPath, 'utf8')) as { entries: Array<{ idx: number; tag: string }> }; journal.entries = journal.entries.filter((entry) => entry.idx <= 56); writeFileSync(journalPath, `${JSON.stringify(journal, null, 2)}\n`); rmSync(join(directory, '0057_study_plan_target_ids.sql'))
  return directory
}
...
describe('Coach database migrations', () => {
  ...
  it('repairs published intermediate evidence schemas without losing historical identities', () => {
    const database = openCoachDatabase({ databasePath: createDatabasePath(), migrationsFolder })
    database.sqlite.exec(`
      INSERT INTO workspaces (id,name,objective,status,created_at,updated_at) VALUES ('repair-evidence-workspace','C','Ponteiros','active',1,1);
      INSERT INTO concepts (id,workspace_id,canonical_name,domain,metadata_json,created_at,updated_at) VALUES ('repair-concept','repair-evidence-workspace','Ponteiros','c','{}',1,1);
      INSERT INTO assessment_intents (id,workspace_id,concept_id,kind,objective,created_at,updated_at) VALUES ('repair-intent','repair-evidence-workspace','repair-concept','recall','Explicar',1,1);
      INSERT INTO assessment_variants (id,workspace_id,intent_id,environment,source_ref,source_revision,difficulty,created_at,updated_at) VALUES ('repair-variant','repair-evidence-workspace','repair-intent','checkpoint','lesson','r1','standard',1,1);
      INSERT INTO learning_attempts (id,workspace_id,concept_id,assessment_intent_id,assessment_variant_id,environment,source_ref,source_revision,idempotency_key,payload_hash,outcome,correct,independent,reasoning_quality,occurred_at,created_at) VALUES ('repair-attempt','repair-evidence-workspace','repair-concept','repair-intent','repair-variant','checkpoint','lesson','r1','historical-key','real-hash','correct',1,1,'coherent',2,2);
      INSERT INTO learning_evidence (id,attempt_id,concept_id,type,strength,ordinal,occurred_at,created_at) VALUES ('repair-event','repair-attempt','repair-concept','answer','strong',1,2,2);
      PRAGMA foreign_keys=OFF;
      DROP INDEX assessment_variants_source_unique;
      ALTER TABLE assessment_variants RENAME TO assessment_variants_current;
      CREATE TABLE assessment_variants (id text PRIMARY KEY NOT NULL,intent_id text NOT NULL,environment text NOT NULL,source_ref text NOT NULL,source_revision text NOT NULL,difficulty text NOT NULL,prerequisite_concept_ids_json text DEFAULT '[]' NOT NULL,public_metadata_json text DEFAULT '{}' NOT NULL,public_payload_json text DEFAULT '{}' NOT NULL,evaluator_json text DEFAULT '{}' NOT NULL,created_at integer NOT NULL,updated_at integer NOT NULL,FOREIGN KEY (intent_id) REFERENCES assessment_intents(id) ON DELETE cascade);
      INSERT INTO assessment_variants SELECT id,intent_id,environment,source_ref,source_revision,difficulty,prerequisite_concept_ids_json,public_metadata_json,public_payload_json,evaluator_json,created_at,updated_at FROM assessment_variants_current;
      DROP TABLE assessment_variants_current;
      CREATE UNIQUE INDEX assessment_variants_source_unique ON assessment_variants (environment,source_ref,source_revision);
      ALTER TABLE learning_attempts DROP COLUMN first_seen_at;
      ALTER TABLE learning_attempts DROP COLUMN payload_hash;
      DROP TABLE exercise_help_events;
      ALTER TABLE concept_memories RENAME TO concept_memory;
      DROP INDEX review_sessions_one_current_workspace;
      CREATE UNIQUE INDEX review_sessions_one_current_workspace ON review_sessions (workspace_id) WHERE status='active';
      PRAGMA foreign_keys=ON;
    `)

    repairPublishedEvidenceSchema(database.sqlite)
    repairPublishedEvidenceSchema(database.sqlite)

    expect(database.sqlite.prepare("SELECT workspace_id AS workspaceId FROM assessment_variants WHERE id='repair-variant'").get()).toEqual({ workspaceId: 'repair-evidence-workspace' })
    expect(database.sqlite.prepare("SELECT idempotency_key AS key,payload_hash AS hash,first_seen_at AS firstSeenAt FROM learning_attempts WHERE id='repair-attempt'").get()).toEqual({ key: 'historical-key', hash: 'legacy-unavailable:repair-attempt', firstSeenAt: null })
    expect(database.sqlite.prepare("SELECT attempt_id AS attemptId FROM learning_evidence WHERE id='repair-event'").get()).toEqual({ attemptId: 'repair-attempt' })
    expect(database.sqlite.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('concept_memories','exercise_help_events') ORDER BY name").all()).toEqual([{ name: 'concept_memories' }, { name: 'exercise_help_events' }])
    expect((database.sqlite.prepare("SELECT sql FROM sqlite_master WHERE type='index' AND name='review_sessions_one_current_workspace'").get() as { sql: string }).sql).toContain("'preparation'")
    expect(database.sqlite.pragma('foreign_key_check')).toEqual([])
    validateCoachDatabaseSchema(database.sqlite, currentJournalMigrationCount)
    database.close()
  })
  it('preflights the known unjournaled 0050 columns before the published migration runs', () => {
    const databasePath = createDatabasePath()
    const through0049 = mkdtempSync(join(tmpdir(), 'coach-migrations-0049-')); temporaryDirectories.push(through0049); cpSync(migrationsFolder, through0049, { recursive: true })
    const journalPath = join(through0049, 'meta/_journal.json'); const journal = JSON.parse(readFileSync(journalPath, 'utf8')) as { entries: Array<{ idx: number; tag: string }> }; journal.entries = journal.entries.filter((entry) => entry.idx <= 49); writeFileSync(journalPath, `${JSON.stringify(journal, null, 2)}\n`)
    let database = openCoachDatabase({ databasePath, migrationsFolder: through0049 })
    for (const column of ['public_payload_json', 'evaluator_json']) {
      const columns = new Set((database.sqlite.pragma('table_info(assessment_variants)') as Array<{ name: string }>).map((item) => item.name))
      if (!columns.has(column)) database.sqlite.exec(`ALTER TABLE assessment_variants ADD ${column} text NOT NULL DEFAULT '{}'`)
    }
    database.close()
    database = openCoachDatabase({ databasePath, migrationsFolder })
    const columns = database.sqlite.pragma('table_info(assessment_variants)') as Array<{ name: string }>
    expect(columns.filter((column) => column.name === 'public_payload_json')).toHaveLength(1)
    expect(columns.filter((column) => column.name === 'evaluator_json')).toHaveLength(1)
    expect(database.sqlite.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='review_sessions'").get()).toEqual({ name: 'review_sessions' })
    database.close()
    expect(() => openCoachDatabase({ databasePath, migrationsFolder }).close()).not.toThrow()
  })
  ...
  it('creates the current domain schema and migration history', () => {
    ...
    expect(tables).toEqual([
      ...
      { name: 'assessment_intents' },
      { name: 'assessment_variants' },
      { name: 'checkpoint_reasoning_evidence' },
      { name: 'concept_aliases' },
      { name: 'concept_memories' },
      { name: 'concepts' },
  ...
  it('keeps one authoritative item identity while adjusting a workspace day and shared total across reload', () => {
    const databasePath = createDatabasePath(); let database = openCoachDatabase({ databasePath, migrationsFolder })
    const javaId = crypto.randomUUID(); const otherId = crypto.randomUUID(); const planId = crypto.randomUUID(); const javaItemId = crypto.randomUUID(); const otherItemId = crypto.randomUUID(); const now = Date.parse('2026-09-10T12:00:00Z')
    database.sqlite.prepare("INSERT INTO workspaces (id,name,objective,status,created_at,updated_at) VALUES (?,?,?,'active',?,?),(?,?,?,'active',?,?)").run(javaId, 'Java', 'Java', now, now, otherId, 'C', 'C', now, now)
    database.sqlite.prepare("INSERT INTO planning_settings (id,timezone,updated_at) VALUES ('current','UTC',?)").run(now)
    database.sqlite.prepare('INSERT INTO weekly_plans (id,week_start,timezone,revision,generated_at,updated_at) VALUES (?,?,?,?,?,?)').run(planId, '2026-09-07', 'UTC', 1, now, now)
    const insert = database.sqlite.prepare("INSERT INTO weekly_plan_items (id,plan_id,workspace_id,source_key,date_key,title,duration_minutes,position,status,module_id,topic_id,activity_type,scheduled_start_minutes,reason,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
    insert.run(javaItemId, planId, javaId, 'java', '2026-09-10', 'Java hoje', 60, 1, 'pending', null, null, 'exercise', 600, 'Java', now, now)
    insert.run(otherItemId, planId, otherId, 'c', '2026-09-10', 'C hoje', 30, 2, 'pending', null, null, 'exercise', 660, 'C', now, now)
    const insertSession = database.sqlite.prepare("INSERT INTO study_sessions (id,workspace_id,status,started_at,focus_seconds) VALUES (?,?,'active',?,0)"); const javaSession = crypto.randomUUID(); const otherSession = crypto.randomUUID(); insertSession.run(javaSession, javaId, now); insertSession.run(otherSession, otherId, now)
    const insertMirror = database.sqlite.prepare("INSERT INTO study_plan_items (id,workspace_id,session_id,title,duration_minutes,position,status,activity_type,created_at,updated_at) VALUES (?,?,?,?,?,1,'pending','exercise',?,?)"); insertMirror.run(javaItemId, javaId, javaSession, 'Java hoje', 60, now, now); insertMirror.run(otherItemId, otherId, otherSession, 'C hoje', 30, now, now)
    const repository = new DrizzlePlanningRepository(database); expect(repository.adjustWorkspaceDayLoad(planId, javaId, '2026-09-10', 120, now + 1)).toBe(true); expect(repository.adjustWorkspaceDayLoad(planId, javaId, '2026-09-10', 120, now + 2)).toBe(true)
    let items = repository.findWeeklyPlan('2026-09-07', 'UTC')!.items; expect(items.find((item) => item.id === javaItemId)?.durationMinutes).toBe(300); expect(items.find((item) => item.id === otherItemId)?.durationMinutes).toBe(30); expect(items.reduce((sum, item) => sum + item.durationMinutes, 0)).toBe(330); expect(database.sqlite.prepare('SELECT duration_minutes AS minutes FROM study_plan_items WHERE id=?').get(javaItemId)).toEqual({ minutes: 300 })
    database.close(); database = openCoachDatabase({ databasePath, migrationsFolder }); items = new DrizzlePlanningRepository(database).findWeeklyPlan('2026-09-07', 'UTC')!.items; expect(items.map((item) => item.id)).toEqual([javaItemId, otherItemId]); expect(items.reduce((sum, item) => sum + item.durationMinutes, 0)).toBe(330); database.close()
  })
  it('rolls back load and revision when the study mirror is missing', () => { const databasePath = createDatabasePath(); const database = openCoachDatabase({ databasePath, migrationsFolder }); const workspaceId = crypto.randomUUID(); const planId = crypto.randomUUID(); const itemId = crypto.randomUUID(); const now = Date.parse('2026-09-10T12:00:00Z'); database.sqlite.prepare("INSERT INTO workspaces (id,name,objective,status,created_at,updated_at) VALUES (?,?,?,'active',?,?)").run(workspaceId, 'Java', 'Java', now, now); database.sqlite.prepare('INSERT INTO weekly_plans (id,week_start,timezone,revision,generated_at,updated_at) VALUES (?,?,?,?,?,?)').run(planId, '2026-09-07', 'UTC', 1, now, now); database.sqlite.prepare("INSERT INTO weekly_plan_items (id,plan_id,workspace_id,source_key,date_key,title,duration_minutes,position,status,activity_type,scheduled_start_minutes,reason,created_at,updated_at) VALUES (?,?,?,?,?,?,60,1,'pending','exercise',600,'test',?,?)").run(itemId, planId, workspaceId, 'java', '2026-09-10', 'Java', now, now); expect(() => new DrizzlePlanningRepository(database).adjustWorkspaceDayLoad(planId, workspaceId, '2026-09-10', 30, now + 1)).toThrow(/mirror/); expect(database.sqlite.prepare('SELECT duration_minutes AS minutes FROM weekly_plan_items WHERE id=?').get(itemId)).toEqual({ minutes: 60 }); expect(database.sqlite.prepare('SELECT revision FROM weekly_plans WHERE id=?').get(planId)).toEqual({ revision: 1 }); database.close() })
  ...
  it('roundtrips exact plan target IDs and rejects cross-workspace ownership', async () => {
    const database = openCoachDatabase({ databasePath: createDatabasePath(), migrationsFolder }); const workspaces = new DrizzleWorkspaceRepository(database); const repository = new DrizzleStudyWorkspaceRepository(database)
    const owner = await workspaces.create({ id: crypto.randomUUID(), name: 'Owner', objective: '', createdAt: 1, updatedAt: 1 }); const other = await workspaces.create({ id: crypto.randomUUID(), name: 'Other', objective: '', createdAt: 1, updatedAt: 1 })
    const setId = crypto.randomUUID(); const materialId = crypto.randomUUID(); database.sqlite.prepare("INSERT INTO exercise_sets (id,workspace_id,roadmap_id,module_id,topic_id,lesson_id,status,generation_attempts,created_at,updated_at) VALUES (?,?,'r','m','m:t','l','ready',0,1,1)").run(setId, owner.id); database.sqlite.prepare("INSERT INTO materials (id,workspace_id,name,media_type,page_count,status,relevance,role,created_at) VALUES (?,?,'Book','application/pdf',1,'ready',50,'base',1)").run(materialId, owner.id)
    const state = { workspaceId: owner.id, sessionId: crypto.randomUUID(), sessionStartedAt: 2, fileName: 'main.py', language: 'python', editorContent: '', notes: '', shareContextWithAi: false, timerDurationSeconds: 1200, timerRemainingSeconds: 1200, timerStatus: 'idle' as const, timerStartedAt: null, plan: [{ id: crypto.randomUUID(), title: 'Exercise', durationMinutes: 20, position: 1, status: 'active' as const, moduleId: 'm', topicId: 'm:t', activityType: 'exercise' as const, exerciseSetId: setId, materialId }], updatedAt: 3, documentRevision: 0, notesRevision: 0, accumulatedFocusSeconds: 0 }
    await repository.createState(state); expect((await repository.findState(owner.id, 4))?.plan[0]).toMatchObject({ exerciseSetId: setId, materialId })
    const foreign = crypto.randomUUID(); database.sqlite.prepare("INSERT INTO exercise_sets (id,workspace_id,roadmap_id,module_id,topic_id,lesson_id,status,generation_attempts,created_at,updated_at) VALUES (?,?,'r','m','m:t','l','ready',0,1,1)").run(foreign, other.id)
    database.sqlite.prepare('UPDATE study_plan_items SET exercise_set_id=? WHERE id=?').run(foreign, state.plan[0]!.id); await expect(repository.findState(owner.id, 4)).rejects.toThrow(/does not belong/); database.sqlite.prepare('UPDATE study_plan_items SET exercise_set_id=? WHERE id=?').run(setId, state.plan[0]!.id)
    await expect(repository.replacePlan(owner.id, state.sessionId, [{ ...state.plan[0]!, id: crypto.randomUUID(), position: 2, exerciseSetId: foreign }], 5)).rejects.toThrow(/does not belong/)
    const foreignMaterial = crypto.randomUUID(); database.sqlite.prepare("INSERT INTO materials (id,workspace_id,name,media_type,page_count,status,relevance,role,created_at) VALUES (?,?,'Foreign','application/pdf',1,'ready',50,'base',1)").run(foreignMaterial, other.id); await expect(repository.replacePlan(owner.id, state.sessionId, [{ ...state.plan[0]!, id: crypto.randomUUID(), position: 2, materialId: foreignMaterial }], 5)).rejects.toThrow(/does not belong/)
    database.close()
  })
  it('upgrades a copied pre-0057 database with legacy null target IDs', () => {
    const path = createDatabasePath(); let database = openCoachDatabase({ databasePath: path, migrationsFolder: migrationsThrough0056() }); const workspaceId = crypto.randomUUID(); const sessionId = crypto.randomUUID(); const itemId = crypto.randomUUID()
    database.sqlite.prepare("INSERT INTO workspaces (id,name,objective,status,created_at,updated_at) VALUES (?,'Legacy','','active',1,1)").run(workspaceId); database.sqlite.prepare("INSERT INTO study_sessions (id,workspace_id,status,started_at,focus_seconds) VALUES (?,?,'active',1,0)").run(sessionId, workspaceId); database.sqlite.prepare("INSERT INTO study_plan_items (id,workspace_id,session_id,title,duration_minutes,position,status,activity_type,created_at,updated_at) VALUES (?,?,?,'Exercise',20,1,'active','exercise',1,1)").run(itemId, workspaceId, sessionId); database.close()
    database = openCoachDatabase({ databasePath: path, migrationsFolder }); expect(database.sqlite.prepare('SELECT exercise_set_id AS exerciseSetId,material_id AS materialId FROM study_plan_items WHERE id=?').get(itemId)).toEqual({ exerciseSetId: null, materialId: null }); database.close()
  })
  it('carries exact SQLite mirror targets through StudyWorkspaceService without overriding global authority', async () => {
    const database = openCoachDatabase({ databasePath: createDatabasePath(), migrationsFolder }); const workspaces = new DrizzleWorkspaceRepository(database); const repository = new DrizzleStudyWorkspaceRepository(database); const workspace = await workspaces.create({ id: crypto.randomUUID(), name: 'C', objective: '', createdAt: 1, updatedAt: 1 }); const setId = crypto.randomUUID(); database.sqlite.prepare("INSERT INTO exercise_sets (id,workspace_id,roadmap_id,module_id,topic_id,lesson_id,status,generation_attempts,created_at,updated_at) VALUES (?,?,'r','m','m:t','l','ready',0,1,1)").run(setId, workspace.id)
    const item = { id: crypto.randomUUID(), title: 'mirror stale', durationMinutes: 99, position: 9, status: 'active' as const, moduleId: 'm', topicId: 'm:t', activityType: 'exercise' as const, exerciseSetId: setId }; await repository.createState({ workspaceId: workspace.id, sessionId: crypto.randomUUID(), sessionStartedAt: 1, fileName: 'a', language: 'text', editorContent: '', notes: '', shareContextWithAi: false, timerDurationSeconds: 60, timerRemainingSeconds: 60, timerStatus: 'idle', timerStartedAt: null, plan: [item], updatedAt: 1, documentRevision: 0, notesRevision: 0, accumulatedFocusSeconds: 0 })
    const authority = [{ ...item, title: 'global authority', durationMinutes: 25, position: 1, exerciseSetId: undefined }]; const state = await new StudyWorkspaceService({ repository, getWorkspace: (id) => workspaces.findById(id), getTodayPlan: () => authority }).getState(workspace.id); expect(state.plan[0]).toMatchObject({ title: 'global authority', durationMinutes: 25, position: 1, exerciseSetId: setId }); expect(deriveNextStepCta(state.plan[0])?.exerciseSetId).toBe(setId); database.close()
  })
  ...
  it('persists event Workspace action types and deduplicates repeated discovery', () => { const database = openCoachDatabase({ databasePath: createDatabasePath(), migrationsFolder }); const repository = new DrizzlePlannerActionRepository(database); const eventId = crypto.randomUUID(); const first = repository.create({ id: crypto.randomUUID(), originMessageId: `academic-event-options:${eventId}`, label: 'Manter sem Workspace', contextVersion: 1, type: 'academic.event.keepUnlinked', status: 'proposed', payload: { eventId }, result: null, createdAt: 1, resolvedAt: null }, 'stable-event-option'); const repeated = repository.create({ ...first, id: crypto.randomUUID(), contextVersion: 2 }, 'stable-event-option'); expect(repeated.id).toBe(first.id); expect(repository.listPending()).toHaveLength(1); database.close() })
  it('rolls back event replacement when confirmed alias persistence fails', async () => { const database = openCoachDatabase({ databasePath: createDatabasePath(), migrationsFolder }); const workspaces = new DrizzleWorkspaceRepository(database); const workspace = await workspaces.create({ id: crypto.randomUUID(), name: 'Java', objective: '', createdAt: 1, updatedAt: 1 }); const life = new AcademicLifeService(new SqliteAcademicLifeRepository(database), () => 10); const original = life.save({ kind: 'event', title: 'Prova POO', details: JSON.stringify({ schema: 'academic-event/v1', eventKind: 'exam', subject: 'POO', sourceText: 'prova' }), workspaceId: null, startsAt: null, endsAt: 1000, expiresAt: 1000, timezone: 'UTC', weekday: null, minutes: null, shareWithAi: false, provenance: { source: 'conversation', reference: null } }); const contexts = new AcademicSubjectContextService(new SqliteAcademicSubjectContextRepository(database), () => 10); database.sqlite.exec("CREATE TRIGGER fail_alias BEFORE INSERT ON workspace_academic_contexts BEGIN SELECT RAISE(ABORT, 'alias failed'); END"); const service = new EventWorkspaceLinkService(new SqliteEventWorkspaceLinkRepository(database, life, contexts, (id) => workspaces.findById(id))); await expect(service.link(original.id, workspace.id, 'POO')).rejects.toThrow('alias failed'); expect(life.find(original.id)).toMatchObject({ status: 'active', replacedById: null, workspaceId: null }); expect(life.getProjection().current).toHaveLength(1); database.close() })
  it('atomically links an event, confirms its canonical alias and reuses the replacement on retry', async () => { const database = openCoachDatabase({ databasePath: createDatabasePath(), migrationsFolder }); const workspaces = new DrizzleWorkspaceRepository(database); const workspace = await workspaces.create({ id: crypto.randomUUID(), name: 'Java', objective: '', createdAt: 1, updatedAt: 1 }); const life = new AcademicLifeService(new SqliteAcademicLifeRepository(database), () => 10); const original = life.save({ kind: 'event', title: 'Prova POO', details: JSON.stringify({ schema: 'academic-event/v1', eventKind: 'exam', subject: 'POO', sourceText: 'prova' }), workspaceId: null, startsAt: null, endsAt: 1000, expiresAt: 1000, timezone: 'UTC', weekday: null, minutes: null, shareWithAi: false, provenance: { source: 'conversation', reference: null } }); const contexts = new AcademicSubjectContextService(new SqliteAcademicSubjectContextRepository(database), () => 10); const service = new EventWorkspaceLinkService(new SqliteEventWorkspaceLinkRepository(database, life, contexts, (id) => workspaces.findById(id))); const linked = await service.link(original.id, workspace.id, 'POO'); expect(linked).toMatchObject({ workspaceId: workspace.id, replacesId: original.id, details: original.details, provenance: original.provenance }); expect(database.sqlite.prepare('SELECT relation FROM workspace_academic_contexts WHERE workspace_id=? AND subject=?').get(workspace.id, 'POO')).toEqual({ relation: 'user_selected' }); expect(await service.link(original.id, workspace.id, 'POO')).toEqual(linked); expect(life.getProjection().current).toHaveLength(1); database.close() })
  ...
  it('runs Workspace C through a persisted roadmap, real topic selection, and JIT lesson content', async () => {
    ...
    const calls: string[] = []
    const curricularTopics = (topics: string[]) => topics.map((topic) => { const key = topic.normalize('NFD').replace(/\p{M}/gu, '').replace(/[^a-z0-9]+/gi, '-').toLowerCase(); return { topic, concepts: [{ key, name: topic, aliases: [], domain: 'programming-c' }], assessmentIntents: [{ key: `explain-${key}`, conceptKey: key, objective: `Explicar ${topic}`, evidenceType: 'multiple_choice', difficulty: 'standard', prerequisiteConceptKeys: [] }] } })
    const sendMessage: AIProvider['sendMessage'] = async (request) => {
      calls.push(request.messages[0]?.content ?? '')
      if (calls.length === 1) return { content: JSON.stringify({ title: 'Programação em C', modules: [{ title: 'Tipos, expressões e controle', objective: 'Escrever programas C determinísticos', estimatedMinutes: 120, topics: ['tipos inteiros e conversões', 'if, switch e laços'], curricularTopics: curricularTopics(['tipos inteiros e conversões', 'if, switch e laços']), outcomes: ['Compilar programas com fluxo correto'], practice: 'Construir um conversor de unidades com validação', completionCriteria: ['Compilar sem warnings'], sourceIds: [] }, { title: 'Funções, arrays e memória', objective: 'Modelar dados e memória explicitamente', estimatedMinutes: 180, topics: ['funções e passagem de parâmetros', 'arrays e aritmética de ponteiros', 'malloc, realloc e free', 'structs e composição de dados'], curricularTopics: curricularTopics(['funções e passagem de parâmetros', 'arrays e aritmética de ponteiros', 'malloc, realloc e free', 'structs e composição de dados']), outcomes: ['Gerenciar memória sem vazamentos'], practice: 'Implementar um vetor dinâmico de structs', completionCriteria: ['Liberar toda memória alocada'], sourceIds: [] }] }), providerId: 'openai-compatible', modelId: 'codex/gpt-5.6-sol' }
  ...
  it('publishes one concept across topics and preserves its memory through rebuild', async () => {
    ...
    const memoryBefore = database.sqlite.prepare('SELECT * FROM concept_memories WHERE workspace_id=? AND concept_id=?').get(workspace.id, concept.id)
    const lessonId = crypto.randomUUID(); const checkpointId = 'checkpoint-1'; const checkpointState = { selectedOptionId: 'a', studentJustification: 'porque', attempt: 1, correct: true, feedback: 'ok', rationale: 'ok', answeredAt: 3, history: [] }
    database.sqlite.prepare("INSERT INTO study_lessons (id,workspace_id,roadmap_id,module_id,topic_id,generation_kind,content_json,created_at,updated_at) VALUES (?,?,?,?,?,'ai_generated',?,?,?)").run(lessonId, workspace.id, first.id, moduleId, `${moduleId}:Léxico`, JSON.stringify({ title: 'Léxico', level: 'basic', objective: 'Analisar', blocks: [{ id: 'e1', type: 'explanation', title: 'E', content: 'Conteúdo' }, { id: 'e2', type: 'explanation', title: 'E2', content: 'Conteúdo' }, { id: 'e3', type: 'explanation', title: 'E3', content: 'Conteúdo' }, { id: checkpointId, type: 'checkpoint', title: 'C', questionType: 'multiple_choice', question: 'Q', options: ['a','b','c','d','e'].map((id) => ({ id, text: id, rationale: id })), correctOptionId: 'a', reasoningRequirement: 'none', hint: 'h', reinforcement: 'r' }], sources: [] }), 3, 3)
    database.sqlite.prepare('INSERT INTO study_progress (workspace_id,roadmap_id,current_module_id,current_topic_id,current_lesson_id,current_checkpoint_id,topic_statuses_json,lesson_positions_json,checkpoint_states_json,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)').run(workspace.id, first.id, moduleId, `${moduleId}:Léxico`, lessonId, checkpointId, JSON.stringify({ [`${moduleId}:Léxico`]: 'COMPLETED' }), JSON.stringify({ [lessonId]: { lessonId, blockId: checkpointId, offset: 0, currentCheckpointId: checkpointId, updatedAt: 3 } }), JSON.stringify({ [checkpointId]: checkpointState }), 3)
    const attemptId = crypto.randomUUID(); database.sqlite.prepare("INSERT INTO learning_attempts (id,workspace_id,concept_id,environment,source_ref,source_revision,idempotency_key,payload_hash,outcome,correct,independent,reasoning_quality,occurred_at,created_at) VALUES (?,?,?,'checkpoint',?,'r1',?,?,'correct',1,1,'coherent',3,3)").run(attemptId, workspace.id, concept.id, lessonId, crypto.randomUUID(), 'a'.repeat(64)); database.sqlite.prepare("INSERT INTO learning_evidence (id,attempt_id,concept_id,type,strength,ordinal,metadata_json,occurred_at,created_at) VALUES (?,?,?,'answer','strong',1,'{}',3,3)").run(crypto.randomUUID(), attemptId, concept.id); const attemptBefore = database.sqlite.prepare('SELECT * FROM learning_attempts WHERE id=?').get(attemptId); const evidenceBefore = database.sqlite.prepare('SELECT * FROM learning_evidence WHERE attempt_id=?').get(attemptId)
    const preview = { id: crypto.randomUUID(), workspaceId: workspace.id, currentRoadmapId: first.id, title: 'Compiladores revisto', modules: [{ ...first.modules[0]!, title: 'Front-end revisto', topics: ['Léxico', 'Tokens', 'Parsing'], curricularTopics: [...first.modules[0]!.curricularTopics!, { topic: 'Parsing', concepts: [{ key: 'parser', name: 'Parser', aliases: [], domain: 'compilers' }], assessmentIntents: [{ key: 'parse', conceptKey: 'parser', objective: 'Avaliar parsing', evidenceType: 'multiple_choice' as const, difficulty: 'standard' as const, prerequisiteConceptKeys: ['scanner'] }] }] }], materialIds: [crypto.randomUUID()], impact: { relevance: 'partial' as const, summary: 'Adiciona parsing.', reasons: ['O material cobre parsing.'], preservedModuleIds: [moduleId], preservedTopicIds: [`${moduleId}:Léxico`, `${moduleId}:Tokens`], changedTopicIds: [], addedTopics: [`${moduleId}:Parsing`], removedTopics: [], unsafeProgressTopicIds: [], requiresAcknowledgement: false }, status: 'pending' as const, appliedRoadmapId: null, createdAt: 4, resolvedAt: null }
    repository.saveRebuildPreview(preview)
    const applied = repository.applyRebuildPreview(preview, 5, 'rebuild')
    expect(applied.modules[0]!.id).toBe(moduleId)
    expect(applied.modules[0]!.topics).toEqual(['Léxico', 'Tokens', 'Parsing'])
    expect(database.sqlite.prepare('SELECT id,roadmap_id AS roadmapId,module_id AS moduleId,topic_id AS topicId FROM study_lessons WHERE id=?').get(lessonId)).toEqual({ id: lessonId, roadmapId: applied.id, moduleId, topicId: `${moduleId}:Léxico` })
    expect(database.sqlite.prepare('SELECT current_lesson_id AS lessonId,current_checkpoint_id AS checkpointId,checkpoint_states_json AS checkpoints FROM study_progress WHERE workspace_id=?').get(workspace.id)).toEqual({ lessonId, checkpointId, checkpoints: JSON.stringify({ [checkpointId]: checkpointState }) })
    expect(database.sqlite.prepare('SELECT COUNT(*) AS count FROM learning_attempts WHERE workspace_id=?').get(workspace.id)).toEqual({ count: 1 }); expect(database.sqlite.prepare('SELECT COUNT(*) AS count FROM learning_evidence WHERE concept_id=?').get(concept.id)).toEqual({ count: 1 })
    expect(database.sqlite.prepare('SELECT * FROM learning_attempts WHERE id=?').get(attemptId)).toEqual(attemptBefore); expect(database.sqlite.prepare('SELECT * FROM learning_evidence WHERE attempt_id=?').get(attemptId)).toEqual(evidenceBefore); expect(database.sqlite.prepare('SELECT * FROM concept_memories WHERE workspace_id=? AND concept_id=?').get(workspace.id, concept.id)).toEqual(memoryBefore)
  ...
  it('clears only the live lesson cursor when adaptation changes the current topic', async () => {
    const database = openCoachDatabase({ databasePath: createDatabasePath(), migrationsFolder })
    const workspace = await new DrizzleWorkspaceRepository(database).create({ id: crypto.randomUUID(), name: 'C', objective: 'Aprender C', createdAt: 1, updatedAt: 1 })
    const repository = new DrizzleRoadmapRepository(database); const moduleId = crypto.randomUUID(); const topicId = `${moduleId}:Ponteiros`; const lessonId = crypto.randomUUID(); const checkpointId = 'checkpoint-old'; const checkpointState = { selectedOptionId: 'a', studentJustification: 'histórico', attempt: 1, correct: true, feedback: 'ok', rationale: 'ok', answeredAt: 2, history: [] }
    const curriculum = [{ topic: 'Ponteiros', concepts: [{ key: 'pointers', name: 'Ponteiros', aliases: [], domain: 'c' }], assessmentIntents: [{ key: 'pointer-basics', conceptKey: 'pointers', objective: 'Explicar ponteiros', evidenceType: 'multiple_choice' as const, difficulty: 'standard' as const, prerequisiteConceptKeys: [] }] }]
    const first = { id: crypto.randomUUID(), workspaceId: workspace.id, title: 'C', status: 'accepted' as const, generationKind: 'ai_generated' as const, version: 1, providerId: null, modelId: null, modules: [{ id: moduleId, title: 'Memória', objective: 'Ponteiros', estimatedMinutes: 60, position: 1, status: 'active' as const, topics: ['Ponteiros'], curricularTopics: curriculum, outcomes: ['Usar ponteiros'], practice: 'Código', completionCriteria: ['Explicar'], resources: [] }], createdAt: 1, updatedAt: 1 }; repository.activate(first)
    database.sqlite.prepare("INSERT INTO study_lessons (id,workspace_id,roadmap_id,module_id,topic_id,generation_kind,content_json,created_at,updated_at) VALUES (?,?,?,?,?,'ai_generated',?,?,?)").run(lessonId, workspace.id, first.id, moduleId, topicId, JSON.stringify({ title: 'Antiga', level: 'basic', objective: 'Antiga', blocks: [{ id: 'a', type: 'explanation', title: 'A', content: 'A' }, { id: 'b', type: 'explanation', title: 'B', content: 'B' }, { id: 'c', type: 'explanation', title: 'C', content: 'C' }, { id: 'd', type: 'explanation', title: 'D', content: 'D' }], sources: [] }), 2, 2)
    database.sqlite.prepare('INSERT INTO study_progress (workspace_id,roadmap_id,current_module_id,current_topic_id,current_lesson_id,current_checkpoint_id,topic_statuses_json,lesson_positions_json,checkpoint_states_json,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)').run(workspace.id, first.id, moduleId, topicId, lessonId, checkpointId, JSON.stringify({ [topicId]: 'COMPLETED' }), JSON.stringify({ [lessonId]: { historical: true } }), JSON.stringify({ [checkpointId]: checkpointState }), 2)
    const preview = { id: crypto.randomUUID(), workspaceId: workspace.id, currentRoadmapId: first.id, title: 'C adaptado', modules: first.modules, materialIds: [crypto.randomUUID()], impact: { relevance: 'compatible' as const, summary: 'Aprofunda ponteiros.', reasons: ['Novo enfoque.'], preservedModuleIds: [moduleId], preservedTopicIds: [topicId], changedTopicIds: [topicId], addedTopics: [], removedTopics: [], unsafeProgressTopicIds: [], requiresAcknowledgement: false }, status: 'pending' as const, appliedRoadmapId: null, createdAt: 3, resolvedAt: null }; repository.saveRebuildPreview(preview); const applied = repository.applyRebuildPreview(preview, 4, 'changed-topic')
    expect(database.sqlite.prepare('SELECT roadmap_id AS roadmapId,current_lesson_id AS lessonId,current_checkpoint_id AS checkpointId,topic_statuses_json AS statuses,lesson_positions_json AS positions,checkpoint_states_json AS checkpoints FROM study_progress WHERE workspace_id=?').get(workspace.id)).toEqual({ roadmapId: applied.id, lessonId: null, checkpointId: null, statuses: JSON.stringify({ [topicId]: 'COMPLETED' }), positions: JSON.stringify({ [lessonId]: { historical: true } }), checkpoints: JSON.stringify({ [checkpointId]: checkpointState }) })
    expect(database.sqlite.prepare('SELECT roadmap_id AS roadmapId FROM study_lessons WHERE id=?').get(lessonId)).toEqual({ roadmapId: first.id })
    expect(database.sqlite.prepare('SELECT COUNT(*) AS count FROM study_lessons WHERE roadmap_id=? AND topic_id=?').get(applied.id, topicId)).toEqual({ count: 0 })
    database.close()
  })
